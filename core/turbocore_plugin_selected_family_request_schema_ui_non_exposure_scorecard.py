"""Request/schema/UI non-exposure audit for selected plugin optimizer families."""
...
from core.turbocore_plugin_selected_family_owner_release_hold_scorecard import (
    build_plugin_selected_family_owner_release_hold_scorecard,
)


AUDIT_KIND = "plugin_selected_family_request_schema_ui_non_exposure_v0"
FORBIDDEN_BOUNDARY_TOKENS = (
    "plugin_selected_family_native",
    "plugin_selected_family_canary",
    "plugin_selected_family_dispatch",
    "plugin_selected_family_owner_release",
    "plugin_adamlike_native",
    "plugin_simple_formula_native",
    "plugin_schedulefree_native",
    "plugin_adaptivelr_native",
    "plugin_factored_memory_native",
    "plugin_model_shape_aware_native",
    "plugin_closure_second_order_native",
    "plugin_fused_backward_native",
    "plugin_state_adapter_special_native",
    "plugin_custom_formula_native",
    "turbocore_plugin_selected_family",
    "enable_plugin_selected_family_kernel",
    "enable_plugin_selected_family_native",
    "enable_pytorch_optimizer_native",
    "enable_generic_optimizer_native",
    "pytorch_optimizer_native",
    "generic_optimizer_native",
)
BOUNDARY_PATHS = (
...
def build_plugin_selected_family_request_schema_ui_non_exposure_scorecard(
    *,
    ...
    """Audit product request/schema/UI boundaries while keeping plugin native exposure disabled."""

    root = Path(workspace_root).resolve() if workspace_root is not None else Path(__file__).resolve().parents[2]
    hold_report = _as_dict(
        owner_release_hold_report
        or build_plugin_selected_family_owner_release_hold_scorecard(
    ...
    ready = not blockers
    hold_summary = _as_dict(hold_report.get("summary"))
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_selected_family_request_schema_ui_non_exposure_scorecard_v0",
        "gate": "plugin_selected_family_request_schema_ui_non_exposure",
        "ok": ready,
        "promotion_ready": False,
        "request_schema_ui_non_exposure_ready": ready,
        "owner_release_hold_ready": hold_report.get("owner_release_hold_ready") is True,
        "plugin_optimizer_family_batch_ready": hold_report.get("plugin_optimizer_family_batch_ready") is True,
        ...
        "summary": {
            "request_schema_ui_non_exposure_ready": ready,
            "owner_release_hold_ready": hold_report.get("owner_release_hold_ready") is True,
            "family_count": int(hold_summary.get("family_count", 0) or 0),
            "plugin_optimizer_count": int(hold_summary.get("plugin_optimizer_count", 0) or 0),
        ...
        "promotion_blockers": _dedupe(
            ...
            + [
                ...
                "plugin_selected_family_request_schema_ui_exposure_not_approved",
                "plugin_selected_family_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "await explicit owner/release approval before plugin selected-family request/schema/UI or dispatch wiring"
            if ready
            else "fix plugin selected-family request/schema/UI non-exposure blockers"
        ),
        "notes": [
            "This audit is report-only and does not register request fields, schema fields, UI controls, or routes.",
            "Boundary scanning checks product-facing paths for selected plugin native exposure tokens.",
...
def _validations(
    ...
    return [
        _validation(
            ...
            "plugin_selected_family_owner_release_hold_missing",
        ),
        _validation(
            ...
            "plugin_selected_family_request_schema_ui_unexpected_approval",
        ),
        _validation(
            ...
            "plugin_selected_family_request_schema_ui_hold_enabled_boundary",
        ),
        _validation(
            "boundary_inventory_present",
            len(boundary.get("present_paths", [])) > 0,
            "plugin_selected_family_request_schema_ui_boundary_inventory_missing",
        ),
        _validation(
            "forbidden_tokens_absent",
            not findings.get("forbidden_token_hits"),
            "plugin_selected_family_request_schema_ui_forbidden_boundary_token_found",
        ),
    ]
...
__all__ = ["AUDIT_KIND", "build_plugin_selected_family_request_schema_ui_non_exposure_scorecard"]
