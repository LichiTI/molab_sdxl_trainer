"""TrainingLoop canary for selected plugin NAdam native dispatch."""
...
def build_plugin_nadam_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_nadam_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = _strings(case.get("blocked_reasons"))
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_nadam_training_loop_canary_scorecard_v0",
        "gate": "plugin_nadam_training_loop_native_canary",
        "ok": ok,
        ...
        "summary": {
            ...
            "capturable": case.get("capturable"),
            "foreach": case.get("foreach"),
        },
        "promotion_blockers": blockers
        + [
            "nadam_e2e_shadow_matrix_refresh_required",
            "nadam_canary_rollout_policy_refresh_required",
            "product_rollout_review_missing",
        ],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "refresh selected Adam-like e2e shadow matrix and rollout policy for nadam"
            if ok
            else "fix selected plugin NAdam TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    def _fake_train_step(
        ...
        for item in params:
            item.grad = None
        loss = sum(((item.float() * 0.29) ** 2).mean() + item.float().mean() * 0.007 for item in params)
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_nadam_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_nadam_training_loop_did_not_emit_step"],
        }
    ...
    step_after = int(state["step"].detach().reshape(-1)[0].cpu().item())
    ...
    ok = bool(
        ...
        and step_after == 2
        and type(optimizer).__name__ == "NAdam"
        and str(executor_result.get("optimizer_kind") or "") == "nadam"
        and group.get("capturable") is True
    ...
    return {
        ...
        "blocked_reasons": []
        if ok
        else _dedupe(_strings(runtime.get("blocked_reasons")) + ["plugin_nadam_training_loop_native_step_missing"]),
    }
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    optimizer = _create_selected_nadam_plugin_optimizer(param)
    loop = TrainingLoop(
...
def _create_selected_nadam_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
    ...
    trainer.config = LulynxConfig(
        ...
        learning_rate=1e-3,
        weight_decay=0.01,
        optimizer_args=f"{case.optimizer_args},capturable=True,foreach=False",
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin NAdam TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_nadam_training_loop_canary_scorecard"]
