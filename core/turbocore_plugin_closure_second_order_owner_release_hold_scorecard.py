"""Owner/release hold for selected closure/second-order plugin optimizers."""
...
HOLD_KIND = "plugin_closure_second_order_owner_release_hold_v0"


def build_plugin_closure_second_order_owner_release_hold_scorecard(
    *,
    family_batch_report: Mapping[str, Any] | None = None,
    write_artifact: bool = False,
) -> dict[str, Any]:
    """Record a default-off hold after closure/second-order ABI matrices are ready."""

    batch = _as_dict(family_batch_report or build_plugin_closure_second_order_family_batch_scorecard())
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_closure_second_order_owner_release_hold_scorecard_v0",
        "gate": "plugin_closure_second_order_owner_release_hold",
        "ok": ready,
        "promotion_ready": False,
        "owner_release_hold_ready": ready,
        "family_batch_ready": batch.get("selected_closure_second_order_family_batch_ready") is True,
        ...
        "promotion_blockers": _dedupe(
            blockers
            + [
                "plugin_closure_second_order_owner_approval_missing",
                "plugin_closure_second_order_release_approval_missing",
                "plugin_closure_second_order_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "add closure/second-order request/schema/UI non-exposure guard before product dispatch wiring"
            if ready
            else "fix closure/second-order owner/release hold blockers"
        ),
        "notes": [
            "This gate records a hold state only; it does not approve native dispatch.",
            "Closure and second-order ABI matrices remain report-only until owner/release approval.",
...
def _hold_manifest(batch: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "hold_kind": HOLD_KIND,
        "optimizer_family": "closure_or_second_order",
        ...
        "required_approval_artifacts": [
            ...
            "closure_second_order_training_loop_abi_review",
...
def _validations(batch: Mapping[str, Any], hold: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _validation(
            "family_batch_ready",
            batch.get("selected_closure_second_order_family_batch_ready") is True,
            "plugin_closure_second_order_family_batch_missing",
        ),
        _validation(
            "optimizer_set_complete",
            sorted(hold.get("selected_optimizer_names", [])) == sorted(TARGET_PLUGIN_OPTIMIZERS)
            and optimizer_count == len(TARGET_PLUGIN_OPTIMIZERS),
            "plugin_closure_second_order_optimizer_set_incomplete",
        ),
        _validation(
            "abi_matrices_ready",
            int(summary.get("training_loop_abi_implementation_ready_count", 0) or 0) == optimizer_count
            ...
            == optimizer_count,
            "plugin_closure_second_order_abi_matrices_not_ready",
        ),
        _validation(
        ...
        _validation(
            ...
            "plugin_closure_second_order_batch_enabled_dispatch",
        ),
    ]
...
def _write_artifact(report: Mapping[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_plugin_closure_second_order_owner_release_hold_scorecard.json"
...
__all__ = ["build_plugin_closure_second_order_owner_release_hold_scorecard"]
