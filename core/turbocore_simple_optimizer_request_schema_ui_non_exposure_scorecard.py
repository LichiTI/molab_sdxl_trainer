"""Request/schema/UI non-exposure audit for simple optimizer native canaries."""
...
from core.turbocore_simple_optimizer_owner_release_hold_scorecard import (
...
AUDIT_KIND = "simple_formula_request_schema_ui_non_exposure_v0"
EXPECTED_OPTIMIZERS = (
    "Lion",
    "Lion8bit",
    "PagedLion8bit",
...
FORBIDDEN_BOUNDARY_TOKENS = (
    ...
    "simple_formula_canary",
    "simple_formula_dispatch",
    "simple_formula_owner_release",
    "simple_schedulefree_native",
    "simple_quantized_native",
    "turbocore_simple_formula",
    "enable_simple_formula_kernel",
...
def build_simple_optimizer_request_schema_ui_non_exposure_scorecard(
    *,
    owner_release_hold_report: Mapping[str, Any] | None = None,
    quantized_owner_approval_hold_report: Mapping[str, Any] | None = None,
    schedulefree_owner_release_hold_report: Mapping[str, Any] | None = None,
    ...
    owner_hold = _as_dict(owner_release_hold_report or build_simple_optimizer_owner_release_hold_scorecard())
    quantized_hold = _as_dict(
        quantized_owner_approval_hold_report
        or build_simple_optimizer_quantized_owner_approval_hold_scorecard()
    )
    schedulefree_hold = _as_dict(
        schedulefree_owner_release_hold_report
        or build_simple_optimizer_schedulefree_owner_release_hold_scorecard()
    )
    optimizer_types = _optimizer_types(owner_hold, quantized_hold, schedulefree_hold)
    boundary = _boundary_inventory(root)
    findings = _scan_boundaries(root, boundary["present_paths"])
    validations = _validations(owner_hold, quantized_hold, schedulefree_hold, optimizer_types, boundary, findings)
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_simple_optimizer_request_schema_ui_non_exposure_scorecard_v0",
        "gate": "simple_formula_request_schema_ui_non_exposure",
        "ok": ready,
        "promotion_ready": False,
        "request_schema_ui_non_exposure_ready": ready,
        "owner_release_hold_ready": owner_hold.get("owner_release_hold_ready") is True,
        "quantized_owner_approval_hold_ready": quantized_hold.get("owner_approval_hold_ready") is True,
        "schedulefree_owner_release_hold_ready": schedulefree_hold.get("owner_release_hold_ready") is True,
        ...
        "target_optimizer_types": optimizer_types,
        "boundary_inventory": boundary,
        "forbidden_boundary_tokens": list(FORBIDDEN_BOUNDARY_TOKENS),
        "boundary_findings": findings,
        "owner_release_hold_summary": _as_dict(owner_hold.get("summary")),
        "quantized_owner_approval_hold_summary": _as_dict(quantized_hold.get("summary")),
        "schedulefree_owner_release_hold_summary": _as_dict(schedulefree_hold.get("summary")),
        ...
        "promotion_blockers": _dedupe(
            ...
            + [
                ...
                "simple_formula_request_schema_ui_exposure_not_approved",
                "simple_formula_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "record explicit owner and release approval before simple optimizer request/schema/UI or dispatch wiring"
            if ready
            else "fix simple optimizer request/schema/UI non-exposure blockers"
        ),
        "notes": [
            "This audit is report-only and does not register request fields, schema fields, UI controls, or routes.",
            "Boundary scanning checks product-facing paths for simple optimizer native exposure tokens.",
...
def _validations(
    ...
    quantized_hold: Mapping[str, Any],
    schedulefree_hold: Mapping[str, Any],
    ...
    return [
        _validation(
            ...
            owner_hold.get("owner_release_hold_ready") is True,
            "simple_formula_owner_release_hold_missing",
        ),
        _validation(
            "quantized_owner_approval_hold_ready",
            quantized_hold.get("owner_approval_hold_ready") is True,
            "simple_formula_quantized_owner_approval_hold_missing",
        ),
        _validation(
            "schedulefree_owner_release_hold_ready",
            schedulefree_hold.get("owner_release_hold_ready") is True,
            "simple_formula_schedulefree_owner_release_hold_missing",
        ),
        _validation(
            "optimizer_set_complete",
            tuple(optimizer_types) == EXPECTED_OPTIMIZERS,
            "simple_formula_request_schema_ui_optimizer_set_incomplete",
        ),
        _validation(
            "approval_not_recorded",
            _approvals_absent(owner_hold, quantized_hold, schedulefree_hold),
            "simple_formula_request_schema_ui_unexpected_approval",
        ),
        _validation(
            "holds_kept_product_boundaries_off",
            all(_product_boundaries_off(item) for item in (owner_hold, quantized_hold, schedulefree_hold)),
            "simple_formula_request_schema_ui_hold_enabled_boundary",
        ),
        _validation(
            "boundary_inventory_present",
            len(boundary.get("present_paths", [])) > 0,
            "simple_formula_request_schema_ui_boundary_inventory_missing",
        ),
        _validation(
            "forbidden_tokens_absent",
            not findings.get("forbidden_token_hits"),
            "simple_formula_request_schema_ui_forbidden_boundary_token_found",
        ),
    ]


def _optimizer_types(*reports: Mapping[str, Any]) -> list[str]:
    values: list[str] = []
    for report in reports:
        hold = _as_dict(report.get("hold_manifest"))
        raw = hold.get("optimizer_types")
        if isinstance(raw, list):
            values.extend(str(value) for value in raw if str(value))
    return sorted(set(values))


def _approvals_absent(*reports: Mapping[str, Any]) -> bool:
    return all(
        report.get("owner_approval_recorded") is False and report.get("release_approval_recorded") is False
        for report in reports
    )


def _product_boundaries_off(report: Mapping[str, Any]) -> bool:
    return (
        report.get("request_fields_emitted") is False
...
__all__ = ["AUDIT_KIND", "EXPECTED_OPTIMIZERS", "build_simple_optimizer_request_schema_ui_non_exposure_scorecard"]
