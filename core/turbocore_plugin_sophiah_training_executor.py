"""Default-off SophiaH Triton TrainingLoop executor for selected plugin canaries."""
...
class SophiaHTrainingExecutorConfig:
    optimizer_kind: str = "sophiah"
    lr: float = 6.0e-2
    beta1: float = 0.96
    beta2: float = 0.99
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _sophiah_first_step_dense_kernel(
        param_ptr,
        grad_ptr,
        momentum_ptr,
        hessian_moment_ptr,
        ...
        hessian_moment = tl.load(hessian_moment_ptr + offsets, mask=mask, other=0.0).to(tl.float32)

        momentum_new = momentum * beta1 + grad * (1.0 - beta1)
        denom = tl.maximum(hessian_moment, eps)
        raw_update = momentum_new / denom
        update = tl.minimum(tl.maximum(raw_update, -clip_p), clip_p)
        param_new = param - lr * update

        tl.store(momentum_ptr + offsets, momentum_new, mask=mask)
        tl.store(hessian_moment_ptr + offsets, hessian_moment, mask=mask)
        tl.store(param_ptr + offsets, param_new, mask=mask)


class SophiaHTrainingExecutor:
    """Launch a real SophiaH first-step fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: SophiaHTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("SophiaHTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("sophiah_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("sophiah_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("sophiah_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"sophiah_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("sophiah_maximize_not_supported")
        ...
        if not cases:
            blockers.append("sophiah_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_sophiah_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "sophiah_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("sophiah_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("sophiah_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("sophiah_training_executor_only_first_step_supported_for_canary")
        state = self.optimizer.state[param]
        if "hessian" in state:
            return _case_failed("sophiah_external_hessian_not_supported_for_canary")
        momentum = _state_tensor(state, "momentum", param)
        hessian_moment = _state_tensor(state, "hessian_moment", param)
        before = param.detach().clone()
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _sophiah_first_step_dense_kernel[grid](
            param,
            param.grad.detach(),
            momentum,
            hessian_moment,
        ...
        state["hessian_moment"] = hessian_moment
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["sophiah_training_executor_parameters_not_mutated"],
        }


def build_plugin_sophiah_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: SophiaHTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> SophiaHTrainingExecutor:
    return SophiaHTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: SophiaHTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> SophiaHTrainingExecutorConfig:
    if isinstance(value, SophiaHTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.96, 0.99)))
    return SophiaHTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "sophiah"),
        lr=float(payload.get("lr", group.get("lr", 6.0e-2)) or 6.0e-2),
        beta1=float(betas[0]),
        beta2=float(betas[1]),
        ...
        p=float(payload.get("p", group.get("p", 1.0e-2)) or 1.0e-2),
        eps=float(payload.get("eps", group.get("eps", 1.0e-12)) or 1.0e-12),
        block_size=int(payload.get("block_size", 1024) or 1024),
        require_native_cuda=bool(payload.get("require_native_cuda", True)),
    )


def _group_config(group: Mapping[str, Any], config: SophiaHTrainingExecutorConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "p": float(group.get("p", config.p) or config.p),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("sophiah_weight_decay_not_supported_for_canary")
    return blockers
...
__all__ = [
    "SophiaHTrainingExecutor",
    "SophiaHTrainingExecutorConfig",
    "build_plugin_sophiah_training_executor",
]
