"""Default-off training executor for selected plugin AdamP native dispatch."""
...
class PluginAdamPTrainingExecutorConfig:
    lr: float = 1e-3
...
class PluginAdamPTrainingExecutor:
    """Launch selected AdamP native steps against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: PluginAdamPTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("PluginAdamPTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("plugin_adamp_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("plugin_adamp_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("plugin_adamp_training_dispatch_entrypoints_missing")
        ...
        if not cases:
            blockers.append("plugin_adamp_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_adamp_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "plugin_adamp_native_step_failed"),
    ...
    def _step_param(
        ...
        if type(self.optimizer).__name__.lower() != "adamp":
            return _case_failed("plugin_adamp_training_executor_optimizer_not_adamp", "plugin_adamp_optimizer_class_unsupported")
        if bool(group_cfg.get("maximize", False)):
            return _case_failed("plugin_adamp_native_step_requires_plain_gradient", "plugin_adamp_gradient_mode_unsupported")
        if bool(group_cfg.get("use_gc", False)) or bool(group_cfg.get("adanorm", False)) or bool(group_cfg.get("cautious", False)):
            return _case_failed("plugin_adamp_native_step_requires_plain_adamp", "plugin_adamp_variant_unsupported")
        if bool(group_cfg.get("nesterov", False)):
            return _case_failed("plugin_adamp_native_step_nesterov_unsupported", "plugin_adamp_nesterov_unsupported")
        if not bool(group_cfg.get("weight_decouple", True)) and float(group_cfg.get("weight_decay", 0.0)) != 0.0:
            return _case_failed("plugin_adamp_native_step_coupled_decay_unsupported", "plugin_adamp_weight_decay_mode_unsupported")
        if param.dtype != torch.float32 or param.grad is None or param.grad.dtype != torch.float32:
            return _case_failed("plugin_adamp_native_step_requires_float32", "plugin_adamp_native_step_dtype_unsupported")
        if not param.is_contiguous() or not param.grad.is_contiguous():
            return _case_failed("plugin_adamp_native_step_requires_contiguous_param_grad", "plugin_adamp_native_step_layout_unsupported")
        state = self.optimizer.state[param]
        if "exp_avg" not in state or "exp_avg_sq" not in state:
            return _case_failed("plugin_adamp_live_state_missing", "plugin_adamp_live_state_missing")
        exp_avg = state["exp_avg"]
        exp_avg_sq = state["exp_avg_sq"]
        if not (torch.is_tensor(exp_avg) and torch.is_tensor(exp_avg_sq)):
            return _case_failed("plugin_adamp_live_state_tensors_missing", "plugin_adamp_live_state_missing")
        if exp_avg.dtype != torch.float32 or exp_avg_sq.dtype != torch.float32:
            return _case_failed("plugin_adamp_native_state_requires_float32", "plugin_adamp_native_state_dtype_unsupported")
        if not exp_avg.is_contiguous() or not exp_avg_sq.is_contiguous():
            return _case_failed("plugin_adamp_native_state_requires_contiguous", "plugin_adamp_native_state_layout_unsupported")
        registry = TurboCoreTensorHandleRegistry(namespace="plugin_adamp_training_executor")
        ...
        try:
            ...
            if not bool(session.get("ok", False)):
                return _case_failed(
                    "plugin_adamp_tensor_binding_session_create_failed",
                    "plugin_adamp_tensor_binding_session_create_failed",
                    session=session,
                )
            ...
            try:
                launch = dict(
                    native.tensor_binding_session_cuda_adamp_tensor_probe(
                        session_id,
                        json.dumps(
                            {
                                **dict(group_cfg),
            ...
            return _case_failed(f"{type(exc).__name__}: {exc}", "plugin_adamp_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(
                str(launch.get("reason") or "plugin_adamp_native_step_failed"),
                "plugin_adamp_native_step_failed",
                launch=launch,
            )
        return {
            "schema_version": 1,
...
def build_plugin_adamp_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: PluginAdamPTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> PluginAdamPTrainingExecutor:
    return PluginAdamPTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: PluginAdamPTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> PluginAdamPTrainingExecutorConfig:
    if isinstance(value, PluginAdamPTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return PluginAdamPTrainingExecutorConfig(
        ...
        delta=float(payload.get("delta", group.get("delta", 0.1))),
        wd_ratio=float(payload.get("wd_ratio", group.get("wd_ratio", 0.1))),
        block_size=int(payload.get("block_size", 128)),
        require_native_cuda=bool(payload.get("require_native_cuda", True)),
    )


def _group_config(group: Mapping[str, Any], config: PluginAdamPTrainingExecutorConfig, optimizer: torch.optim.Optimizer) -> dict[str, Any]:
    ...
    return {
        ...
        "delta": float(group.get("delta", config.delta)),
        "wd_ratio": float(group.get("wd_ratio", config.wd_ratio)),
        "nesterov": bool(group.get("nesterov", False)),
        "adam_debias": bool(group.get("adam_debias", False)),
        "maximize": bool(group.get("maximize", getattr(optimizer, "maximize", False))),
        "use_gc": bool(group.get("use_gc", False)),
        "adanorm": bool(group.get("adanorm", False)),
        "cautious": bool(group.get("cautious", False)),
    }


def _matrix_shape(param: torch.nn.Parameter) -> tuple[int, int]:
    if param.ndim <= 1:
        return 1, int(param.numel())
...
__all__ = [
    "PluginAdamPTrainingExecutor",
    "PluginAdamPTrainingExecutorConfig",
    "build_plugin_adamp_training_executor",
]
