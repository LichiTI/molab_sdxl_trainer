"""Default-off training executor for selected plugin Adamax native dispatch."""
...
ENTRYPOINTS = (
    ...
    "tensor_binding_session_cuda_adamax_tensor_probe",
...
class PluginAdamaxTrainingExecutorConfig:
    lr: float = 1e-3
...
class PluginAdamaxTrainingExecutor:
    """Launch selected Adamax native steps against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: PluginAdamaxTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("PluginAdamaxTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("plugin_adamax_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("plugin_adamax_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("plugin_adamax_training_dispatch_entrypoints_missing")
        ...
        if not cases:
            blockers.append("plugin_adamax_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_adamax_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "plugin_adamax_native_step_failed"),
    ...
    def _step_param(
        ...
        if type(self.optimizer).__name__.lower() != "adamax":
            return _case_failed("plugin_adamax_training_executor_optimizer_not_adamax", "plugin_adamax_optimizer_class_unsupported")
        if bool(group_cfg.get("weight_decouple", False)) or bool(group_cfg.get("fixed_decay", False)):
            return _case_failed("plugin_adamax_native_step_requires_coupled_weight_decay", "plugin_adamax_weight_decay_mode_unsupported")
        if bool(group_cfg.get("maximize", False)) or bool(group_cfg.get("adanorm", False)):
            return _case_failed("plugin_adamax_native_step_requires_plain_gradient", "plugin_adamax_gradient_mode_unsupported")
        if param.dtype != torch.float32 or param.grad is None or param.grad.dtype != torch.float32:
            return _case_failed("plugin_adamax_native_step_requires_float32", "plugin_adamax_native_step_dtype_unsupported")
        if not param.is_contiguous() or not param.grad.is_contiguous():
            return _case_failed(
                "plugin_adamax_native_step_requires_contiguous_param_grad",
                "plugin_adamax_native_step_layout_unsupported",
            )
        state = self.optimizer.state[param]
        if "exp_avg" not in state or "exp_inf" not in state:
            return _case_failed("plugin_adamax_live_state_missing", "plugin_adamax_live_state_missing")
        exp_avg = state["exp_avg"]
        exp_inf = state["exp_inf"]
        if not (torch.is_tensor(exp_avg) and torch.is_tensor(exp_inf)):
            return _case_failed("plugin_adamax_live_state_tensors_missing", "plugin_adamax_live_state_missing")
        if exp_avg.dtype != torch.float32 or exp_inf.dtype != torch.float32:
            return _case_failed("plugin_adamax_native_state_requires_float32", "plugin_adamax_native_state_dtype_unsupported")
        if not exp_avg.is_contiguous() or not exp_inf.is_contiguous():
            return _case_failed("plugin_adamax_native_state_requires_contiguous", "plugin_adamax_native_state_layout_unsupported")
        registry = TurboCoreTensorHandleRegistry(namespace="plugin_adamax_training_executor")
        param_flat = param.detach().reshape(-1)
        grad_flat = param.grad.detach().reshape(-1)
        exp_avg_flat = exp_avg.reshape(-1)
        exp_inf_flat = exp_inf.reshape(-1)
        try:
            handles = registry.register_flat_adamw_buffers(
                param_flat=param_flat,
                grad_flat=grad_flat,
                exp_avg=exp_avg_flat,
                exp_avg_sq=exp_inf_flat,
            )
            ...
            if not bool(session.get("ok", False)):
                return _case_failed(
                    "plugin_adamax_tensor_binding_session_create_failed",
                    "plugin_adamax_tensor_binding_session_create_failed",
                    session=session,
                )
            ...
            try:
                launch = dict(
                    native.tensor_binding_session_cuda_adamax_tensor_probe(
                        session_id,
                        json.dumps(
                            {
                                **dict(group_cfg),
            ...
            return _case_failed(f"{type(exc).__name__}: {exc}", "plugin_adamax_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(
                str(launch.get("reason") or "plugin_adamax_native_step_failed"),
                "plugin_adamax_native_step_failed",
                launch=launch,
            )
        return {
            "schema_version": 1,
...
def build_plugin_adamax_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: PluginAdamaxTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> PluginAdamaxTrainingExecutor:
    return PluginAdamaxTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: PluginAdamaxTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> PluginAdamaxTrainingExecutorConfig:
    if isinstance(value, PluginAdamaxTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return PluginAdamaxTrainingExecutorConfig(
...
def _group_config(group: Mapping[str, Any], config: PluginAdamaxTrainingExecutorConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "maximize": bool(getattr(group, "maximize", False) or group.get("maximize", False)),
...
__all__ = [
    "PluginAdamaxTrainingExecutor",
    "PluginAdamaxTrainingExecutorConfig",
    "build_plugin_adamax_training_executor",
]
