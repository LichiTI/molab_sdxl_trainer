"""Default-off training executor for selected plugin AdamC native dispatch."""
...
ENTRYPOINTS = (
    ...
    "tensor_binding_session_cuda_adamc_tensor_probe",
...
class PluginAdamCTrainingExecutorConfig:
    lr: float = 1e-3
...
class PluginAdamCTrainingExecutor:
    """Launch selected AdamC native steps against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: PluginAdamCTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("PluginAdamCTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("plugin_adamc_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("plugin_adamc_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("plugin_adamc_training_dispatch_entrypoints_missing")
        ...
        if not cases:
            blockers.append("plugin_adamc_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_adamc_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "plugin_adamc_native_step_failed"),
    ...
    def _step_param(
        ...
        if type(self.optimizer).__name__.lower() != "adamc":
            return _case_failed("plugin_adamc_training_executor_optimizer_not_adamc", "plugin_adamc_optimizer_class_unsupported")
        if not bool(group_cfg.get("weight_decouple", False)) or bool(group_cfg.get("fixed_decay", False)):
            return _case_failed("plugin_adamc_native_step_requires_default_decoupled_decay", "plugin_adamc_weight_decay_mode_unsupported")
        if bool(group_cfg.get("ams_bound", False)):
            return _case_failed("plugin_adamc_native_step_ams_bound_unsupported", "plugin_adamc_ams_bound_unsupported")
        if bool(group_cfg.get("maximize", False)):
            return _case_failed("plugin_adamc_native_step_requires_plain_gradient", "plugin_adamc_gradient_mode_unsupported")
        if param.dtype != torch.float32 or param.grad is None or param.grad.dtype != torch.float32:
            return _case_failed("plugin_adamc_native_step_requires_float32", "plugin_adamc_native_step_dtype_unsupported")
        if not param.is_contiguous() or not param.grad.is_contiguous():
            return _case_failed("plugin_adamc_native_step_requires_contiguous_param_grad", "plugin_adamc_native_step_layout_unsupported")
        state = self.optimizer.state[param]
        if "exp_avg" not in state or "exp_avg_sq" not in state:
            return _case_failed("plugin_adamc_live_state_missing", "plugin_adamc_live_state_missing")
        exp_avg = state["exp_avg"]
        exp_avg_sq = state["exp_avg_sq"]
        if not (torch.is_tensor(exp_avg) and torch.is_tensor(exp_avg_sq)):
            return _case_failed("plugin_adamc_live_state_tensors_missing", "plugin_adamc_live_state_missing")
        if exp_avg.dtype != torch.float32 or exp_avg_sq.dtype != torch.float32:
            return _case_failed("plugin_adamc_native_state_requires_float32", "plugin_adamc_native_state_dtype_unsupported")
        if not exp_avg.is_contiguous() or not exp_avg_sq.is_contiguous():
            return _case_failed("plugin_adamc_native_state_requires_contiguous", "plugin_adamc_native_state_layout_unsupported")
        registry = TurboCoreTensorHandleRegistry(namespace="plugin_adamc_training_executor")
        ...
        try:
            ...
            if not bool(session.get("ok", False)):
                return _case_failed(
                    "plugin_adamc_tensor_binding_session_create_failed",
                    "plugin_adamc_tensor_binding_session_create_failed",
                    session=session,
                )
            ...
            try:
                launch = dict(
                    native.tensor_binding_session_cuda_adamc_tensor_probe(
                        session_id,
                        json.dumps(
                            {
                                **dict(group_cfg),
            ...
            return _case_failed(f"{type(exc).__name__}: {exc}", "plugin_adamc_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(
                str(launch.get("reason") or "plugin_adamc_native_step_failed"),
                "plugin_adamc_native_step_failed",
                launch=launch,
            )
        return {
            "schema_version": 1,
...
def build_plugin_adamc_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: PluginAdamCTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> PluginAdamCTrainingExecutor:
    return PluginAdamCTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: PluginAdamCTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> PluginAdamCTrainingExecutorConfig:
    if isinstance(value, PluginAdamCTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return PluginAdamCTrainingExecutorConfig(
...
def _group_config(group: Mapping[str, Any], config: PluginAdamCTrainingExecutorConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "ams_bound": bool(group.get("ams_bound", False)),
...
__all__ = [
    "PluginAdamCTrainingExecutor",
    "PluginAdamCTrainingExecutorConfig",
    "build_plugin_adamc_training_executor",
]
