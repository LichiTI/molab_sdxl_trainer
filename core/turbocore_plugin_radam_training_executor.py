"""Default-off training executor for selected plugin RAdam native dispatch."""
...
ENTRYPOINT = "probe_radam_training_tensor_binding_canary_py"
REPO_ROOT = Path(__file__).resolve().parents[2]


@dataclass(frozen=True)
class PluginRAdamTrainingExecutorConfig:
    lr: float = 1e-3
...
class PluginRAdamTrainingExecutor:
    """Launch selected RAdam native steps against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: PluginRAdamTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("PluginRAdamTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("plugin_radam_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("plugin_radam_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("plugin_radam_training_dispatch_entrypoint_missing")
        ...
        if not cases:
            blockers.append("plugin_radam_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_radam_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "plugin_radam_native_step_failed"),
    ...
    def _step_param(
        ...
        if type(self.optimizer).__name__.lower() != "radam":
            return _case_failed("plugin_radam_training_executor_optimizer_not_radam", "plugin_radam_optimizer_class_unsupported")
        if bool(group_cfg.get("adanorm", False)):
            return _case_failed("plugin_radam_native_step_adanorm_unsupported", "plugin_radam_variant_unsupported")
        if param.dtype != torch.float32 or param.grad is None or param.grad.dtype != torch.float32:
            return _case_failed("plugin_radam_native_step_requires_float32", "plugin_radam_native_step_dtype_unsupported")
        if not param.is_contiguous() or not param.grad.is_contiguous():
            return _case_failed("plugin_radam_native_step_requires_contiguous_param_grad", "plugin_radam_native_step_layout_unsupported")
        state = self.optimizer.state[param]
        if "exp_avg" not in state or "exp_avg_sq" not in state:
            return _case_failed("plugin_radam_live_state_missing", "plugin_radam_live_state_missing")
        exp_avg = state["exp_avg"]
        exp_avg_sq = state["exp_avg_sq"]
        if not (torch.is_tensor(exp_avg) and torch.is_tensor(exp_avg_sq)):
            return _case_failed("plugin_radam_live_state_tensors_missing", "plugin_radam_live_state_missing")
        if exp_avg.dtype != torch.float32 or exp_avg_sq.dtype != torch.float32:
            return _case_failed("plugin_radam_native_state_requires_float32", "plugin_radam_native_state_dtype_unsupported")
        if not exp_avg.is_contiguous() or not exp_avg_sq.is_contiguous():
            return _case_failed("plugin_radam_native_state_requires_contiguous", "plugin_radam_native_state_layout_unsupported")
        launch_config = {
            **dict(group_cfg),
        ...
        try:
            ...
            return _case_failed(f"{type(exc).__name__}: {exc}", "plugin_radam_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(
                str(launch.get("reason") or "plugin_radam_native_step_failed"),
                "plugin_radam_native_step_failed",
                launch=launch,
            )
        return {
            "schema_version": 1,
...
def build_plugin_radam_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: PluginRAdamTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> PluginRAdamTrainingExecutor:
    return PluginRAdamTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: PluginRAdamTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> PluginRAdamTrainingExecutorConfig:
    if isinstance(value, PluginRAdamTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return PluginRAdamTrainingExecutorConfig(
        ...
        degenerated_to_sgd=bool(payload.get("degenerated_to_sgd", getattr(optimizer, "degenerated_to_sgd", False))),
...
def _group_config(
    ...
    config: PluginRAdamTrainingExecutorConfig,
    optimizer: torch.optim.Optimizer,
) -> dict[str, Any]:
    ...
    return {
        ...
        "n_sma_threshold": int(getattr(optimizer, "n_sma_threshold", config.n_sma_threshold) or 5),
        "degenerated_to_sgd": bool(getattr(optimizer, "degenerated_to_sgd", config.degenerated_to_sgd)),
...
__all__ = [
    "PluginRAdamTrainingExecutor",
    "PluginRAdamTrainingExecutorConfig",
    "build_plugin_radam_training_executor",
]
