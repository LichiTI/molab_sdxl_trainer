"""TrainingLoop canary for selected plugin Adamax native dispatch."""
...
def build_plugin_adamax_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_adamax_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = list(case.get("blocked_reasons", []) or [])
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_adamax_training_loop_canary_scorecard_v0",
        "gate": "plugin_adamax_training_loop_native_canary",
        "ok": ok,
        ...
        "native_route": "rust_cuda_plugin_adamax_v0",
        ...
        "recommended_next_step": (
            "add selected Adamax e2e shadow matrix and rollout policy"
            if ok
            else "fix selected plugin Adamax TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    def _fake_train_step(
        ...
        for item in params:
            item.grad = None
        loss = sum(((item.float() * 0.31) ** 2).mean() + item.float().mean() * 0.006 for item in params)
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_adamax_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_adamax_training_loop_did_not_emit_step"],
        }
    ...
    step_after = _group_step_to_int(optimizer.param_groups[0])
    ok = bool(
        result.get("steps") == 1
        and native_step
        and native_kernel
        ...
        and step_after == 2
        and type(optimizer).__name__ == "AdaMax"
        and _has_adamax_state(param, optimizer)
    )
    return {
        "schema_version": 1,
        "ok": ok,
        ...
        "blocked_reasons": []
        if ok
        else _dedupe(list(runtime.get("blocked_reasons", []) or []) + ["plugin_adamax_training_loop_native_step_missing"]),
    }


def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    device = torch.device("cuda")
    param = torch.nn.Parameter(torch.linspace(-0.16, 0.26, steps=4096, device=device, dtype=torch.float32))
    optimizer = _create_selected_adamax_plugin_optimizer(param)
    loop = TrainingLoop(
        ...
        turbocore_native_update_quantized_optimizer_kind="adamax",
    )
    loop.total_steps = 1
    return loop, param, optimizer


def _create_selected_adamax_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
    trainer = LulynxTrainer.__new__(LulynxTrainer)
    case = plugin_resume_case("adamax")
...
def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> None:
    param.grad = None
    loss = ((param.float() * 0.19) ** 2).mean() + param.float().mean() * 0.004
    loss.backward()
    optimizer.step()
...
def _group_step_to_int(group: dict[str, Any]) -> int:
...
def _has_adamax_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> bool:
    state = optimizer.state[param]
    return torch.is_tensor(state.get("exp_avg")) and torch.is_tensor(state.get("exp_inf"))
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin Adamax TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_adamax_training_loop_canary_scorecard"]
