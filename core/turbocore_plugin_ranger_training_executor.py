"""Default-off training executor for selected plugin Ranger native dispatch."""
...
ENTRYPOINT = "probe_ranger_training_tensor_binding_canary_py"
REPO_ROOT = Path(__file__).resolve().parents[2]


@dataclass(frozen=True)
class PluginRangerTrainingExecutorConfig:
    lr: float = 1e-3
...
class PluginRangerTrainingExecutor:
    """Launch selected Ranger native steps against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: PluginRangerTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("PluginRangerTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("plugin_ranger_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("plugin_ranger_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("plugin_ranger_training_dispatch_entrypoint_missing")
        ...
        if not cases:
            blockers.append("plugin_ranger_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_ranger_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "plugin_ranger_native_step_failed"),
            "optimizer_kind": "ranger",
    ...
    def _step_param(
        ...
        if type(self.optimizer).__name__.lower() != "ranger":
            return _case_failed("plugin_ranger_training_executor_optimizer_not_ranger", "plugin_ranger_optimizer_class_unsupported")
        if param.dtype != torch.float32 or param.grad is None or param.grad.dtype != torch.float32:
            return _case_failed("plugin_ranger_native_step_requires_float32", "plugin_ranger_native_step_dtype_unsupported")
        if not param.is_contiguous() or not param.grad.is_contiguous():
            return _case_failed("plugin_ranger_native_step_requires_contiguous_param_grad", "plugin_ranger_native_step_layout_unsupported")
        state = self.optimizer.state[param]
        required = ("exp_avg", "exp_avg_sq", "slow_buffer")
        if not all(name in state for name in required):
            return _case_failed("plugin_ranger_live_state_missing", "plugin_ranger_live_state_missing")
        exp_avg = state["exp_avg"]
        exp_avg_sq = state["exp_avg_sq"]
        slow_buffer = state["slow_buffer"]
        state_tensors = (exp_avg, exp_avg_sq, slow_buffer)
        if not all(torch.is_tensor(item) for item in state_tensors):
            return _case_failed("plugin_ranger_live_state_tensors_missing", "plugin_ranger_live_state_missing")
        if any(item.dtype != torch.float32 for item in state_tensors):
            return _case_failed("plugin_ranger_native_state_requires_float32", "plugin_ranger_native_state_dtype_unsupported")
        if not all(item.device.type == "cuda" for item in state_tensors):
            return _case_failed("plugin_ranger_native_state_requires_cuda", "plugin_ranger_native_state_device_unsupported")
        if not all(item.is_contiguous() for item in state_tensors):
            return _case_failed("plugin_ranger_native_state_requires_contiguous", "plugin_ranger_native_state_layout_unsupported")
        launch_config = {
            **dict(group_cfg),
        ...
        try:
            ...
            return _case_failed(f"{type(exc).__name__}: {exc}", "plugin_ranger_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(
                str(launch.get("reason") or "plugin_ranger_native_step_failed"),
                "plugin_ranger_native_step_failed",
                launch=launch,
            )
        return {
            "schema_version": 1,
...
def build_plugin_ranger_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: PluginRangerTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> PluginRangerTrainingExecutor:
    return PluginRangerTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: PluginRangerTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> PluginRangerTrainingExecutorConfig:
    if isinstance(value, PluginRangerTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.95, 0.999)))
    return PluginRangerTrainingExecutorConfig(
        lr=float(payload.get("lr", group.get("lr", 1e-3))),
        betas=(float(betas[0]), float(betas[1])),
        alpha=float(payload.get("alpha", group.get("alpha", 0.5))),
        k=int(payload.get("k", group.get("k", 6))),
        n_sma_threshold=int(payload.get("n_sma_threshold", getattr(optimizer, "n_sma_threshold", 5) or 5)),
        degenerated_to_sgd=bool(
            payload.get("degenerated_to_sgd", getattr(optimizer, "degenerated_to_sgd", False))
        ),
        eps=float(payload.get("eps", group.get("eps", 1e-5))),
...
def _group_config(group: Mapping[str, Any], config: PluginRangerTrainingExecutorConfig) -> dict[str, Any]:
    betas = group.get("betas", config.betas)
    return {
        "lr": float(group.get("lr", config.lr)),
        "betas": [float(betas[0]), float(betas[1])],
        "alpha": float(group.get("alpha", config.alpha)),
        "k": int(group.get("k", config.k)),
        "n_sma_threshold": int(config.n_sma_threshold),
        "degenerated_to_sgd": bool(config.degenerated_to_sgd),
...
__all__ = [
    "PluginRangerTrainingExecutor",
    "PluginRangerTrainingExecutorConfig",
    "build_plugin_ranger_training_executor",
]
