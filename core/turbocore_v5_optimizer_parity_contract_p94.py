"""Optimizer parity contract for TurboCore V5-P94.

P94 records the review shape for a future optimizer parity stage. It
...
from core.turbocore_v5_optimizer_tensor_transfer_contract_p93 import (
    P93_READY_DECISION,
    UNSAFE_NON_EMPTY_FIELDS as _P93_UNSAFE_NON_EMPTY_FIELDS,
    UNSAFE_TRUE_FIELDS as _P93_UNSAFE_TRUE_FIELDS,
)
from core.turbocore_v5_owner_review_evidence_package import load_json


P94_READY_DECISION = "optimizer_parity_contract_p94_recorded_default_off"
P94_BLOCKED_DECISION = "optimizer_parity_contract_p94_blocked_default_off"
P94_HOLD_DECISION = "optimizer_parity_contract_p94_hold_for_signed_review_default_off"
P94_REJECTED_DECISION = "optimizer_parity_contract_p94_rejected_default_off"
P94_SCOPE = "optimizer_parity_contract"
ALLOWED_REVIEW_INTENTS = {"parity_candidate", "hold_for_more_evidence", "reject_parity"}
REQUIRED_SECTIONS = (
    "p93_optimizer_tensor_transfer_contract_reference",
    "optimizer_parity_package",
    "per_optimizer_parity_rows",
...
REQUIRED_REVIEW_ACKS = (
    "acknowledge_p93_optimizer_tensor_transfer_contract_recorded",
    "acknowledge_optimizer_parity_contract_only",
    ...
    "acknowledge_later_optimizer_training_step_contract_required",
    "acknowledge_manual_review_only",
)
UNSAFE_TRUE_FIELDS = tuple(
    dict.fromkeys(
        (
            *_P93_UNSAFE_TRUE_FIELDS,
...
UNSAFE_NON_EMPTY_FIELDS = tuple(
    dict.fromkeys(
        (
            *_P93_UNSAFE_NON_EMPTY_FIELDS,
            "post_p94_request_fields",
...
def build_v5_optimizer_parity_contract_p94(
    *,
    p93_optimizer_tensor_transfer_contract: Mapping[str, Any] | None = None,
    optimizer_parity_evidence: Mapping[str, Any] | None = None,
    optimizer_parity_signed_review: Mapping[str, Any] | None = None,
    failure_history: Sequence[Any] | None = None,
    rollback_history: Sequence[Any] | None = None,
) -> dict[str, Any]:
    """Record optimizer parity evidence without launching kernels."""

    p93 = _as_dict(p93_optimizer_tensor_transfer_contract)
    evidence = _as_dict(optimizer_parity_evidence)
    review = _as_dict(optimizer_parity_signed_review)
    failure_events = _event_list(failure_history)
    rollback_events = _event_list(rollback_history)
    p93_summary = _p93_summary(p93)
    evidence_summary = _evidence_summary(evidence)
    review_summary = _review_summary(review)
    evidence_blockers = _evidence_blockers(p93_summary, evidence_summary, failure_events, rollback_events)
    ...
    if decision == P94_HOLD_DECISION:
        blockers.append("v5_p94_signed_optimizer_parity_review_missing")
    blockers = _dedupe(blockers)
    ready = decision == P94_READY_DECISION
    rejected = decision == P94_REJECTED_DECISION
    decision_record_ready = ready or rejected
    return {
        "schema_version": 1,
        "package": "turbocore_v5_optimizer_parity_contract_p94_v0",
        "gate": "v5_optimizer_parity_contract_p94",
        "ok": decision_record_ready,
        "decision_record_ready": decision_record_ready,
        "optimizer_parity_recorded": decision_record_ready,
        "optimizer_parity_signed": decision_record_ready,
        "optimizer_parity_contract_ready": ready,
        "optimizer_parity_evidence_recorded": ready,
        ...
        **{field: False for field in UNSAFE_TRUE_FIELDS},
        "post_p94_request_fields": {},
        "p93_optimizer_tensor_transfer_summary": p93_summary,
        "optimizer_parity_evidence_summary": evidence_summary,
        "optimizer_parity_signed_review": review_summary,
        "optimizer_parity_review_template": _review_template(),
        "progress_gates": _progress_gates(p93_summary, evidence_summary, review_summary),
        ...
        "notes": [
            "P94 records optimizer parity evidence only.",
            "P94 does not execute kernel launch, tensor transfer, or parity.",
            "A later optimizer training-step contract is still required.",
        ],
    }


def _p93_summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "optimizer_tensor_transfer_contract_ready": report.get("optimizer_tensor_transfer_contract_ready") is True,
        "optimizer_tensor_transfer_evidence_recorded": report.get("optimizer_tensor_transfer_evidence_recorded") is True,
        "optimizer_tensor_transfer_signed": report.get("optimizer_tensor_transfer_signed") is True,
        "decision": decision,
        ...
        "post_fields_empty": not bool(_as_dict(report.get("post_p93_request_fields"))),
        "unsafe_claims": _unsafe_claims(report, "p93"),
...
def _evidence_summary(evidence: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "ready": not blockers,
        "optimizer_parity_package_ready": evidence.get("optimizer_parity_package_ready") is True,
        "parity_policy_ready": evidence.get("parity_policy_ready") is True,
        "later_optimizer_training_step_contract_required": (
            evidence.get("later_optimizer_training_step_contract_required") is True
        ),
...
def _review_summary(review: Mapping[str, Any]) -> dict[str, Any]:
    summary = {
        ...
        "approve_optimizer_parity_contract": review.get("approve_optimizer_parity_contract") is True,
    }
...
def _progress_gates(p93: Mapping[str, Any], evidence: Mapping[str, Any], review: Mapping[str, Any]) -> dict[str, bool]:
    return {
        "p93_optimizer_tensor_transfer_contract_ready": _p93_ready(p93),
        "optimizer_parity_evidence_ready": bool(evidence.get("ready", False)),
        "signed_optimizer_parity_review_present": bool(review.get("present", False)),
...
def _evidence_blockers(
    ...
    blockers: list[str] = []
    if not _p93_ready(p93):
        blockers.append("v5_p94_p93_optimizer_tensor_transfer_contract_not_ready")
    blockers.extend(f"v5_p94_unsafe_upstream_claim:p93:{item}" for item in p93.get("unsafe_claims") or [])
    if not evidence.get("present"):
        blockers.append("v5_p94_optimizer_parity_evidence_missing")
    blockers.extend(_string_list(evidence.get("blockers")))
    if failure_events:
        blockers.extend(f"v5_p94_failure_history_not_clear:{item}" for item in failure_events)
    if rollback_events:
        blockers.extend(f"v5_p94_rollback_history_not_clear:{item}" for item in rollback_events)
...
def _evidence_blocker_list(
    ...
    checks = (
        ...
        ("optimizer_parity_package_ready", True, "package_not_ready"),
        ("parity_policy_ready", True, "policy_not_ready"),
        ("later_optimizer_training_step_contract_required", True, "later_training_step_contract_missing"),
    ...
    for field, expected, reason in checks:
        if evidence.get(field) is not expected:
            blockers.append(f"v5_p94_optimizer_parity_evidence_{reason}")
    if str(evidence.get("review_intent") or "") not in ALLOWED_REVIEW_INTENTS:
        blockers.append("v5_p94_optimizer_parity_review_intent_invalid")
    if not evidence.get("default_off") or not _default_off_confirmed(evidence):
        blockers.append("v5_p94_optimizer_parity_evidence_default_off_violation")
    if not evidence.get("request_adapter_off") or not _request_adapter_off(evidence):
        blockers.append("v5_p94_optimizer_parity_evidence_request_adapter_boundary_violation")
    if not _source(evidence):
        blockers.append("v5_p94_optimizer_parity_evidence_source_missing")
    if not _digest(evidence):
        blockers.append("v5_p94_optimizer_parity_evidence_digest_missing")
    blockers.extend(f"v5_p94_required_section_missing:{item}" for item in missing_sections)
    blockers.extend(f"v5_p94_optimizer_parity_row_missing:{item}" for item in missing_optimizers)
    blockers.extend(row_blockers)
    blockers.extend(_unsafe_claims(evidence, "optimizer_parity_evidence"))
    blockers.extend(_non_empty_claims(evidence, "optimizer_parity_evidence"))
...
def _review_blockers(review: Mapping[str, Any]) -> list[str]:
    if not review.get("present"):
        return []
    blockers: list[str] = []
    if review.get("requested_scope") != P94_SCOPE:
        blockers.append("v5_p94_review_scope_mismatch")
    if not review.get("reviewer") or not review.get("reviewed_at"):
        blockers.append("v5_p94_review_identity_or_timestamp_missing")
    for field in REQUIRED_REVIEW_ACKS:
        if review.get(field) is not True:
            blockers.append(f"v5_p94_review_ack_missing:{field}")
    blockers.extend(
        f"v5_p94_unsafe_review_approval:{field}"
        for field in UNSAFE_REVIEW_APPROVAL_FIELDS
...
def _decision(blockers: Sequence[str], review: Mapping[str, Any], review_blockers: Sequence[str]) -> str:
    if blockers or review_blockers:
        return P94_BLOCKED_DECISION
    if not review:
        return P94_HOLD_DECISION
    if review.get("approve_optimizer_parity_contract") is not True:
        return P94_REJECTED_DECISION
    return P94_READY_DECISION


def _p93_ready(summary: Mapping[str, Any]) -> bool:
    return bool(
        summary.get("present")
        and summary.get("ok")
        and summary.get("decision_record_ready")
        and summary.get("optimizer_tensor_transfer_contract_ready")
        and summary.get("optimizer_tensor_transfer_evidence_recorded")
        and summary.get("optimizer_tensor_transfer_signed")
        and summary.get("decision") == P93_READY_DECISION
...
def _parity_rows(evidence: Mapping[str, Any]) -> list[dict[str, Any]]:
    rows = evidence.get("optimizer_parity_rows") or evidence.get("optimizer_parity_check_rows") or []
    if not isinstance(rows, Sequence) or isinstance(rows, (str, bytes)):
        return []
    return [_parity_row(row) for row in rows if isinstance(row, Mapping)]


def _parity_row(row: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "optimizer_kind": str(row.get("optimizer_kind") or ""),
        "source": _source(row),
        "ready": row.get("ready") is True,
        "parity_review_ready": row.get("parity_review_ready") is True,
        ...
        "later_training_step_contract_required": row.get("later_training_step_contract_required") is True,
...
def _row_blockers(rows: Sequence[Mapping[str, Any]]) -> list[str]:
    ...
    for row in rows:
        ...
        if not row.get("source"):
            blockers.append(f"v5_p94_optimizer_parity_row_source_missing:{kind}")
        for field in (
            "ready",
            "parity_review_ready",
            ...
            if row.get(field) is not True:
                blockers.append(f"v5_p94_optimizer_parity_row_{field}_missing:{kind}")
        if row.get("later_training_step_contract_required") is not True:
            blockers.append(f"v5_p94_optimizer_parity_row_later_training_step_contract_required_missing:{kind}")
        for field in (
            ...
            if row.get(field) is True:
                blockers.append(f"v5_p94_optimizer_parity_row_unsafe_claim:{kind}:{field}")
        blockers.extend(
            f"v5_p94_optimizer_parity_row_blocker:{kind}:{item}" for item in row.get("blocked_reasons") or []
        )
    return _dedupe(blockers)


def _unsafe_claims(value: Mapping[str, Any], label: str) -> list[str]:
    return [f"v5_p94_unsafe_claim:{label}:{field}" for field in UNSAFE_TRUE_FIELDS if value.get(field) is True]


def _non_empty_claims(value: Mapping[str, Any], label: str) -> list[str]:
    return [f"v5_p94_unsafe_claim:{label}:{field}" for field in UNSAFE_NON_EMPTY_FIELDS if bool(value.get(field))]


def _review_template() -> dict[str, Any]:
    return {
        "requested_scope": P94_SCOPE,
        "approve_optimizer_parity_contract": True,
...
def _allowed_next_actions(decision: str, blockers: Sequence[str]) -> list[str]:
    if decision == P94_READY_DECISION:
        return ["prepare_optimizer_training_step_contract_default_off"]
    if decision == P94_REJECTED_DECISION:
        return ["keep_default_off", "refresh_optimizer_parity_evidence"]
    if decision == P94_HOLD_DECISION:
        return ["collect_signed_optimizer_parity_review"]
    return ["resolve_blockers", *list(blockers[:6])]


def _recommended_next_step(decision: str, blockers: Sequence[str]) -> str:
    if decision == P94_READY_DECISION:
        return "draft optimizer training-step contract"
    if decision == P94_HOLD_DECISION:
        return "collect signed owner review for optimizer parity contract"
    if decision == P94_REJECTED_DECISION:
        return "keep optimizer parity default-off and refresh evidence"
    return blockers[0] if blockers else "complete optimizer parity evidence"
...
def _main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--p93-optimizer-tensor-transfer-contract")
    parser.add_argument("--optimizer-parity-evidence")
    parser.add_argument("--optimizer-parity-signed-review")
    args = parser.parse_args(argv)
    report = build_v5_optimizer_parity_contract_p94(
        p93_optimizer_tensor_transfer_contract=_load_optional(args.p93_optimizer_tensor_transfer_contract),
        optimizer_parity_evidence=_load_optional(args.optimizer_parity_evidence),
        optimizer_parity_signed_review=_load_optional(args.optimizer_parity_signed_review),
    )
