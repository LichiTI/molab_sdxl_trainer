"""Default-off DiffGrad Triton TrainingLoop executor for selected plugin canaries."""
...
class DiffGradTrainingExecutorConfig:
    optimizer_kind: str = "diffgrad"
    lr: float = 1.0e-3
    beta1: float = 0.9
    beta2: float = 0.999
    eps: float = 1.0e-8
...
class DiffGradTrainingExecutor:
    """Launch a real DiffGrad fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: DiffGradTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("DiffGradTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("diffgrad_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("diffgrad_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("diffgrad_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"diffgrad_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("diffgrad_maximize_not_supported")
        ...
        if not cases:
            blockers.append("diffgrad_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_diffgrad_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "diffgrad_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("diffgrad_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("diffgrad_training_executor_requires_contiguous_tensors")
        state = self.optimizer.state[param]
        exp_avg = _state_tensor(state, "exp_avg", param)
        exp_avg_sq = _state_tensor(state, "exp_avg_sq", param)
        previous_grad = _state_tensor(state, "previous_grad", param)
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["diffgrad_training_executor_parameters_not_mutated"],
        }


def build_plugin_diffgrad_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: DiffGradTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> DiffGradTrainingExecutor:
    return DiffGradTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: DiffGradTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> DiffGradTrainingExecutorConfig:
    if isinstance(value, DiffGradTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return DiffGradTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "diffgrad"),
...
def _group_config(group: Mapping[str, Any], config: DiffGradTrainingExecutorConfig) -> dict[str, Any]:
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if bool(group_cfg.get("rectify", False)):
        blockers.append("diffgrad_rectify_not_supported_for_canary")
    if bool(group_cfg.get("ams_bound", False)):
        blockers.append("diffgrad_ams_bound_not_supported_for_canary")
    if bool(group_cfg.get("adam_debias", False)):
        blockers.append("diffgrad_adam_debias_not_supported_for_canary")
    if bool(group_cfg.get("adanorm", False)):
        blockers.append("diffgrad_adanorm_not_supported_for_canary")
    return blockers
...
__all__ = ["DiffGradTrainingExecutor", "DiffGradTrainingExecutorConfig", "build_plugin_diffgrad_training_executor"]
