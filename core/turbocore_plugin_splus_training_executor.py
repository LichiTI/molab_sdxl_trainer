"""Default-off SPlus Triton TrainingLoop executor for selected plugin canaries."""
...
class SPlusTrainingExecutorConfig:
    optimizer_kind: str = "splus"
    lr: float = 1.0e-1
    beta1: float = 0.9
    weight_decay: float = 0.0
    weight_decouple: bool = True
    ema_rate: float = 0.999
    nonstandard_constant: float = 1.0e-3
    block_size: int = 1024
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _splus_first_step_1d_kernel(
        param_ptr,
        grad_ptr,
        momentum_ptr,
        ema_ptr,
        ...
        ema = tl.load(ema_ptr + offsets, mask=mask, other=0.0).to(tl.float32)

        m_new = m + (g - m) * (1.0 - beta1)
        update = tl.where(m_new > 0.0, 1.0, tl.where(m_new < 0.0, -1.0, 0.0))
        p_new = p - scaled_lr * update
        ema_new = ema + (p_new - ema) * (1.0 - ema_rate)

        tl.store(momentum_ptr + offsets, m_new, mask=mask)
        tl.store(ema_ptr + offsets, ema_new, mask=mask)
        tl.store(param_ptr + offsets, p_new, mask=mask)


class SPlusTrainingExecutor:
    """Launch a real SPlus first-step 1D fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: SPlusTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("SPlusTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("splus_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("splus_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("splus_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"splus_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("splus_maximize_not_supported")
        ...
        if not cases:
            blockers.append("splus_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_splus_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "splus_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("splus_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("splus_training_executor_requires_contiguous_tensors")
        if len(param.shape) == 2:
            return _case_failed("splus_training_executor_1d_canary_only")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("splus_training_executor_only_first_step_supported_for_canary")
        state = self.optimizer.state[param]
        momentum = _state_tensor(state, "momentum", param)
        ema = _state_tensor(state, "ema", param)
        before = param.detach().clone()
        scaled_lr = float(group_cfg["lr"]) * float(group_cfg["nonstandard_constant"])
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _splus_first_step_1d_kernel[grid](
            param,
            param.grad.detach(),
            momentum,
            ema,
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["splus_training_executor_parameters_not_mutated"],
        }


def build_plugin_splus_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: SPlusTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> SPlusTrainingExecutor:
    return SPlusTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: SPlusTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> SPlusTrainingExecutorConfig:
    if isinstance(value, SPlusTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return SPlusTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "splus"),
        ...
        ema_rate=float(payload.get("ema_rate", group.get("ema_rate", 0.999)) or 0.999),
        nonstandard_constant=float(
            payload.get("nonstandard_constant", group.get("nonstandard_constant", 1.0e-3)) or 1.0e-3
        ),
...
def _group_config(group: Mapping[str, Any], config: SPlusTrainingExecutorConfig) -> dict[str, Any]:
    betas = group.get("betas", (config.beta1, 0.999))
    return {
        ...
        "ema_rate": float(group.get("ema_rate", config.ema_rate) or config.ema_rate),
        "nonstandard_constant": float(
            group.get("nonstandard_constant", config.nonstandard_constant) or config.nonstandard_constant
        ),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("splus_weight_decay_not_supported_for_canary")
    return blockers
...
__all__ = ["SPlusTrainingExecutor", "SPlusTrainingExecutorConfig", "build_plugin_splus_training_executor"]
