"""TrainingLoop canary for selected plugin AdamC native dispatch."""
...
def build_plugin_adamc_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_adamc_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = list(case.get("blocked_reasons", []) or [])
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_adamc_training_loop_canary_scorecard_v0",
        "gate": "plugin_adamc_training_loop_native_canary",
        "ok": ok,
        ...
        "selected_optimizer_name": "adamc",
        "native_route": "rust_cuda_plugin_adamc_v0",
        "case": case,
        "summary": {
        ...
        "recommended_next_step": (
            "add selected AdamC e2e shadow matrix and rollout policy"
            if ok
            else "fix selected plugin AdamC TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_adamc_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_adamc_training_loop_did_not_emit_step"],
        }
    ...
    ok = bool(
        ...
        and step_after == 2
        and type(optimizer).__name__ == "AdamC"
        and _has_adamc_state(param, optimizer)
    )
    return {
        "schema_version": 1,
        "ok": ok,
        ...
        "blocked_reasons": []
        if ok
        else _dedupe(list(runtime.get("blocked_reasons", []) or []) + ["plugin_adamc_training_loop_native_step_missing"]),
    }
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    optimizer = _create_selected_adamc_plugin_optimizer(param)
    loop = TrainingLoop(
        ...
        turbocore_native_update_quantized_optimizer_kind="adamc",
    )
    loop.total_steps = 1
    return loop, param, optimizer


def _create_selected_adamc_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
    trainer = LulynxTrainer.__new__(LulynxTrainer)
    case = plugin_resume_case("adamc")
...
def _has_adamc_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> bool:
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin AdamC TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_adamc_training_loop_canary_scorecard"]
