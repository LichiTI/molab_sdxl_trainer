"""Default-off Kate Triton TrainingLoop executor for selected plugin canaries."""
...
class KateTrainingExecutorConfig:
    optimizer_kind: str = "kate"
    lr: float = 1.0e-3
    delta: float = 0.0
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _kate_first_step_dense_kernel(
        param_ptr,
        grad_ptr,
        m_ptr,
        b_ptr,
        ...
        grad_sq = grad * grad
        b_raw = b * b + grad_sq + eps
        m_new = tl.sqrt(m * m + grad_sq * delta + grad_sq / b_raw)
        update = m_new * grad / b_raw
...
class KateTrainingExecutor:
    """Launch a real Kate first-step fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: KateTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("KateTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("kate_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("kate_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("kate_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"kate_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("kate_maximize_not_supported")
        ...
        if not cases:
            blockers.append("kate_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_kate_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "kate_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("kate_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("kate_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("kate_training_executor_only_first_step_supported_for_canary")
        state = self.optimizer.state[param]
        m = _state_tensor(state, "m", param)
        b = _state_tensor(state, "b", param)
        before = param.detach().clone()
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _kate_first_step_dense_kernel[grid](
            param,
            param.grad.detach(),
            m,
            b,
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["kate_training_executor_parameters_not_mutated"],
        }


def build_plugin_kate_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: KateTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> KateTrainingExecutor:
    return KateTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: KateTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> KateTrainingExecutorConfig:
    if isinstance(value, KateTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    return KateTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "kate"),
        lr=float(payload.get("lr", group.get("lr", 1.0e-3)) or 1.0e-3),
        delta=float(payload.get("delta", group.get("delta", 0.0)) or 0.0),
...
def _group_config(group: Mapping[str, Any], config: KateTrainingExecutorConfig) -> dict[str, Any]:
    return {
        "lr": float(group.get("lr", config.lr) or config.lr),
        "delta": float(group.get("delta", config.delta) or config.delta),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("kate_weight_decay_not_supported_for_canary")
    return blockers
...
__all__ = [
    "KateTrainingExecutor",
    "KateTrainingExecutorConfig",
    "build_plugin_kate_training_executor",
]
