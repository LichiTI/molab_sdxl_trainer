"""Default-off training tensor binding canary for selected plugin Adamax."""
...
def build_plugin_adamax_training_tensor_binding_canary_scorecard(
    *,
    ...
    scratch = dict(
        scratch_report
        or build_plugin_adamax_native_scratch_kernel_scorecard(workspace_root=workspace_root, arch=arch)
    )
    ...
    return {
        ...
        "promotion_blockers": _dedupe(
            blockers
            + [
                "adamax_runtime_dispatch_shadow_missing",
                "adamax_training_loop_canary_missing",
                "adamax_rollback_review_missing",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "add selected Adamax runtime dispatch shadow before TrainingLoop canary"
            if ready
            else "fix selected Adamax training tensor binding canary blockers"
        ),
    }
...
def _live_probe(*, workspace_root: str | Path | None, arch: str | None) -> dict[str, Any]:
    ...
    if native is None:
        return _failed("lulynx_native_entrypoint_missing", ["adamax_training_tensor_binding_entrypoint_missing"])
    ...
    reference = _reference_adamax(role_tensors, config)
    ...
    if not bool(session.get("ok", False)):
        return _failed("tensor_binding_session_create_failed", ["adamax_tensor_binding_session_create_failed"], session=session)
    session_id = int(session["session_id"])
    try:
        launch = dict(native.tensor_binding_session_cuda_adamax_tensor_probe(session_id, json.dumps(config)))
        if not bool(launch.get("ok", False)):
            return _failed(
                f"adamax_tensor_probe_launch_failed:{launch.get('reason', 'unknown')}",
                ["adamax_tensor_probe_launch_failed"],
                session=session,
                launch=launch,
            )
        ...
        diffs = {
            ...
            "exp_inf": _max_abs_diff(role_tensors["exp_avg_sq"], reference["exp_inf"]),
        }
        ...
        return {
            ...
            "probe_kind": "plugin_adamax_training_tensor_binding_canary_v0",
            "ok": ok,
            ...
            "session": session,
            "launch": launch,
            "blocked_reasons": [] if ok else ["adamax_training_tensor_binding_parity_failed"],
        }
    finally:
...
def _make_cuda_request() -> tuple[dict[str, Any], dict[str, torch.Tensor], dict[str, torch.Tensor]]:
    ...
    registry = TurboCoreTensorHandleRegistry(namespace="plugin_adamax_tensor_binding_canary")
    param = (torch.arange(24, dtype=torch.float32, device=device) * 0.115 + 0.35).contiguous()
    grad = (torch.arange(24, dtype=torch.float32, device=device) * 0.0008 + 0.014).contiguous()
    exp_avg = (torch.arange(24, dtype=torch.float32, device=device) * 0.00025 - 0.001).contiguous()
    exp_inf = (torch.arange(24, dtype=torch.float32, device=device) * 0.00015 + 0.008).contiguous()
...
def _reference_adamax(role_tensors: dict[str, torch.Tensor], config: Mapping[str, Any]) -> dict[str, torch.Tensor]:
    ...
    exp_inf = role_tensors["exp_avg_sq"].detach().clone()
    ...
    exp_inf.mul_(beta2)
    torch.maximum(exp_inf, grad.abs().add(eps), out=exp_inf)
    step_size = lr / (1.0 - beta1**step_number)
    param.addcdiv_(exp_avg, exp_inf, value=-step_size)
    return {"param_flat": param, "exp_avg": exp_avg, "exp_inf": exp_inf}


def _validations(scratch: Mapping[str, Any], live_probe: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        _validation(
            "p47_adamax_scratch_kernel_ready",
            bool(scratch.get("adamax_native_kernel_parity", False)),
            "adamax_native_scratch_kernel_missing",
        ),
        _validation(
            "training_tensor_binding_parity",
            bool(live_probe.get("training_tensor_binding_parity_passed", False)),
            "adamax_training_tensor_binding_canary_failed",
        ),
        _validation(
            ...
            "adamax_training_tensor_binding_enabled_dispatch",
        ),
    ]
...
__all__ = ["build_plugin_adamax_training_tensor_binding_canary_scorecard"]
