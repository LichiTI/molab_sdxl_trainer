"""TrainingLoop canary for selected plugin ScheduleFreeSGD native dispatch."""
...
SELECTED_OPTIMIZER = "schedulefreesgd"
NATIVE_ROUTE = "rust_cuda_schedulefree_sgd_v0"
...
def build_plugin_schedulefree_sgd_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_schedulefreesgd_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = _strings(case.get("blocked_reasons"))
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_schedulefree_sgd_training_loop_canary_scorecard_v0",
        "gate": "plugin_schedulefreesgd_training_loop_native_canary",
        "ok": ok,
        ...
        + [
            "schedulefreesgd_e2e_shadow_matrix_refresh_required",
            "schedulefreesgd_canary_rollout_policy_refresh_required",
            "product_rollout_review_missing",
        ],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "refresh selected schedule-free e2e shadow matrix and rollout policy for schedulefreesgd"
            if ok
            else "fix selected plugin ScheduleFreeSGD TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_schedulefreesgd_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_schedulefreesgd_training_loop_did_not_emit_step"],
        }
    ...
    ok = bool(
        ...
        and step_after == 2
        and type(optimizer).__name__ == "ScheduleFreeSGD"
        and str(executor_result.get("optimizer_kind") or "") == "schedulefree_sgd"
    )
    return {
        "schema_version": 1,
        "ok": ok,
        ...
        "blocked_reasons": [] if ok else _dedupe(_strings(runtime.get("blocked_reasons")) + [
            "plugin_schedulefreesgd_training_loop_native_step_missing"
        ]),
    }
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    optimizer = _create_selected_schedulefreesgd_plugin_optimizer(param)
    loop = TrainingLoop(
...
def _create_selected_schedulefreesgd_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin ScheduleFreeSGD TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_schedulefree_sgd_training_loop_canary_scorecard"]
