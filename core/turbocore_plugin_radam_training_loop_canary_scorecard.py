"""TrainingLoop canary for selected plugin RAdam native dispatch."""
...
def build_plugin_radam_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_radam_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = _strings(case.get("blocked_reasons"))
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_radam_training_loop_canary_scorecard_v0",
        "gate": "plugin_radam_training_loop_native_canary",
        "ok": ok,
        ...
        + [
            "radam_e2e_shadow_matrix_refresh_required",
            "radam_canary_rollout_policy_refresh_required",
            "product_rollout_review_missing",
        ],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "refresh selected Adam-like e2e shadow matrix and rollout policy for radam"
            if ok
            else "fix selected plugin RAdam TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_radam_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_radam_training_loop_did_not_emit_step"],
        }
    ...
    ok = bool(
        ...
        and step_after == 2
        and type(optimizer).__name__ == "RAdam"
        and str(executor_result.get("optimizer_kind") or "") == "radam"
    )
    return {
        "schema_version": 1,
        "ok": ok,
        ...
        "blocked_reasons": [] if ok else _dedupe(_strings(runtime.get("blocked_reasons")) + [
            "plugin_radam_training_loop_native_step_missing"
        ]),
    }
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    param = torch.nn.Parameter(torch.linspace(-0.18, 0.28, steps=4096, device=device, dtype=torch.float32))
    optimizer = _create_selected_radam_plugin_optimizer(param)
    loop = TrainingLoop(
...
def _create_selected_radam_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
...
def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> None:
    param.grad = None
    loss = ((param.float() * 0.23) ** 2).mean() + param.float().mean() * 0.005
    loss.backward()
    optimizer.step()
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin RAdam TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_radam_training_loop_canary_scorecard"]
