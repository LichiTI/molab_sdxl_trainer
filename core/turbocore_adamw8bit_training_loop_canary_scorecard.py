"""TrainingLoop canary for AdamW8bit native dispatch."""
...
from core.turbocore_paged_adamw8bit_residency_scorecard import _first_live_state
...
def build_adamw8bit_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_adamw8bit_training_loop_canary")
    if importlib.util.find_spec("bitsandbytes") is None:
        return _blocked("bitsandbytes_required_for_adamw8bit_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = list(case.get("blocked_reasons", []) or [])
    return {
        "schema_version": 1,
        "scorecard": "turbocore_adamw8bit_training_loop_canary_scorecard_v0",
        "gate": "adamw8bit_training_loop_native_canary",
        "ok": ok,
        ...
        "recommended_next_step": (
            "measure AdamW8bit product canary overhead and decide rollout scope"
            if ok
            else "fix AdamW8bit TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    if optimizer_name == "AdamW":
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "adamw8bit_training_loop_native_canary_v0",
            "optimizer_class": optimizer_name,
            "blocked_reasons": ["adamw8bit_resolved_to_fallback_adamw"],
        }
    ...
    def _fake_train_step(
        ...
        for item in params:
            item.grad = None
        loss = sum(((item.float() * 0.61) ** 2).mean() + item.float().mean() * 0.011 for item in params)
    ...
    if not captured:
        return {
            ...
            "blocked_reasons": ["adamw8bit_training_loop_did_not_emit_step"],
        }
    ...
    return {
        ...
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "blocked_reasons": [] if ok else _dedupe(list(runtime.get("blocked_reasons", []) or []) + ["adamw8bit_training_loop_native_step_missing"]),
    }
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    loop = TrainingLoop(
        ...
        turbocore_native_update_quantized_optimizer_kind="adamw8bit",
    )
    loop.total_steps = 1
...
def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> None:
    param.grad = None
    loss = ((param.float() * 0.31) ** 2).mean() + param.float().mean() * 0.011
    loss.backward()
    optimizer.step()
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run AdamW8bit TrainingLoop canary on CUDA with bitsandbytes",
    }
...
__all__ = ["build_adamw8bit_training_loop_canary_scorecard"]
