"""Batch runtime precondition rehearsal for selected plugin factored-memory optimizers."""
...
from core.turbocore_adafactor_training_loop_canary_scorecard import build_adafactor_training_loop_canary_scorecard
...
ARTIFACT_NAME = "turbocore_plugin_factored_memory_runtime_precondition_rehearsal_scorecard.json"


def build_plugin_factored_memory_runtime_precondition_rehearsal_scorecard(
    *,
    family_batch_report: Mapping[str, Any] | None = None,
    include_representative_runtime_canary: bool = False,
    write_artifact: bool = False,
) -> dict[str, Any]:
    """Normalize factored-memory layout/matrix readiness as a runtime precondition gate."""

    family = _as_dict(
        family_batch_report
        if family_batch_report is not None
        else build_plugin_factored_memory_family_batch_scorecard(write_artifact=True)
    )
    representative_runtime = (
        _compact_adafactor_runtime_canary(build_adafactor_training_loop_canary_scorecard())
    ...
    adapter_coverage = build_family_runtime_launch_adapter_coverage(
        ...
        adapter_kind="factored_memory_layout_runtime_adapter",
        representative_optimizer_name="adafactor",
    )
    ready = (
        family.get("selected_factored_memory_family_batch_ready") is True
        and bool(rows)
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_factored_memory_runtime_precondition_rehearsal_scorecard_v0",
        "gate": "plugin_factored_memory_runtime_precondition_rehearsal",
        "roadmap": ROADMAP,
        "ok": ready,
        ...
        "summary": {
            ...
            "native_layout_abi_ready_count": sum(1 for row in rows if row.get("native_layout_abi_ready") is True),
            "quality_matrix_ready_count": sum(1 for row in rows if row.get("quality_matrix_ready") is True),
            "formula_tensor_binding_matrix_ready_count": sum(
                1 for row in rows if row.get("formula_tensor_binding_matrix_ready") is True
            ),
            "dispatch_review_ready_count": sum(
                1 for row in rows if row.get("dispatch_integration_review_ready") is True
            ),
        ...
        "promotion_blockers": _dedupe(
            blockers
            + [
                "plugin_factored_memory_runtime_dispatch_rehearsal_missing",
                "plugin_factored_memory_owner_release_review_missing",
                "plugin_factored_memory_product_training_route_not_bound",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "add selected plugin factored-memory native executor representatives"
            if ready
            else "fix selected plugin factored-memory runtime precondition blockers"
        ),
        "notes": [
            "This is a precondition rehearsal, not a CUDA runtime dispatch rehearsal.",
            "It proves selected plugin layout, formula, tensor-binding, and dispatch-review gates in one batch.",
            "When requested, it also records an Adafactor representative runtime canary without promoting the whole family.",
...
def _case_row(row: Mapping[str, Any]) -> dict[str, Any]:
    matrix = _as_dict(row.get("formula_tensor_binding_matrix"))
    ready = (
        row.get("native_layout_abi_ready") is True
        and row.get("quality_matrix_ready") is True
        and row.get("native_kernel_entry_condition_ready") is True
        and matrix.get("matrix_implementation_ready") is True
        and matrix.get("formula_step_execution_ready") is True
        and matrix.get("resume_next_step_replay_ready") is True
        and matrix.get("tensor_binding_ready") is True
        and row.get("dispatch_integration_review_ready") is True
    )
    ...
    return {
        ...
        "quality_matrix_ready": row.get("quality_matrix_ready") is True,
        "native_kernel_entry_condition_ready": row.get("native_kernel_entry_condition_ready") is True,
        "formula_tensor_binding_matrix_ready": matrix.get("matrix_implementation_ready") is True,
        "formula_step_execution_ready": matrix.get("formula_step_execution_ready") is True,
        "resume_next_step_replay_ready": matrix.get("resume_next_step_replay_ready") is True,
        "tensor_binding_ready": matrix.get("tensor_binding_ready") is True,
        "dispatch_integration_review_ready": row.get("dispatch_integration_review_ready") is True,
        ...
        "runtime_dispatch_ready": False,
        "native_dispatch_allowed": False,
        "blocked_reasons": [] if ready else [f"plugin_{name}_factored_memory_runtime_precondition_missing"],
    }
...
def _compact_adafactor_runtime_canary(report: Mapping[str, Any]) -> dict[str, Any]:
    summary = _as_dict(report.get("summary"))
    ready = (
    ...
    return {
        "schema_version": 1,
        "selected_optimizer_name": "adafactor",
...
__all__ = ["ARTIFACT_NAME", "ROADMAP", "build_plugin_factored_memory_runtime_precondition_rehearsal_scorecard"]
