"""Default-off training executor for Automagic++ native dispatch."""

from __future__ import annotations

import json
import time
from dataclasses import dataclass
from pathlib import Path
from typing import Any, Iterable, Mapping

import torch

from core.services.native_module_loader import native_with_entrypoints


ENTRYPOINT = "probe_automagicpp_training_tensor_binding_canary_py"
REPO_ROOT = Path(__file__).resolve().parents[2]


@dataclass(frozen=True)
class AutomagicPPTrainingExecutorConfig:
    beta2: float = 0.999
    eps1: float = 1e-30
    clip_threshold: float = 1.0
    min_lr: float = 1e-7
    max_lr: float = 1e-3
    lr_up: float = 1.01
    lr_down: float = 0.95
    weight_decay: float = 0.0
    max_numel: int = 1_048_576
    require_native_cuda: bool = True


class AutomagicPPTrainingExecutor:
    """Launch native Automagic++ steps against live optimizer state."""

    def __init__(
        self,
        *,
        optimizer: torch.optim.Optimizer,
        params: Iterable[torch.nn.Parameter],
        config: AutomagicPPTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AutomagicPPTrainingExecutor requires trainable parameters")
        self._param_ids = {id(param) for param in self.params}
        self.config = _normalize_config(config, optimizer)
        self.workspace_root = Path(workspace_root or REPO_ROOT)
        self._native: Any | None = None

    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        payload = dict(request or {})
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("automagicpp_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("automagicpp_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("automagicpp_training_dispatch_entrypoint_missing")
        started = time.perf_counter()
        cases: list[dict[str, Any]] = []
        for group in self.optimizer.param_groups:
            group_cfg = _group_config(group, self.config, self.optimizer)
            for param in group["params"]:
                if id(param) not in self._param_ids or param.grad is None:
                    continue
                cases.append(self._step_param(native, param, group_cfg))
        ok = bool(cases) and all(bool(case.get("ok", False)) for case in cases)
        blockers = _dedupe([reason for case in cases for reason in case.get("blocked_reasons", [])])
        if not cases:
            blockers.append("automagicpp_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_automagicpp_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "automagicpp_native_step_failed"),
            "optimizer_kind": "automagicpp",
            "training_dispatch": True,
            "training_path_enabled": True,
            "native_step_executed": ok,
            "native_kernel_launched": any(bool(case.get("kernel_executed", False)) for case in cases),
            "training_parameters_mutated": ok,
            "should_call_pytorch_optimizer_step": not ok,
            "pytorch_optimizer_state_synced": True,
            "parameter_step_count": len(cases),
            "cases": cases,
            "timing": {"elapsed_ms": _elapsed_ms(started)},
            "blocked_reasons": blockers,
        }

    def close(self) -> None:
        return None

    def _load_native(self) -> Any | None:
        if self._native is None:
            self._native = native_with_entrypoints(ENTRYPOINT)
        return self._native

    def _step_param(self, native: Any, param: torch.nn.Parameter, group_cfg: Mapping[str, Any]) -> dict[str, Any]:
        state = self.optimizer.state[param]
        missing = [key for key in ("local_lr", "prev_sign", "full_var", "avg_lr") if key not in state]
        if missing:
            return _case_failed("automagicpp_live_state_missing", f"automagicpp_live_state_missing:{','.join(missing)}")
        if torch.is_tensor(state.get("prev_sign")) and state["prev_sign"].dtype != torch.float32:
            state["prev_sign"] = state["prev_sign"].to(device=param.device, dtype=torch.float32)
        has_prev = torch.tensor([1 if bool(state.get("has_prev_sign", False)) else 0], device=param.device, dtype=torch.int32)
        avg_lr = state["avg_lr"].detach().reshape(1).contiguous().to(device=param.device, dtype=torch.float32)
        launch_config = {
            **dict(group_cfg),
            "max_numel": max(int(self.config.max_numel), int(param.numel())),
            "canary_probe_only": True,
            "training_tensor_binding": True,
            "training_dispatch": False,
            "training_path_enabled": False,
        }
        try:
            launch = dict(
                getattr(native, ENTRYPOINT)(
                    param,
                    param.grad.detach().contiguous(),
                    state["local_lr"],
                    state["prev_sign"],
                    state["full_var"],
                    has_prev,
                    avg_lr,
                    json.dumps(launch_config),
                    str(self.workspace_root.resolve()),
                    _cuda_arch(param.device),
                )
            )
        except Exception as exc:  # pragma: no cover - native/CUDA dependent
            return _case_failed(f"{type(exc).__name__}: {exc}", "automagicpp_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(str(launch.get("reason") or "native_step_failed"), "automagicpp_native_step_failed", launch)
        state["step"] = int(state.get("step", 0) or 0) + 1
        state["has_prev_sign"] = bool(int(has_prev.detach().cpu().item()))
        state["avg_lr"] = avg_lr.detach().mean()
        return {
            "schema_version": 1,
            "ok": True,
            "param_numel": int(param.numel()),
            "param_dtype": str(param.dtype).replace("torch.", ""),
            "step_after": int(state.get("step", 0) or 0),
            "kernel_executed": bool(launch.get("kernel_executed", False)),
            "training_parameters_mutated": bool(launch.get("parameters_mutated", False)),
            "launch": launch,
            "blocked_reasons": [],
        }


def build_automagicpp_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AutomagicPPTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> AutomagicPPTrainingExecutor:
    return AutomagicPPTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AutomagicPPTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AutomagicPPTrainingExecutorConfig:
    if isinstance(value, AutomagicPPTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    eps = _eps_values(payload.get("eps", group.get("eps", (1e-30, 1e-3))))
    return AutomagicPPTrainingExecutorConfig(
        beta2=float(payload.get("beta2", group.get("beta2", 0.999))),
        eps1=float(payload.get("eps1", eps[0])),
        clip_threshold=float(payload.get("clip_threshold", group.get("clip_threshold", 1.0))),
        min_lr=float(payload.get("min_lr", getattr(optimizer, "min_lr", 1e-7))),
        max_lr=float(payload.get("max_lr", getattr(optimizer, "max_lr", 1e-3))),
        lr_up=float(payload.get("lr_up", getattr(optimizer, "lr_up", 1.01))),
        lr_down=float(payload.get("lr_down", getattr(optimizer, "lr_down", 0.95))),
        weight_decay=float(payload.get("weight_decay", group.get("weight_decay", 0.0))),
        max_numel=int(payload.get("max_numel", 1_048_576)),
        require_native_cuda=bool(payload.get("require_native_cuda", True)),
    )


def _group_config(group: Mapping[str, Any], config: AutomagicPPTrainingExecutorConfig, optimizer: torch.optim.Optimizer) -> dict[str, Any]:
    eps = _eps_values(group.get("eps", (config.eps1, 1e-3)))
    return {
        "beta2": float(group.get("beta2", config.beta2)),
        "eps1": float(eps[0]),
        "clip_threshold": float(group.get("clip_threshold", config.clip_threshold)),
        "min_lr": float(getattr(optimizer, "min_lr", config.min_lr)),
        "max_lr": float(getattr(optimizer, "max_lr", config.max_lr)),
        "lr_up": float(getattr(optimizer, "lr_up", config.lr_up)),
        "lr_down": float(getattr(optimizer, "lr_down", config.lr_down)),
        "weight_decay": float(group.get("weight_decay", config.weight_decay)),
    }


def _eps_values(value: Any) -> tuple[float, float]:
    if isinstance(value, (tuple, list)):
        if len(value) >= 2:
            return float(value[0]), float(value[1])
        if len(value) == 1:
            return float(value[0]), 1e-3
    return float(value), 1e-3


def _cuda_arch(device: torch.device) -> str:
    try:
        index = device.index if device.index is not None else torch.cuda.current_device()
        major, minor = torch.cuda.get_device_capability(index)
        return f"compute_{int(major)}{int(minor)}"
    except Exception:
        return "compute_89"


def _elapsed_ms(started: float) -> float:
    return round((time.perf_counter() - started) * 1000.0, 4)


def _case_failed(reason: str, blocker: str, launch: Mapping[str, Any] | None = None) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "ok": False,
        "reason": reason,
        "kernel_executed": False,
        "training_parameters_mutated": False,
        "launch": dict(launch or {}),
        "blocked_reasons": [blocker],
    }


def _blocked(reason: str) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "executor": "turbocore_automagicpp_training_executor_v0",
        "ok": False,
        "reason": reason,
        "training_dispatch": True,
        "training_path_enabled": True,
        "native_step_executed": False,
        "native_kernel_launched": False,
        "training_parameters_mutated": False,
        "should_call_pytorch_optimizer_step": True,
        "blocked_reasons": [reason],
    }


def _dedupe(values: list[str]) -> list[str]:
    out: list[str] = []
    for value in values:
        text = str(value or "")
        if text and text not in out:
            out.append(text)
    return out


__all__ = ["AutomagicPPTrainingExecutor", "AutomagicPPTrainingExecutorConfig", "build_automagicpp_training_executor"]
