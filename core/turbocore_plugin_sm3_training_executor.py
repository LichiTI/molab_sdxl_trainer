"""Default-off SM3 Triton TrainingLoop executor for selected plugin canaries."""
...
class SM3TrainingExecutorConfig:
    optimizer_kind: str = "sm3"
    lr: float = 1e-1
    momentum: float = 0.0
    beta: float = 0.0
    eps: float = 1e-30
...
if triton is not None and tl is not None:
    ...
    def _sm3_1d_dense_kernel(
        ...
        acc = tl.load(accumulator_ptr + offsets, mask=mask, other=0.0).to(tl.float32)
        mom = tl.load(momentum_ptr + offsets, mask=mask, other=0.0).to(tl.float32)

        candidate = acc * beta + g * g * (1.0 - beta)
        acc_new = tl.where(beta > 0.0, tl.maximum(acc, candidate), candidate)
        update = g * tl.rsqrt(candidate + eps)
        mom_new = mom * momentum + update * (1.0 - momentum)
        step_update = tl.where(momentum > 0.0, mom_new, update)
        p_new = p - lr * step_update

        tl.store(accumulator_ptr + offsets, acc_new, mask=mask)
        tl.store(momentum_ptr + offsets, mom_new, mask=mask)
        tl.store(param_ptr + offsets, p_new, mask=mask)


class SM3TrainingExecutor:
    """Launch a real SM3 1D dense Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: SM3TrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("SM3TrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("sm3_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("sm3_training_executor_requires_cuda_params")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"sm3_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        started = time.perf_counter()
        cases: list[dict[str, Any]] = []
        for group in self.optimizer.param_groups:
            group_cfg = _group_config(group, self.config)
            if bool(getattr(self.optimizer, "maximize", False)):
                cases.append(_case_failed("sm3_maximize_not_supported"))
                continue
        ...
        if not cases:
            blockers.append("sm3_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_sm3_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "sm3_native_step_failed"),
    ...
    def _step_param(
        ...
        state["accumulator_0"] = accumulator
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["sm3_training_executor_parameters_not_mutated"],
        }


def build_plugin_sm3_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: SM3TrainingExecutorConfig | Mapping[str, Any] | None = None,
    ...
    return SM3TrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: SM3TrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> SM3TrainingExecutorConfig:
    if isinstance(value, SM3TrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    return SM3TrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "sm3"),
        lr=float(payload.get("lr", group.get("lr", 1e-1)) or 1e-1),
        momentum=float(payload.get("momentum", group.get("momentum", 0.0)) or 0.0),
        beta=float(payload.get("beta", group.get("beta", 0.0)) or 0.0),
        eps=float(payload.get("eps", group.get("eps", 1e-30)) or 1e-30),
        block_size=int(payload.get("block_size", 1024) or 1024),
        require_native_cuda=bool(payload.get("require_native_cuda", True)),
    )


def _group_config(group: Mapping[str, Any], config: SM3TrainingExecutorConfig) -> dict[str, Any]:
    return {
...
__all__ = ["SM3TrainingExecutor", "SM3TrainingExecutorConfig", "build_plugin_sm3_training_executor"]
