"""Request/schema/UI non-exposure audit for built-in Muon routing."""
...
from core.turbocore_muon_owner_release_hold_scorecard import build_muon_owner_release_hold_scorecard


AUDIT_KIND = "muon_model_shape_aware_request_schema_ui_non_exposure_v0"
...
def build_muon_request_schema_ui_non_exposure_scorecard(
    *,
    owner_release_hold_report: Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
    write_artifact: bool = False,
) -> dict[str, Any]:
    """Audit product request/schema/UI boundaries while keeping Muon exposure disabled."""

    root = Path(workspace_root).resolve() if workspace_root is not None else Path(__file__).resolve().parents[2]
    hold_report = _as_dict(owner_release_hold_report or build_muon_owner_release_hold_scorecard())
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_muon_request_schema_ui_non_exposure_scorecard_v0",
        "gate": "muon_model_shape_aware_request_schema_ui_non_exposure",
        "ok": ready,
        ...
        "promotion_blockers": _dedupe(
            ...
            + [
                ...
                "muon_request_schema_ui_exposure_not_approved",
                "muon_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "record explicit owner and release approval before Muon request/schema/UI or dispatch wiring"
            if ready
            else "fix Muon request/schema/UI non-exposure blockers"
        ),
        "notes": [
            "This audit is report-only and does not register request fields, schema fields, UI controls, or routes.",
            "Boundary scanning checks product-facing paths for Muon native exposure tokens.",
...
def _validations(
    ...
    return [
        _validation(
            ...
            "muon_owner_release_hold_missing",
        ),
        _validation(
            ...
            "muon_request_schema_ui_unexpected_approval",
        ),
        _validation(
            ...
            and hold_report.get("native_kernel_ready") is False
            and hold_report.get("training_path_enabled") is False,
            "muon_request_schema_ui_hold_enabled_boundary",
        ),
        _validation(
            "boundary_inventory_present",
            len(boundary.get("present_paths", [])) > 0,
            "muon_request_schema_ui_boundary_inventory_missing",
        ),
        _validation(
            "forbidden_tokens_absent",
            not findings.get("forbidden_token_hits"),
            "muon_request_schema_ui_forbidden_boundary_token_found",
        ),
    ]
...
def _write_artifact(root: Path, report: Mapping[str, Any]) -> None:
    target = root / "temp" / "turbocore_optimizer" / "turbocore_muon_request_schema_ui_non_exposure_scorecard.json"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")


__all__ = ["AUDIT_KIND", "build_muon_request_schema_ui_non_exposure_scorecard"]
