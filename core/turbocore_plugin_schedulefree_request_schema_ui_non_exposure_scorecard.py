"""Request/schema/UI non-exposure audit for schedule-free plugin native work."""
...
AUDIT_KIND = "plugin_schedulefree_request_schema_ui_non_exposure_v0"
FORBIDDEN_BOUNDARY_TOKENS = (
    "plugin_schedulefree_native",
    "plugin_schedulefree_dispatch",
    "plugin_schedulefree_canary",
    "enable_plugin_schedulefree_native",
    "enable_schedulefree_native",
    "schedulefree_native",
    "schedulefree_turbocore",
)
BOUNDARY_PATHS = (
...
def build_plugin_schedulefree_request_schema_ui_non_exposure_scorecard(
    *,
    owner_release_hold_report: Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
    write_artifact: bool = False,
) -> dict[str, Any]:
    """Audit product boundaries while schedule-free native dispatch stays disabled."""

    root = Path(workspace_root).resolve() if workspace_root is not None else Path(__file__).resolve().parents[2]
    hold = _as_dict(owner_release_hold_report or build_plugin_schedulefree_owner_release_hold_scorecard())
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_schedulefree_request_schema_ui_non_exposure_scorecard_v0",
        "gate": "plugin_schedulefree_request_schema_ui_non_exposure",
        "ok": ready,
        ...
        "promotion_blockers": _dedupe(
            ...
            + [
                ...
                "plugin_schedulefree_request_schema_ui_exposure_not_approved",
                "plugin_schedulefree_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "await explicit owner/release approval before schedule-free request/schema/UI or dispatch wiring"
            if ready
            else "fix schedule-free request/schema/UI non-exposure blockers"
        ),
        "notes": [
            "This audit is report-only and does not register request fields, schema fields, UI controls, or routes.",
            "Boundary scanning checks product-facing paths for schedule-free native exposure tokens.",
...
def _validations(
    ...
    return [
        _validation(
            ...
            "plugin_schedulefree_owner_release_hold_missing",
        ),
        _validation(
            ...
            "plugin_schedulefree_request_schema_ui_unexpected_approval",
        ),
        _validation(
            ...
            "plugin_schedulefree_request_schema_ui_hold_enabled_boundary",
        ),
        _validation(
            "boundary_inventory_present",
            len(boundary.get("present_paths", [])) > 0,
            "plugin_schedulefree_request_schema_ui_boundary_inventory_missing",
        ),
        _validation(
            "forbidden_tokens_absent",
            not findings.get("forbidden_token_hits"),
            "plugin_schedulefree_request_schema_ui_forbidden_boundary_token_found",
        ),
    ]
...
def _write_artifact(root: Path, report: Mapping[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_plugin_schedulefree_request_schema_ui_non_exposure_scorecard.json"
...
__all__ = ["build_plugin_schedulefree_request_schema_ui_non_exposure_scorecard"]
