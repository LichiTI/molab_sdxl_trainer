"""Default-off AvaGrad Triton TrainingLoop executor for selected plugin canaries."""
...
class AvaGradTrainingExecutorConfig:
    optimizer_kind: str = "avagrad"
    lr: float = 1.0e-1
    beta1: float = 0.9
    beta2: float = 0.999
    eps: float = 1.0e-1
...
if triton is not None and tl is not None:
    ...
    def _avagrad_dense_kernel(
        ...
        denom = tl.sqrt(v) / prev_bias_correction2_sq + eps
...
class AvaGradTrainingExecutor:
    """Launch a real AvaGrad fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: AvaGradTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AvaGradTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("avagrad_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("avagrad_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("avagrad_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"avagrad_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("avagrad_maximize_not_supported")
        ...
        if not cases:
            blockers.append("avagrad_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_avagrad_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "avagrad_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("avagrad_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("avagrad_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step <= 1:
            return _case_failed("avagrad_training_executor_requires_warmed_step")
        state = self.optimizer.state[param]
        exp_avg = _state_tensor(state, "exp_avg", param)
        exp_avg_sq = _state_tensor(state, "exp_avg_sq", param)
        gamma = group.get("gamma", None)
        if gamma is None:
            return _case_failed("avagrad_training_executor_requires_gamma_warmup")
        ...
        prev_bias_correction2_sq = math.sqrt(1.0 - math.pow(beta2, step - 1))
        step_size = float(gamma) * float(group_cfg["lr"])
        if not bool(group_cfg["adam_debias"]):
            step_size /= bias_correction1
        gamma_next = _next_gamma(exp_avg_sq, bias_correction2_sq, float(group_cfg["eps"]))
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _avagrad_dense_kernel[grid](
            param,
            param.grad.detach(),
            exp_avg,
            exp_avg_sq,
            ...
            beta1,
            beta2,
            float(prev_bias_correction2_sq),
            float(group_cfg["eps"]),
        ...
        return {
            ...
            "group_gamma": float(gamma_next),
            "blocked_reasons": [] if mutated else ["avagrad_training_executor_parameters_not_mutated"],
        }


def build_plugin_avagrad_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AvaGradTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> AvaGradTrainingExecutor:
    return AvaGradTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AvaGradTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AvaGradTrainingExecutorConfig:
    if isinstance(value, AvaGradTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return AvaGradTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "avagrad"),
        lr=float(payload.get("lr", group.get("lr", 1.0e-1)) or 1.0e-1),
        beta1=float(betas[0]),
        beta2=float(betas[1]),
        eps=float(payload.get("eps", group.get("eps", 1.0e-1)) or 1.0e-1),
...
def _group_config(group: Mapping[str, Any], config: AvaGradTrainingExecutorConfig) -> dict[str, Any]:
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("avagrad_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("fixed_decay", False)):
        blockers.append("avagrad_fixed_decay_not_supported_for_canary")
    return blockers
...
def _next_gamma(exp_avg_sq: torch.Tensor, bias_correction2_sq: float, eps: float) -> float:
    param_wise_lr = exp_avg_sq.detach().sqrt().div(float(bias_correction2_sq)).add(float(eps))
    squared_norm = float(param_wise_lr.pow(-2).sum().detach().cpu().item())
    if squared_norm <= 0.0 or not math.isfinite(squared_norm):
        return 0.0
    return 1.0 / math.sqrt(squared_norm / float(param_wise_lr.numel()))
...
__all__ = ["AvaGradTrainingExecutor", "AvaGradTrainingExecutorConfig", "build_plugin_avagrad_training_executor"]
