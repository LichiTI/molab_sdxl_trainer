"""TrainingLoop canary for KahanAdamW8bit native dispatch."""
...
def build_kahan_adamw8bit_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_kahan_adamw8bit_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = list(case.get("blocked_reasons", []) or [])
    return {
        "schema_version": 1,
        "scorecard": "turbocore_kahan_adamw8bit_training_loop_canary_scorecard_v0",
        "gate": "kahan_adamw8bit_training_loop_native_canary",
        "ok": ok,
        ...
        "recommended_next_step": (
            "measure KahanAdamW8bit product canary overhead and decide rollout scope"
            if ok
            else "fix KahanAdamW8bit TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "kahan_adamw8bit_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": len(captured),
            "blocked_reasons": ["kahan_training_loop_did_not_emit_step"],
        }
    ...
    step_after = int(optimizer_state.get("step") or 0)
    ok = bool(
        result.get("steps") == 1
        and native_step
        and native_kernel
    ...
    return {
        ...
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "blocked_reasons": [] if ok else _dedupe(list(runtime.get("blocked_reasons", []) or []) + ["kahan_training_loop_native_step_missing"]),
    }


def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: KahanAdamW8bit) -> None:
    param.grad = None
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, KahanAdamW8bit]:
    values = torch.linspace(-0.5, 0.5, steps=4096, device="cuda", dtype=torch.float32)
    param = torch.nn.Parameter(values)
    optimizer = KahanAdamW8bit([param], lr=1e-3, weight_decay=0.01)
    loop = TrainingLoop(
        ...
        turbocore_native_update_quantized_optimizer_kind="kahan_adamw8bit",
    )
    loop.total_steps = 1
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run KahanAdamW8bit TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_kahan_adamw8bit_training_loop_canary_scorecard"]
