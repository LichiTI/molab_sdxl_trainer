"""Default-off Ano Triton TrainingLoop executor for selected plugin canaries."""
...
class AnoTrainingExecutorConfig:
    optimizer_kind: str = "ano"
    lr: float = 1.0e-4
    beta1: float = 0.92
    beta2: float = 0.99
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _ano_first_step_dense_kernel(
        param_ptr,
        grad_ptr,
        exp_avg_ptr,
        exp_avg_sq_ptr,
        ...
        signed_square = tl.where(square_grad - v > 0.0, square_grad, -square_grad)
        v_new = v * beta2 + signed_square * (1.0 - beta2)
        denom = tl.sqrt(v_new / bias_correction2) + eps
        update = tl.abs(g) * tl.where(m_new > 0.0, 1.0, tl.where(m_new < 0.0, -1.0, 0.0))
        p_new = p - lr * update / denom
...
class AnoTrainingExecutor:
    """Launch a real Ano first-step fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: AnoTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AnoTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("ano_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("ano_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("ano_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"ano_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("ano_maximize_not_supported")
        ...
        if not cases:
            blockers.append("ano_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_ano_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "ano_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("ano_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("ano_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("ano_training_executor_only_first_step_supported_for_canary")
        ...
        _ano_first_step_dense_kernel[grid](
            param,
            param.grad.detach(),
            exp_avg,
            exp_avg_sq,
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["ano_training_executor_parameters_not_mutated"],
        }


def build_plugin_ano_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AnoTrainingExecutorConfig | Mapping[str, Any] | None = None,
    ...
    return AnoTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AnoTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AnoTrainingExecutorConfig:
    if isinstance(value, AnoTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.92, 0.99)))
    return AnoTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "ano"),
        ...
        logarithmic_schedule=bool(payload.get("logarithmic_schedule", getattr(optimizer, "logarithmic_schedule", False))),
...
def _group_config(group: Mapping[str, Any], config: AnoTrainingExecutorConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "logarithmic_schedule": bool(group.get("logarithmic_schedule", config.logarithmic_schedule)),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("ano_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("fixed_decay", False)):
        blockers.append("ano_fixed_decay_not_supported_for_canary")
    if bool(group_cfg.get("logarithmic_schedule", False)):
        blockers.append("ano_logarithmic_schedule_not_supported_for_canary")
    return blockers
...
__all__ = ["AnoTrainingExecutor", "AnoTrainingExecutorConfig", "build_plugin_ano_training_executor"]
