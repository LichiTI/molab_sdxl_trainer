"""Default-off rollout policy for selected plugin simple-formula routes."""
...
POLICY_KIND = "plugin_simple_formula_canary_rollout_policy_v0"
OPTIMIZER_FAMILY = "simple_formula"
FALLBACK_BACKEND = "python_plugin_selected_optimizer"
REPO_ROOT = Path(__file__).resolve().parents[2]


def build_plugin_simple_formula_canary_rollout_policy_scorecard(
    *,
    ...
    shadow = _as_dict(shadow_matrix_report or build_plugin_simple_formula_e2e_shadow_matrix_scorecard())
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_simple_formula_canary_rollout_policy_scorecard_v0",
        "gate": "plugin_simple_formula_canary_rollout_policy",
        "ok": ready,
        ...
        "summary": {
            "optimizer_count": len(policy["selected_optimizer_names"]),
            "canary_rollout_policy_ready": ready,
            "canary_rollout_policy_ready_count": len(policy["selected_optimizer_names"]) if ready else 0,
            "manual_review_required": True,
            "canary_auto_enabled": False,
        ...
        "promotion_blockers": _dedupe(
            blockers
            + [
                "plugin_simple_formula_runtime_dispatch_disabled_pending_review",
                "plugin_simple_formula_real_dispatch_wiring_missing",
                "owner_release_hold_missing",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "requires explicit owner review before wiring selected plugin simple-formula canary dispatch"
            if ready
            else "fix selected plugin simple-formula canary rollout policy blockers"
        ),
        "notes": [
            ...
            "Actual dispatch wiring remains a separate owner/release decision.",
        ],
    }
    if write_artifact:
...
def _policy(shadow: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "required_preflight_gates": [
            "plugin_simple_formula_e2e_shadow_matrix",
...
def _validations(shadow: Mapping[str, Any], policy: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _validation(
            ...
            "plugin_simple_formula_e2e_shadow_matrix_missing",
        ),
        _validation(
            "policy_default_off",
            ...
            "plugin_simple_formula_policy_not_default_off",
        ),
        _validation(
            ...
            "plugin_simple_formula_policy_missing_manual_review",
        ),
        _validation(
            ...
            "plugin_simple_formula_policy_missing_rollback",
        ),
        _validation(
            ...
            "plugin_simple_formula_policy_enabled_dispatch",
        ),
        _validation(
            "default_behavior_unchanged",
            policy.get("default_behavior_changed") is False and shadow.get("default_behavior_changed") is False,
            "plugin_simple_formula_policy_changed_default_behavior",
        ),
    ]
...
def _write_artifact(report: Mapping[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_plugin_simple_formula_canary_rollout_policy_scorecard.json"
    path.write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")


__all__ = ["POLICY_KIND", "build_plugin_simple_formula_canary_rollout_policy_scorecard"]
