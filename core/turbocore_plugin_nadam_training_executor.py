"""Default-off training executor for selected plugin NAdam native dispatch."""
...
ENTRYPOINT = "probe_nadam_training_tensor_binding_canary_py"
REPO_ROOT = Path(__file__).resolve().parents[2]


@dataclass(frozen=True)
class PluginNAdamTrainingExecutorConfig:
    lr: float = 1e-3
...
class PluginNAdamTrainingExecutor:
    """Launch selected NAdam native steps against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: PluginNAdamTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("PluginNAdamTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("plugin_nadam_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("plugin_nadam_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("plugin_nadam_training_dispatch_entrypoint_missing")
        ...
        if not cases:
            blockers.append("plugin_nadam_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_nadam_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "plugin_nadam_native_step_failed"),
            "optimizer_kind": "nadam",
    ...
    def _step_param(self, native: Any, param: torch.nn.Parameter, group_cfg: Mapping[str, Any]) -> dict[str, Any]:
        if type(self.optimizer).__name__.lower() != "nadam":
            return _case_failed("plugin_nadam_training_executor_optimizer_not_nadam", "plugin_nadam_optimizer_class_unsupported")
        if param.dtype != torch.float32 or param.grad is None or param.grad.dtype != torch.float32:
            return _case_failed("plugin_nadam_native_step_requires_float32", "plugin_nadam_native_step_dtype_unsupported")
        if not param.is_contiguous() or not param.grad.is_contiguous():
            return _case_failed("plugin_nadam_native_step_requires_contiguous_param_grad", "plugin_nadam_native_step_layout_unsupported")
        state = self.optimizer.state[param]
        required = ("exp_avg", "exp_avg_sq", "mu_product", "step")
        if not all(name in state for name in required):
            return _case_failed("plugin_nadam_live_state_missing", "plugin_nadam_live_state_missing")
        exp_avg = state["exp_avg"]
        exp_avg_sq = state["exp_avg_sq"]
        mu_product = state["mu_product"]
        step_state = state["step"]
        state_tensors = (exp_avg, exp_avg_sq, mu_product, step_state)
        if not all(torch.is_tensor(item) for item in state_tensors):
            return _case_failed("plugin_nadam_live_state_tensors_missing", "plugin_nadam_live_state_missing")
        if any(item.dtype != torch.float32 for item in state_tensors):
            return _case_failed("plugin_nadam_native_state_requires_float32", "plugin_nadam_native_state_dtype_unsupported")
        if not all(item.device.type == "cuda" for item in state_tensors):
            return _case_failed("plugin_nadam_native_state_requires_cuda", "plugin_nadam_native_state_device_unsupported")
        if not exp_avg.is_contiguous() or not exp_avg_sq.is_contiguous():
            return _case_failed("plugin_nadam_native_state_requires_contiguous", "plugin_nadam_native_state_layout_unsupported")
        if mu_product.numel() != 1 or step_state.numel() != 1:
            return _case_failed("plugin_nadam_scalar_state_requires_scalar_tensor", "plugin_nadam_live_state_shape_unsupported")
        step_before = int(step_state.detach().reshape(-1)[0].cpu().item())
        launch_config = {
            **dict(group_cfg),
        ...
        try:
            ...
            return _case_failed(f"{type(exc).__name__}: {exc}", "plugin_nadam_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(
                str(launch.get("reason") or "plugin_nadam_native_step_failed"),
                "plugin_nadam_native_step_failed",
                launch=launch,
            )
        return {
            "schema_version": 1,
            ...
            "step_after": int(launch.get("step_after", step_before + 1) or (step_before + 1)),
...
def build_plugin_nadam_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: PluginNAdamTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> PluginNAdamTrainingExecutor:
    return PluginNAdamTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: PluginNAdamTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> PluginNAdamTrainingExecutorConfig:
    if isinstance(value, PluginNAdamTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return PluginNAdamTrainingExecutorConfig(
        lr=float(payload.get("lr", group.get("lr", 1e-3))),
        betas=(float(betas[0]), float(betas[1])),
        momentum_decay=float(payload.get("momentum_decay", group.get("momentum_decay", 0.004))),
        eps=float(payload.get("eps", group.get("eps", 1e-8))),
        weight_decay=float(payload.get("weight_decay", group.get("weight_decay", 0.0))),
        decoupled_weight_decay=bool(
            payload.get("decoupled_weight_decay", group.get("decoupled_weight_decay", False))
        ),
        maximize=bool(payload.get("maximize", group.get("maximize", False))),
        max_numel=int(payload.get("max_numel", 1_048_576)),
        require_native_cuda=bool(payload.get("require_native_cuda", True)),
    )


def _group_config(group: Mapping[str, Any], config: PluginNAdamTrainingExecutorConfig) -> dict[str, Any]:
    betas = group.get("betas", config.betas)
    return {
        "lr": float(group.get("lr", config.lr)),
        "betas": [float(betas[0]), float(betas[1])],
        "momentum_decay": float(group.get("momentum_decay", config.momentum_decay)),
        "eps": float(group.get("eps", config.eps)),
        "weight_decay": float(group.get("weight_decay", config.weight_decay)),
        "decoupled_weight_decay": bool(group.get("decoupled_weight_decay", config.decoupled_weight_decay)),
        "maximize": bool(group.get("maximize", config.maximize)),
    }
...
__all__ = [
    "PluginNAdamTrainingExecutor",
    "PluginNAdamTrainingExecutorConfig",
    "build_plugin_nadam_training_executor",
]
