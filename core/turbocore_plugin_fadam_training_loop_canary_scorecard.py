"""TrainingLoop canary for selected plugin FAdam native dispatch."""
...
def build_plugin_fadam_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_fadam_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = _strings(case.get("blocked_reasons"))
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_fadam_training_loop_canary_scorecard_v0",
        "gate": "plugin_fadam_training_loop_native_canary",
        "ok": ok,
        ...
        "summary": {
            ...
            "momentum_dtype": case.get("momentum_dtype"),
            "fim_dtype": case.get("fim_dtype"),
        },
        "promotion_blockers": blockers
        + [
            "fadam_e2e_shadow_matrix_refresh_required",
            "fadam_canary_rollout_policy_refresh_required",
            "product_rollout_review_missing",
        ],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "refresh selected Adam-like e2e shadow matrix and rollout policy for fadam"
            if ok
            else "fix selected plugin FAdam TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    def _fake_train_step(
        ...
        for item in params:
            item.grad = None
        loss = sum(((item.float() * 0.17) ** 2).mean() + item.float().mean() * 0.003 for item in params)
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_fadam_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_fadam_training_loop_did_not_emit_step"],
        }
    ...
    momentum_dtype = str(state["momentum"].dtype).replace("torch.", "") if "momentum" in state else ""
    fim_dtype = str(state["fim"].dtype).replace("torch.", "") if "fim" in state else ""
    ok = bool(
        result.get("steps") == 1
        and native_step
        and native_kernel
        ...
        and step_after == 2
        and type(optimizer).__name__ == "FAdam"
        and str(executor_result.get("optimizer_kind") or "") == "fadam"
        and {"momentum", "fim"}.issubset(state_keys)
        and momentum_dtype == "float32"
        and fim_dtype == "float32"
    )
    return {
        "schema_version": 1,
        "ok": ok,
        ...
        "blocked_reasons": []
        if ok
        else _dedupe(_strings(runtime.get("blocked_reasons")) + ["plugin_fadam_training_loop_native_step_missing"]),
    }


def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    device = torch.device("cuda")
    param = torch.nn.Parameter(torch.linspace(-0.13, 0.23, steps=4096, device=device, dtype=torch.float32))
    optimizer = _create_selected_fadam_plugin_optimizer(param)
    loop = TrainingLoop(
...
def _create_selected_fadam_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
...
def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> None:
    param.grad = None
    loss = ((param.float() * 0.11) ** 2).mean() + param.float().mean() * 0.002
    loss.backward()
    optimizer.step()
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin FAdam TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_fadam_training_loop_canary_scorecard"]
