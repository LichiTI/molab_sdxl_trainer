"""Owner-approval hold gate for quantized simple optimizer canaries."""
...
HOLD_KIND = "simple_formula_quantized_owner_approval_hold_v0"


def build_simple_optimizer_quantized_owner_approval_hold_scorecard(
    *,
    dispatch_review_report: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Mark review evidence complete while keeping product dispatch blocked."""

    review_report = _as_dict(
        dispatch_review_report or build_simple_optimizer_quantized_dispatch_integration_review_scorecard()
    )
    ...
    ready = not blockers
    approval_blockers = [
        "simple_quantized_owner_approval_missing",
        "simple_quantized_product_dispatch_not_approved",
    ]
    return {
        "schema_version": 1,
        "scorecard": "turbocore_simple_optimizer_quantized_owner_approval_hold_scorecard_v0",
        "gate": "simple_formula_quantized_owner_approval_hold",
        "ok": ready,
        "promotion_ready": False,
        "owner_approval_hold_ready": ready,
        ...
        "recommended_next_step": (
            "record explicit owner approval before any quantized simple product dispatch wiring"
            if ready
            else "fix quantized simple owner-approval hold blockers"
        ),
        "notes": [
            "This gate is a hold state, not a product promotion.",
            "Dispatch review evidence is packaged but owner approval is intentionally absent.",
...
def _hold_manifest(review_report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "approval_state": "pending_owner_approval",
        "owner_approval_recorded": False,
        "release_approval_recorded": False,
        "allowed_initial_modes": ["off", "observe"],
        "blocked_modes_until_owner_approval": ["canary", "auto"],
...
def _validations(review_report: Mapping[str, Any], hold: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _validation(
            ...
            "simple_quantized_dispatch_review_missing",
        ),
        _validation(
            "optimizer_set_complete",
            optimizer_types == EXPECTED_OPTIMIZERS,
            "simple_quantized_owner_hold_optimizer_set_incomplete",
        ),
        _validation(
            "owner_approval_not_recorded",
            hold.get("owner_approval_recorded") is False and hold.get("approval_state") == "pending_owner_approval",
            "simple_quantized_owner_hold_unexpected_approval",
        ),
        _validation(
            "canary_auto_blocked",
            hold.get("allowed_initial_modes") == ["off", "observe"]
            and hold.get("blocked_modes_until_owner_approval") == ["canary", "auto"],
            "simple_quantized_owner_hold_allows_dispatch",
        ),
        _validation(
            "product_boundaries_frozen",
            all(frozen.get(field) is False for field in frozen),
            "simple_quantized_owner_hold_changed_product_boundaries",
        ),
        _validation(
            ...
            "simple_quantized_owner_hold_review_enabled_dispatch",
        ),
    ]
...
__all__ = ["HOLD_KIND", "build_simple_optimizer_quantized_owner_approval_hold_scorecard"]
