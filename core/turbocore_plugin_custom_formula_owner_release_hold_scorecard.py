"""Owner/release hold for selected custom-formula plugin optimizers."""
...
HOLD_KIND = "plugin_custom_formula_owner_release_hold_v0"


def build_plugin_custom_formula_owner_release_hold_scorecard(
    *,
    family_batch_report: Mapping[str, Any] | None = None,
    write_artifact: bool = False,
) -> dict[str, Any]:
    """Record a default-off hold after custom-formula matrices are implementation-ready."""

    batch = _as_dict(family_batch_report or build_plugin_custom_formula_family_batch_scorecard())
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_custom_formula_owner_release_hold_scorecard_v0",
        "gate": "plugin_custom_formula_owner_release_hold",
        "ok": ready,
        "promotion_ready": False,
        "owner_release_hold_ready": ready,
        "family_batch_ready": batch.get("selected_custom_formula_family_batch_ready") is True,
        ...
        "summary": {
            ...
            "formula_step_execution_ready_count": int(
                summary.get("formula_step_execution_ready_count", 0) or 0
            ),
            "resume_next_step_replay_ready_count": int(
                summary.get("resume_next_step_replay_ready_count", 0) or 0
            ),
            "owner_approval_recorded": False,
            "release_approval_recorded": False,
            "runtime_dispatch_ready": False,
        ...
        "promotion_blockers": _dedupe(
            blockers
            + [
                "plugin_custom_formula_owner_approval_missing",
                "plugin_custom_formula_release_approval_missing",
                "plugin_custom_formula_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "add custom-formula request/schema/UI non-exposure guard before product dispatch wiring"
            if ready
            else "fix custom-formula owner/release hold blockers"
        ),
        "notes": [
            "This gate records a hold state only; it does not approve native dispatch.",
            "Custom-formula matrix evidence remains report-only until owner/release approval.",
...
def _hold_manifest(batch: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "hold_kind": HOLD_KIND,
        "optimizer_family": "custom_formula",
        ...
        "required_approval_artifacts": [
            ...
            "custom_formula_formula_parity_review",
...
def _validations(batch: Mapping[str, Any], hold: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _validation(
            "family_batch_ready",
            batch.get("selected_custom_formula_family_batch_ready") is True,
            "plugin_custom_formula_family_batch_missing",
        ),
        _validation(
            "optimizer_set_complete",
            len(hold.get("selected_optimizer_names", [])) == optimizer_count == 47,
            "plugin_custom_formula_optimizer_set_incomplete",
        ),
        _validation(
            "formula_resume_matrices_ready",
            int(summary.get("formula_parity_matrix_implementation_ready_count", 0) or 0) == optimizer_count
            ...
            and int(summary.get("execution_failed_count", 0) or 0) == 0,
            "plugin_custom_formula_matrices_not_ready",
        ),
        _validation(
        ...
        _validation(
            ...
            "plugin_custom_formula_batch_enabled_dispatch",
        ),
    ]
...
def _write_artifact(report: Mapping[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_plugin_custom_formula_owner_release_hold_scorecard.json"
...
__all__ = ["build_plugin_custom_formula_owner_release_hold_scorecard"]
