"""Default-off AdaGC Triton TrainingLoop executor for selected plugin canaries."""
...
class AdaGCTrainingExecutorConfig:
    optimizer_kind: str = "adagc"
    lr: float = 1.0e-3
    beta1: float = 0.9
    beta2: float = 0.999
    beta: float = 0.98
    lambda_abs: float = 100.0
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _adagc_warmup_dense_kernel(
        param_ptr,
        grad_ptr,
        exp_avg_ptr,
        exp_avg_sq_ptr,
        ...
        g_hat = g * h_t
        m_new = m * beta1 + g_hat * (1.0 - beta1)
        v_new = v * beta2 + g_hat * g_hat * (1.0 - beta2)
...
class AdaGCTrainingExecutor:
    """Launch a real AdaGC warmup fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: AdaGCTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AdaGCTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("adagc_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("adagc_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("adagc_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"adagc_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("adagc_maximize_not_supported")
        ...
        if not cases:
            blockers.append("adagc_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_adagc_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "adagc_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("adagc_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("adagc_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step >= int(group_cfg["warmup_steps"]):
            return _case_failed("adagc_training_executor_only_warmup_path_supported_for_canary")
        state = self.optimizer.state[param]
        exp_avg = _state_tensor(state, "exp_avg", param)
        exp_avg_sq = _state_tensor(state, "exp_avg_sq", param)
        gamma = _state_scalar_tensor(state, "gamma", param)
        before = param.detach().clone()
        grad = param.grad.detach()
        grad_norm = float(torch.linalg.vector_norm(grad.float()).detach().cpu().item()) + float(group_cfg["eps"])
        h_t = min(float(group_cfg["lambda_abs"]) / grad_norm, 1.0) if grad_norm > 0.0 else 1.0
        g_hat_norm = float(torch.linalg.vector_norm((grad.float() * h_t)).detach().cpu().item())
        gamma.fill_(g_hat_norm if step == 1 else min(float(gamma.item()), g_hat_norm))
        ...
        step_size = float(group_cfg["lr"]) / bias_correction1
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _adagc_warmup_dense_kernel[grid](
            param,
            grad,
            exp_avg,
            exp_avg_sq,
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["adagc_training_executor_parameters_not_mutated"],
        }


def build_plugin_adagc_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AdaGCTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> AdaGCTrainingExecutor:
    return AdaGCTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AdaGCTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AdaGCTrainingExecutorConfig:
    if isinstance(value, AdaGCTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return AdaGCTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "adagc"),
        lr=float(payload.get("lr", group.get("lr", 1.0e-3)) or 1.0e-3),
        beta1=float(betas[0]),
        beta2=float(betas[1]),
        beta=float(payload.get("beta", group.get("beta", 0.98)) or 0.98),
        lambda_abs=float(payload.get("lambda_abs", group.get("lambda_abs", 100.0)) or 100.0),
        lambda_rel=float(payload.get("lambda_rel", group.get("lambda_rel", 1.05)) or 1.05),
        warmup_steps=int(payload.get("warmup_steps", group.get("warmup_steps", 100)) or 100),
...
def _group_config(group: Mapping[str, Any], config: AdaGCTrainingExecutorConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "lambda_abs": float(group.get("lambda_abs", config.lambda_abs) or config.lambda_abs),
        "lambda_rel": float(group.get("lambda_rel", config.lambda_rel) or config.lambda_rel),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("adagc_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("fixed_decay", False)):
        blockers.append("adagc_fixed_decay_not_supported_for_canary")
    return blockers
...
def _state_scalar_tensor(state: dict[str, Any], key: str, param: torch.nn.Parameter) -> torch.Tensor:
    ...
    if torch.is_tensor(existing) and existing.numel() == 1:
        ...
        state[key] = converted
        return converted
    state[key] = torch.empty((1,), dtype=torch.float32, device=param.device)
    return state[key]
...
__all__ = ["AdaGCTrainingExecutor", "AdaGCTrainingExecutorConfig", "build_plugin_adagc_training_executor"]
