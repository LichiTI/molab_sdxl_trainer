"""Owner/release hold for selected state-adapter-special plugin optimizers."""
...
HOLD_KIND = "plugin_state_adapter_special_owner_release_hold_v0"


def build_plugin_state_adapter_special_owner_release_hold_scorecard(
    *,
    family_batch_report: Mapping[str, Any] | None = None,
    write_artifact: bool = False,
) -> dict[str, Any]:
    """Record a default-off hold after state-adapter ABI matrices are implementation-ready."""

    batch = _as_dict(family_batch_report or build_plugin_state_adapter_special_family_batch_scorecard())
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_state_adapter_special_owner_release_hold_scorecard_v0",
        "gate": "plugin_state_adapter_special_owner_release_hold",
        "ok": ready,
        "promotion_ready": False,
        "owner_release_hold_ready": ready,
        "family_batch_ready": batch.get("selected_state_adapter_special_family_batch_ready") is True,
        ...
        "promotion_blockers": _dedupe(
            blockers
            + [
                "plugin_state_adapter_special_owner_approval_missing",
                "plugin_state_adapter_special_release_approval_missing",
                "plugin_state_adapter_special_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "add state-adapter-special request/schema/UI non-exposure guard before product dispatch wiring"
            if ready
            else "fix state-adapter-special owner/release hold blockers"
        ),
        "notes": [
            "This gate records a hold state only; it does not approve native dispatch.",
            "State-adapter-special evidence remains report-only until owner/release approval.",
...
def _hold_manifest(batch: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "hold_kind": HOLD_KIND,
        "optimizer_family": "state_adapter_special",
        ...
        "required_approval_artifacts": [
            ...
            "state_adapter_param_ownership_abi_review",
...
def _validations(batch: Mapping[str, Any], hold: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _validation(
            "family_batch_ready",
            batch.get("selected_state_adapter_special_family_batch_ready") is True,
            "plugin_state_adapter_special_family_batch_missing",
        ),
        _validation(
            "optimizer_set_complete",
            len(hold.get("selected_optimizer_names", [])) == optimizer_count == 3,
            "plugin_state_adapter_special_optimizer_set_incomplete",
        ),
        _validation(
            "adapter_matrices_ready",
            int(summary.get("adapter_abi_implementation_ready_count", 0) or 0) == optimizer_count
            and int(summary.get("native_kernel_precondition_implementation_ready_count", 0) or 0)
            == optimizer_count
            and int(summary.get("adapter_resume_matrix_implementation_ready_count", 0) or 0) == optimizer_count
            and replay_planned == 15
            and translation_planned == 12
            and int(summary.get("adapter_resume_replay_case_implementation_ready_count", 0) or 0)
            == replay_planned
            and int(summary.get("adapter_resume_translation_case_implementation_ready_count", 0) or 0)
            == translation_planned,
            "plugin_state_adapter_special_matrices_not_ready",
        ),
        _validation(
        ...
        _validation(
            ...
            "plugin_state_adapter_special_batch_enabled_dispatch",
        ),
    ]
...
def _write_artifact(report: Mapping[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_plugin_state_adapter_special_owner_release_hold_scorecard.json"
...
__all__ = ["build_plugin_state_adapter_special_owner_release_hold_scorecard"]
