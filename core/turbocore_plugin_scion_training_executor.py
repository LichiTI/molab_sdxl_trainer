"""Default-off SCION Triton TrainingLoop executor for selected plugin canaries."""
...
class ScionTrainingExecutorConfig:
    optimizer_kind: str = "scion"
    lr: float = 1.0e-3
    momentum: float = 0.1
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _scion_first_step_none_kernel(
        param_ptr,
        grad_ptr,
        d_ptr,
        n_elements,
        ...
        d = grad * momentum
        param_new = param - lr * scale * d

        tl.store(d_ptr + offsets, d, mask=mask)
        tl.store(param_ptr + offsets, param_new, mask=mask)


class ScionTrainingExecutor:
    """Launch a real SCION norm_type=NONE first-step fp32 Triton kernel."""
    ...
    def __init__(
        ...
        *,
        ...
        config: ScionTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("ScionTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("scion_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("scion_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("scion_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"scion_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("scion_maximize_not_supported")
        ...
        if not cases:
            blockers.append("scion_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_scion_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "scion_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("scion_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("scion_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("scion_training_executor_only_first_step_supported_for_canary")
        state = self.optimizer.state[param]
        d = state.get("d")
        if d is None:
            d = torch.zeros_like(param)
            state["d"] = d
        if not torch.is_tensor(d) or tuple(d.shape) != tuple(param.shape) or d.dtype != torch.float32:
            return _case_failed("scion_training_executor_requires_d_state_float32")
        if not d.is_contiguous():
            return _case_failed("scion_training_executor_requires_contiguous_d_state")
        before = param.detach().clone()
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _scion_first_step_none_kernel[grid](
            param,
            param.grad.detach(),
            d,
            int(param.numel()),
        ...
        return {
            "schema_version": 1,
            "ok": mutated and torch.isfinite(param).all().item() and torch.isfinite(d).all().item(),
            ...
            "blocked_reasons": [] if mutated else ["scion_training_executor_parameters_not_mutated"],
        }


def build_plugin_scion_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: ScionTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> ScionTrainingExecutor:
    return ScionTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: ScionTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> ScionTrainingExecutorConfig:
    if isinstance(value, ScionTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    return ScionTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "scion"),
...
def _group_config(group: Mapping[str, Any], config: ScionTrainingExecutorConfig) -> dict[str, Any]:
    return {
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if bool(group_cfg.get("constraint", False)):
        blockers.append("scion_constraint_not_supported_for_canary")
    if int(group_cfg.get("norm_type", 0) or 0) != 0:
        blockers.append("scion_only_norm_type_none_supported_for_canary")
    if dict(group_cfg.get("norm_kwargs") or {}):
        blockers.append("scion_norm_kwargs_not_supported_for_canary")
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("scion_weight_decay_not_supported_for_canary")
    if float(group_cfg.get("scale", 0.0) or 0.0) <= 0.0:
        blockers.append("scion_scale_must_be_positive")
    momentum = float(group_cfg.get("momentum", 0.0) or 0.0)
    if momentum <= 0.0 or momentum > 1.0:
        blockers.append("scion_momentum_out_of_range")
    return blockers
...
__all__ = [
    "ScionTrainingExecutor",
    "ScionTrainingExecutorConfig",
    "build_plugin_scion_training_executor",
]
