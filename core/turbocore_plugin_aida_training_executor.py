"""Default-off Aida Triton TrainingLoop executor for selected plugin canaries."""
...
class AidaTrainingExecutorConfig:
    optimizer_kind: str = "aida"
    lr: float = 1.0e-3
    beta1: float = 0.9
    beta2: float = 0.999
    k: int = 0
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _aida_first_step_dense_kernel(
        param_ptr,
        grad_ptr,
        exp_avg_ptr,
        exp_avg_var_ptr,
        ...
        exp_avg_var = tl.load(exp_avg_var_ptr + offsets, mask=mask, other=0.0).to(tl.float32)

        exp_avg_new = exp_avg * beta1 + grad * (1.0 - beta1)
        grad_residual = exp_avg_new - grad
        exp_avg_var_new = exp_avg_var * beta2 + grad_residual * grad_residual * (1.0 - beta2) + eps
        denom = (tl.sqrt(exp_avg_var_new + 1.0e-15) + eps) / bias_correction2_sqrt
        ...
        tl.store(exp_avg_var_ptr + offsets, exp_avg_var_new, mask=mask)


class AidaTrainingExecutor:
    """Launch a real Aida k=0 first-step fp32 Triton kernel."""
    ...
    def __init__(
        ...
        *,
        ...
        config: AidaTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AidaTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("aida_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("aida_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("aida_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"aida_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("aida_maximize_not_supported")
        ...
        if not cases:
            blockers.append("aida_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_aida_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "aida_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("aida_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("aida_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("aida_training_executor_only_first_step_supported_for_canary")
        ...
        _aida_first_step_dense_kernel[grid](
            param,
            param.grad.detach(),
            exp_avg,
            exp_avg_var,
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["aida_training_executor_parameters_not_mutated"],
        }


def build_plugin_aida_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AidaTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> AidaTrainingExecutor:
    return AidaTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AidaTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AidaTrainingExecutorConfig:
    if isinstance(value, AidaTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return AidaTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "aida"),
        lr=float(payload.get("lr", group.get("lr", 1.0e-3)) or 1.0e-3),
        beta1=float(betas[0]),
        beta2=float(betas[1]),
        k=int(payload.get("k", group.get("k", 0)) or 0),
...
def _group_config(group: Mapping[str, Any], config: AidaTrainingExecutorConfig) -> dict[str, Any]:
    betas = group.get("betas", (config.beta1, config.beta2))
    return {
        "lr": float(group.get("lr", config.lr) or config.lr),
        "beta1": float(betas[0]),
        "beta2": float(betas[1]),
        "k": int(group.get("k", config.k) or config.k),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if int(group_cfg.get("k", 0) or 0) != 0:
        blockers.append("aida_projection_loop_not_supported_for_canary")
    if bool(group_cfg.get("rectify", False)):
        blockers.append("aida_rectify_not_supported_for_canary")
    if bool(group_cfg.get("ams_bound", False)):
        blockers.append("aida_ams_bound_not_supported_for_canary")
    if bool(group_cfg.get("adanorm", False)):
        blockers.append("aida_adanorm_not_supported_for_canary")
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("aida_weight_decay_not_supported_for_canary")
    return blockers
...
__all__ = [
    "AidaTrainingExecutor",
    "AidaTrainingExecutorConfig",
    "build_plugin_aida_training_executor",
]
