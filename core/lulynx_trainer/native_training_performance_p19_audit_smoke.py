"""Smoke checks for Native Training Performance V2-P19 audit."""
...
from devtools.audit_native_training_performance_p19 import (  # noqa: E402
    build_p19_plugin_schedulefree_dispatch_review_audit,
)


def run_smoke() -> dict[str, Any]:
    report = build_p19_plugin_schedulefree_dispatch_review_audit(native_training_mode="observe")
    gates = report["progress_gates"]
    assert report["ok"] is True, report
    assert report["milestone_completed"] is True, report
    assert gates["p18_rollout_policy_complete"] is True, gates
    ...
    return {
        "schema_version": 1,
        "probe": "native_training_performance_p19_audit_smoke",
        "ok": True,
