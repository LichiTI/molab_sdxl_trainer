"""Smoke checks for factored-memory request/schema/UI non-exposure."""
...
from core.turbocore_plugin_factored_memory_request_schema_ui_non_exposure_scorecard import (  # noqa: E402
    build_plugin_factored_memory_request_schema_ui_non_exposure_scorecard,
)


def run_smoke() -> dict[str, Any]:
    report = build_plugin_factored_memory_request_schema_ui_non_exposure_scorecard(write_artifact=True)
    summary = report["summary"]

    assert report["scorecard"] == (
        "turbocore_plugin_factored_memory_request_schema_ui_non_exposure_scorecard_v0"
    ), report
    ...
    assert "plugin_factored_memory_request_schema_ui_exposure_not_approved" in report[
        "promotion_blockers"
    ], report

    unsafe = build_plugin_factored_memory_request_schema_ui_non_exposure_scorecard(
        owner_release_hold_report=_hold_fixture(native_dispatch_allowed=True),
    )
    assert unsafe["ok"] is False, unsafe
    assert "plugin_factored_memory_request_schema_ui_hold_enabled_boundary" in unsafe[
        "blocked_reasons"
    ], unsafe

    return {
        "schema_version": 1,
        "probe": "turbocore_plugin_factored_memory_request_schema_ui_non_exposure_scorecard_smoke",
        "ok": True,
        "summary": summary,
