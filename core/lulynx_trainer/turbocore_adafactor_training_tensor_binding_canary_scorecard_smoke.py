"""Smoke checks for Adafactor training tensor binding canary."""
...
from core.turbocore_adafactor_training_tensor_binding_canary_scorecard import (  # noqa: E402
    build_adafactor_training_tensor_binding_canary_scorecard,
)


def run_smoke() -> dict[str, Any]:
    report = build_adafactor_training_tensor_binding_canary_scorecard(workspace_root=REPO_ROOT)
    assert report["scorecard"] == "turbocore_adafactor_training_tensor_binding_canary_scorecard_v0", report
    ...
    assert {"factored_128x128", "unfactored_4x4"}.issubset(names), probe
    return {
        "schema_version": 1,
        "probe": "turbocore_adafactor_training_tensor_binding_canary_scorecard_smoke",
        "ok": True,
