"""Run-dispatch boundary for profiled adapter-target execution-job templates."""
...
def build_adapter_target_run_dispatch_boundary(
    *,
    ...
    if boundary.get("scorecard") != "adapter_target_execution_job_creation_boundary_v0":
    ...
    return {
        "schema_version": 1,
        "scorecard": "adapter_target_run_dispatch_boundary_v0",
        "ok": ready,
        ...
        "recommended_next_step": (
            "prepare training-launch boundary while keeping profiled-target runs undispatched"
            if ready
            else "complete default-off adapter-target run-dispatch boundary"
        ),
    }
...
__all__ = ["build_adapter_target_run_dispatch_boundary"]
