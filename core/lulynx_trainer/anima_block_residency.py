"""Block-scoped residency helpers for native Anima DiT.

The first implementation is deliberately conservative: it does not move whole
blocks, because LoRA adapter parameters inside those blocks must remain on the
training device. Instead it enables the existing CPU-pinned frozen-weight path
only for base Linear modules under ``net.blocks``.
"""
...
VALID_ANIMA_BLOCK_RESIDENCY_MODES = {"resident", "streaming_offload", "block_cpu_pinned"}


def normalize_anima_block_residency(value: Any) -> str:
    return normalize_dit_block_residency(value)


@dataclass
class AnimaBlockResidencyReport:
    mode: str
    block_count: int = 0
...
def _block_modules(model: nn.Module) -> list[nn.Module]:
    ...
    net = getattr(unet, "net", None)
    blocks = getattr(net, "blocks", None)
    if blocks is None:
        return []
    try:
        return [blocks[index] for index in range(len(blocks))]
    except Exception:
        return list(blocks)


def apply_anima_block_residency(
    model: nn.Module,
    *,
    mode: str = "resident",
    min_parameter_count: int = 0,
...
) -> AnimaBlockResidencyReport:
    normalized = normalize_anima_block_residency(mode)
    ...
    report = AnimaBlockResidencyReport(
        mode=normalized,
