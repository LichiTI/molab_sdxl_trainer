"""Smoke checks for Native Training Performance V2-p62 audit."""
...
from devtools.audit_native_training_performance_p62 import (  # noqa: E402
    build_p62_plugin_adamod_training_loop_canary_audit,
)


def run_smoke() -> dict[str, Any]:
    report = build_p62_plugin_adamod_training_loop_canary_audit()
    gates = report["progress_gates"]
    assert report["ok"] is True, report
    assert report["milestone_completed"] is True, report
    assert gates["p61_runtime_dispatch_shadow_complete"] is True, gates
    ...
    return {
        "schema_version": 1,
        "probe": "native_training_performance_p62_audit_smoke",
        "ok": True,
