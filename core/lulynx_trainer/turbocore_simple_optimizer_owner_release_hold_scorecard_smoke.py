"""Smoke checks for fp32 simple optimizer owner/release hold scorecard."""
...
def run_smoke() -> dict[str, Any]:
    product_canary = build_simple_optimizer_product_training_canary_scorecard(workspace_root=REPO_ROOT)
    assert product_canary["representative_product_training_canary_ready"] is True, product_canary
    payload = build_simple_optimizer_owner_release_hold_scorecard(
        product_training_canary_report=product_canary,
        workspace_root=REPO_ROOT,
    )
    hold = payload["hold_manifest"]
    frozen = hold["frozen_product_boundaries"]
    assert payload["scorecard"] == "turbocore_simple_optimizer_owner_release_hold_scorecard_v0", payload
    ...
    assert hold["optimizer_types"] == ["Lion", "SGDNesterov"], hold
    ...
    assert "simple_formula_release_approval_missing" in payload["promotion_blockers"], payload
    _write_real_artifact(payload)
    return {
        "schema_version": 1,
        "probe": "turbocore_simple_optimizer_owner_release_hold_scorecard_smoke",
        "ok": True,
...
def _write_real_artifact(payload: dict[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_simple_optimizer_owner_release_hold_scorecard.json"
