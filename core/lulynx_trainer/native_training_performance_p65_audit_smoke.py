"""Smoke checks for Native Training Performance V2-p65 audit."""
...
from devtools.audit_native_training_performance_p65 import (  # noqa: E402
    build_p65_plugin_adamp_runtime_adapter_shadow_audit,
)


def run_smoke() -> dict[str, Any]:
    report = build_p65_plugin_adamp_runtime_adapter_shadow_audit(quick=True)
    ...
    assert report["native_call_performed_by_p65"] is False, report
    assert report["p64_audit_builder"] == "build_p64_plugin_adamp_training_tensor_binding_audit", report
    assert gates["p64_training_tensor_binding_complete"] is True, gates
    ...
    return {
        "schema_version": 1,
        "probe": "native_training_performance_p65_audit_smoke",
        "ok": True,
