"""Execution-job creation boundary for T-LoRA A/B request submissions."""
...
def build_tlora_ab_execution_job_creation_boundary(
    *,
    ...
    if submission.get("scorecard") != "tlora_ab_request_submission_boundary_v0":
    ...
    return {
        "schema_version": 1,
        "scorecard": "tlora_ab_execution_job_creation_boundary_v0",
        "ok": ready,
        ...
        "recommended_next_step": (
            "prepare run-dispatch boundary while keeping T-LoRA execution job uncreated"
            if ready
            else "complete default-off T-LoRA execution-job creation boundary"
        ),
    }
...
__all__ = ["build_tlora_ab_execution_job_creation_boundary"]
