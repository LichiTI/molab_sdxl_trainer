"""Default-off request adapter boundaries for DiT compute reducers."""
...
def build_dit_compute_reducer_request_adapter_registration_boundary(
    *,
    ...
    blockers = _base_blockers(
        replay,
        plan,
        expected_scorecard="dit_compute_reducer_request_field_config_adapter_replay_v0",
    ...
    blockers.extend(f"mapping_field_missing:{name}" for name in REQUIRED_REDUCER_FIELDS if name not in mapping_fields)
    ...
    return {
        **_safe_report(
            "dit_compute_reducer_request_adapter_registration_boundary_v0",
            ready,
        ...
        "recommended_next_step": (
            "prepare request submission boundary while keeping DiT compute-reducer adapter unregistered"
            if ready
            else "complete default-off DiT compute-reducer request-adapter registration boundary"
        ),
    }


def build_dit_compute_reducer_request_submission_boundary(
    *,
    ...
    blockers = _base_blockers(
        boundary,
        plan,
        expected_scorecard="dit_compute_reducer_request_adapter_registration_boundary_v0",
    ...
    blockers.extend(f"payload_field_missing:{name}" for name in REQUIRED_REDUCER_FIELDS if name not in template)
    ...
    return {
        **_safe_report(
            "dit_compute_reducer_request_submission_boundary_v0",
            ready,
        ...
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "prepare execution-job creation boundary while keeping DiT compute-reducer request submission disabled"
            if ready
            else "complete default-off DiT compute-reducer request submission boundary"
        ),
    }


def build_dit_compute_reducer_execution_job_creation_boundary(
    *,
    ...
    blockers = _base_blockers(
        submission,
        plan,
        expected_scorecard="dit_compute_reducer_request_submission_boundary_v0",
    ...
    return {
        **_safe_report(
            "dit_compute_reducer_execution_job_creation_boundary_v0",
            ready,
        ...
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "prepare run-dispatch boundary while keeping DiT compute-reducer execution job uncreated"
            if ready
            else "complete default-off DiT compute-reducer execution-job creation boundary"
        ),
    }


def build_dit_compute_reducer_run_dispatch_boundary(
    *,
    ...
    blockers = _base_blockers(
        boundary,
        plan,
        expected_scorecard="dit_compute_reducer_execution_job_creation_boundary_v0",
    ...
    return {
        **_safe_report(
            "dit_compute_reducer_run_dispatch_boundary_v0",
            ready,
        ...
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "prepare training-launch boundary while keeping DiT compute-reducer runs undispatched"
            if ready
            else "complete default-off DiT compute-reducer run-dispatch boundary"
        ),
    }


def build_dit_compute_reducer_training_launch_boundary(
    *,
    ...
    blockers = _base_blockers(
        boundary,
        plan,
        expected_scorecard="dit_compute_reducer_run_dispatch_boundary_v0",
    ...
    return {
        **_safe_report(
            "dit_compute_reducer_training_launch_boundary_v0",
            ready,
        ...
        "recommended_next_step": (
            ...
            if ready
            else "complete default-off DiT compute-reducer training-launch boundary"
        ),
    }


def build_dit_compute_reducer_operator_training_launch_boundary(
    *,
    ...
    blockers = _base_blockers(
        boundary,
        review,
        expected_scorecard="dit_compute_reducer_training_launch_boundary_v0",
    ...
    return {
        **_safe_report(
            "dit_compute_reducer_operator_training_launch_boundary_v0",
            ready,
        ...
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "hold for explicit route decision before any DiT compute-reducer training execution contract"
            if ready
            else "complete default-off DiT compute-reducer operator training-launch boundary"
        ),
    }
...
__all__ = [
    "build_dit_compute_reducer_execution_job_creation_boundary",
    "build_dit_compute_reducer_operator_training_launch_boundary",
    "build_dit_compute_reducer_request_adapter_registration_boundary",
    "build_dit_compute_reducer_request_submission_boundary",
    "build_dit_compute_reducer_run_dispatch_boundary",
    "build_dit_compute_reducer_training_launch_boundary",
]
