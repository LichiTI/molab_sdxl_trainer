"""Smoke checks for selected plugin model/shape-aware runtime precondition rehearsal."""
...
from core.turbocore_plugin_model_shape_aware_family_batch_scorecard import TARGET_PLUGIN_OPTIMIZERS  # noqa: E402
...
def run_smoke() -> dict[str, Any]:
    report = build_plugin_model_shape_aware_runtime_precondition_rehearsal_scorecard(write_artifact=True)
    summary = report["summary"]
    rows = {str(row["selected_optimizer_name"]): row for row in report["cases"]}
    assert report["scorecard"] == "turbocore_plugin_model_shape_aware_runtime_precondition_rehearsal_scorecard_v0", report
    ...
    assert summary["param_group_abi_ready_count"] == len(TARGET_PLUGIN_OPTIMIZERS), summary
    assert summary["resume_replay_matrix_ready_count"] == len(TARGET_PLUGIN_OPTIMIZERS), summary
    assert summary["resume_replay_row_ready_count"] == 29, summary
    ...
    for expected in TARGET_PLUGIN_OPTIMIZERS:
        ...
        assert row["param_group_abi_ready"] is True, row
        assert row["resume_replay_matrix_ready"] is True, row
    ...
    return {
        "schema_version": 1,
        "probe": "turbocore_plugin_model_shape_aware_runtime_precondition_rehearsal_scorecard_smoke",
        "ok": True,
        "roadmap": ROADMAP,
        "summary": summary,
    }
