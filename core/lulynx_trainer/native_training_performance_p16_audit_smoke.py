"""Smoke checks for Native Training Performance V2-P16 audit."""
...
from devtools.audit_native_training_performance_p16 import (  # noqa: E402
    build_p16_plugin_schedulefree_runtime_dispatch_shadow_audit,
)


def run_smoke() -> dict[str, Any]:
    report = build_p16_plugin_schedulefree_runtime_dispatch_shadow_audit(native_training_mode="canary")
    gates = report["progress_gates"]
    assert report["ok"] is True, report
    assert report["milestone_completed"] is True, report
    assert gates["runtime_dispatch_shadow"] is True, gates
    ...
    return {
        "schema_version": 1,
        "probe": "native_training_performance_p16_audit_smoke",
        "ok": True,
