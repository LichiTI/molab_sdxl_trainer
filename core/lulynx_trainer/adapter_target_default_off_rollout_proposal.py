"""Default-off rollout proposal gate for profiled adapter target selection."""
...
def build_adapter_target_default_off_rollout_proposal(
    *,
    ...
    if decision.get("scorecard") != "adapter_target_quality_review_decision_v0":
    ...
    if not str(proposal.get("selected_target_scope") or "").strip():
        blockers.append("selected_target_scope_missing")
    if not str(proposal.get("rollback_plan") or "").strip():
        blockers.append("rollback_plan_missing")
    if not str(proposal.get("merge_policy") or "").strip():
        blockers.append("merge_policy_missing")
    if not str(proposal.get("quality_monitoring_plan") or "").strip():
        blockers.append("quality_monitoring_plan_missing")
    ...
    return {
        "schema_version": 1,
        "scorecard": "adapter_target_default_off_rollout_proposal_v0",
        "ok": ready,
        ...
        "speedup_ratio": float(decision.get("speedup_ratio", 0.0) or 0.0),
        "quality_delta": float(decision.get("quality_delta", 0.0) or 0.0),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "hold adapter target rollout proposal default-off until explicit runtime activation review"
            if ready
            else "complete default-off adapter target rollout proposal fields before activation review"
        ),
    }
...
__all__ = ["build_adapter_target_default_off_rollout_proposal"]
