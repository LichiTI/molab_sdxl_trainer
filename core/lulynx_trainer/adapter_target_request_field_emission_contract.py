"""Request-field emission boundary for profiled adapter target activation reviews."""
...
def build_adapter_target_request_field_emission_contract(
    *,
    ...
    if review.get("scorecard") != "adapter_target_runtime_activation_review_v0":
    ...
    return {
        "schema_version": 1,
        "scorecard": "adapter_target_request_field_emission_contract_v0",
        "ok": ready,
        ...
        "recommended_next_step": (
            "replay adapter-target request fields through ConfigAdapter before any runtime registration"
            if ready
            else "complete default-off adapter-target request-field emission contract before request wiring"
        ),
    }
...
__all__ = ["build_adapter_target_request_field_emission_contract"]
