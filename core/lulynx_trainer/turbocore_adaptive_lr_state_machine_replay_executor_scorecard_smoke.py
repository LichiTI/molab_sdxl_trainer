"""Smoke checks for built-in adaptive-LR replay executor scorecard."""
...
def run_smoke() -> dict[str, Any]:
    replay_matrix = build_adaptive_lr_state_machine_replay_matrix_scorecard()
    payload = build_adaptive_lr_state_machine_replay_executor_scorecard(
        replay_matrix_report=replay_matrix
    )
    rows = {str(row["optimizer_type"]): row for row in payload["rows"]}
    summary = payload["summary"]

    assert payload["scorecard"] == "turbocore_adaptive_lr_state_machine_replay_executor_scorecard_v0", payload
    assert payload["ok"] is True, payload
    assert payload["state_machine_replay_executor_ready"] is True, payload
    ...
    assert summary["reference_replay_executor_ready_count"] == len(TARGET_CASES), summary
    assert summary["state_machine_replay_matrix_implementation_ready_count"] == len(TARGET_CASES), summary
    assert summary["resume_next_step_parity_passed_count"] == len(TARGET_CASES), summary
    ...
    for case in TARGET_CASES:
        ...
        assert row["state_machine_status"] == "reference_replay_executor_ready", row
        assert row["reference_replay_executor_ready"] is True, row
        assert row["resume_next_step_parity_passed"] is True, row
        assert row["max_resume_diff"] <= row["tolerance"], row
        assert row["matrix_artifact_ready"] is True, row
        assert row["state_machine_replay_matrix_implementation_ready"] is True, row
        ...
        assert row["state_dict_top_level_keys"], row

    _write_real_artifact(payload)
    return {
        "schema_version": 1,
        "probe": "turbocore_adaptive_lr_state_machine_replay_executor_scorecard_smoke",
        "ok": True,
...
def _write_real_artifact(payload: dict[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_adaptive_lr_state_machine_replay_executor_scorecard.json"
