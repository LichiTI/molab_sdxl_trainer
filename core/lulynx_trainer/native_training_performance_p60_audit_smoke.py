"""Smoke checks for Native Training Performance V2-p60 audit."""
...
from devtools.audit_native_training_performance_p60 import (  # noqa: E402
    build_p60_plugin_adamod_training_tensor_binding_audit,
)


def run_smoke() -> dict[str, Any]:
    report = build_p60_plugin_adamod_training_tensor_binding_audit()
    gates = report["progress_gates"]
    assert report["ok"] is True, report
    assert report["milestone_completed"] is True, report
    assert gates["p59_adamod_scratch_kernel_complete"] is True, gates
    ...
    return {
        "schema_version": 1,
        "probe": "native_training_performance_p60_audit_smoke",
        "ok": True,
