"""Smoke checks for Native Training Performance V2-p64 audit."""
...
from devtools.audit_native_training_performance_p64 import (  # noqa: E402
    build_p64_plugin_adamp_training_tensor_binding_audit,
)


def run_smoke() -> dict[str, Any]:
    report = build_p64_plugin_adamp_training_tensor_binding_audit()
    gates = report["progress_gates"]
    assert report["ok"] is True, report
    assert report["milestone_completed"] is True, report
    assert gates["p63_adamp_scratch_kernel_complete"] is True, gates
    ...
    return {
        "schema_version": 1,
        "probe": "native_training_performance_p64_audit_smoke",
        "ok": True,
