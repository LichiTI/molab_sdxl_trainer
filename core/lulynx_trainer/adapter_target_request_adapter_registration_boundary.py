"""Request-adapter registration boundary for profiled adapter target fields."""
...
def build_adapter_target_request_adapter_registration_boundary(
    *,
    ...
    if replay.get("scorecard") != "adapter_target_request_field_config_adapter_replay_v0":
    ...
    return {
        "schema_version": 1,
        "scorecard": "adapter_target_request_adapter_registration_boundary_v0",
        "ok": ready,
        ...
        "parsed_module_count": int(replay.get("parsed_module_count") or 0),
        "parsed_rank_count": int(replay.get("parsed_rank_count") or 0),
        "config_adapter_replay_ready": bool(replay.get("config_adapter_replay_ready", replay.get("ok", False))),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "prepare request submission boundary while keeping profiled adapter targets unregistered"
            if ready
            else "complete default-off adapter-target request-adapter registration boundary"
        ),
    }
...
__all__ = ["build_adapter_target_request_adapter_registration_boundary"]
