"""Shared Newbie DiT runtime guardrails.

This module mirrors the Anima guardrail boundary so Newbie low-VRAM policy
fields have one small, testable runtime consumption point.
"""
...
def _block_count(unet: Any) -> int:
    try:
        return len(getattr(unet, "_block_modules", []) or [])
    except Exception:
        return 0


def apply_newbie_dit_runtime_guardrails(
    *,
    config: Any,
    model: Any,
    device: Any,
    ...
    """Apply Newbie DiT checkpoint/residency knobs and return profiles."""
    ...
    checkpoint_requested = bool(getattr(config, "newbie_block_checkpointing", False))
    checkpoint_mode = str(getattr(config, "newbie_block_checkpointing_mode", "block") or "block")
    checkpoint_source = "newbie_block_checkpointing"
    if not checkpoint_requested and bool(checkpoint_auto_recommended):
        checkpoint_requested = True
        checkpoint_source = "auto_for_dit_residency"
        try:
            setattr(config, "newbie_block_checkpointing", True)
            result["config_updates"]["newbie_block_checkpointing"] = True
        except Exception:
            pass
        _log(
            "Newbie block checkpointing auto-enabled for "
            f"residency={residency_mode}; non-resident 1024/4096-token DiT paths need activation recompute."
        )

    try:
        setter = getattr(unet, "set_newbie_block_checkpointing", None)
        getter = getattr(unet, "get_newbie_block_checkpointing_profile", None)
        checkpoint_profile: Dict[str, Any] = {}
        if checkpoint_requested:
            if callable(setter):
                checkpoint_profile = dict(setter(True, checkpoint_mode))
            else:
                enabler = getattr(unet, "enable_gradient_checkpointing", None)
                if not callable(enabler):
                    raise RuntimeError("native Newbie DiT does not expose block checkpointing controls")
                enabler()
                block_count = _block_count(unet)
                checkpoint_profile = {
                    "enabled": True,
                    "mode": checkpoint_mode,
                    "block_count": block_count,
                    "checkpointed_blocks": block_count,
                }
            checkpoint_profile["source"] = checkpoint_source
        elif callable(getter):
            checkpoint_profile = dict(getter())
            checkpoint_profile["source"] = "gradient_checkpointing" if checkpoint_profile.get("enabled") else "off"
        elif bool(getattr(config, "gradient_checkpointing", False)):
            block_count = _block_count(unet)
            checkpoint_profile = {
                "enabled": True,
                "mode": "block",
        ...
        if checkpoint_profile and checkpoint_profile.get("enabled"):
            result["checkpoint_profile"] = checkpoint_profile
            _log(
                "Newbie block checkpointing: "
                f"mode={checkpoint_profile.get('mode', 'block')}, "
                f"checkpointed_blocks={checkpoint_profile.get('checkpointed_blocks', 0)}/"
                f"{checkpoint_profile.get('block_count', 0)}, "
                f"source={checkpoint_profile.get('source', 'unknown')}"
            )
        ...
        _log(f"Newbie block checkpointing skipped: {exc}")
    ...
    try:
        ...
        min_params = max(int(getattr(config, "newbie_block_residency_min_params", 0) or 0), 0)
        prefetch_enabled = bool(getattr(config, "newbie_block_prefetch", False))
        prefetch_depth = max(int(getattr(config, "newbie_block_prefetch_depth", 1) or 0), 0)
        ...
        _log(f"Newbie block residency skipped: {exc}")
    return result


__all__ = ["apply_newbie_dit_runtime_guardrails"]
