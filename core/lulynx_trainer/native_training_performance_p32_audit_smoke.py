"""Smoke checks for Native Training Performance V2-P32 audit."""
...
from devtools.audit_native_training_performance_p32 import (  # noqa: E402
    P31_AUDIT_BUILDER,
    build_p32_automagicpp_explicit_canary_rollout_policy_audit,
)


def run_smoke() -> dict[str, Any]:
    report = build_p32_automagicpp_explicit_canary_rollout_policy_audit(quick=True)
    ...
    assert report["native_call_performed_by_p32"] is False, report
    assert report["p31_audit_builder"] == P31_AUDIT_BUILDER, report
    assert report["p31_audit_called_by_p32"] is False, report
    ...
    assert gates["p31_e2e_shadow_matrix_dependency_named"] is True, gates
    ...
    return {
        "schema_version": 1,
        "probe": "native_training_performance_p32_audit_smoke",
        "ok": True,
