"""ConfigAdapter replay boundary for DiT compute-reducer request fields."""

from __future__ import annotations

from typing import Any, Mapping, Sequence


REQUIRED_FIELD_NAMES = (
    "dit_compute_reducer_policy",
    "dit_compute_reducer_selected",
    "dit_compute_reducer_thresholds",
    "dit_compute_reducer_ab_contract",
)


def build_dit_compute_reducer_request_field_config_adapter_replay(
    *,
    request_field_contract: Mapping[str, Any],
    replay_result: Mapping[str, Any],
) -> dict[str, Any]:
    contract = dict(request_field_contract)
    replay = dict(replay_result)
    expected_fields = set(_field_names(contract.get("field_names")))
    parsed = dict(replay.get("parsed_fields") or replay.get("normalized_fields") or {})
    selected = _items(parsed.get("dit_compute_reducer_selected"))
    thresholds = dict(parsed.get("dit_compute_reducer_thresholds") or {})
    blockers: list[str] = []

    if contract.get("scorecard") != "dit_compute_reducer_request_field_emission_contract_v0":
        blockers.append("unexpected_request_field_contract")
    if not bool(contract.get("request_field_emission_contract_ready", contract.get("ok", False))):
        blockers.append("request_field_contract_not_ready")
    if _unsafe_flags(contract, replay):
        blockers.append("unsafe_child_flag")
    if not bool(replay.get("ok", False)):
        blockers.append("config_adapter_replay_failed")
    if not bool(replay.get("report_only", False)):
        blockers.append("report_only_missing")
    if not bool(replay.get("dry_run_only", False)):
        blockers.append("dry_run_boundary_missing")
    if not bool(replay.get("request_normalization_preserved", False)):
        blockers.append("request_normalization_not_preserved")
    for name in REQUIRED_FIELD_NAMES:
        if name not in expected_fields:
            blockers.append(f"expected_field_missing:{name}")
        if name not in parsed:
            blockers.append(f"parsed_field_missing:{name}")
    if str(parsed.get("dit_compute_reducer_policy") or "") not in {"manual_selected", "quality_gated", "ab_passed"}:
        blockers.append("dit_compute_reducer_policy_not_preserved")
    if not selected:
        blockers.append("dit_compute_reducer_selected_not_preserved")
    if not thresholds:
        blockers.append("dit_compute_reducer_thresholds_not_preserved")
    if not str(parsed.get("dit_compute_reducer_ab_contract") or "").strip():
        blockers.append("dit_compute_reducer_ab_contract_not_preserved")

    ready = not blockers
    return {
        "schema_version": 1,
        "scorecard": "dit_compute_reducer_request_field_config_adapter_replay_v0",
        "ok": ready,
        "config_adapter_replay_ready": ready,
        "request_normalization_preserved": ready,
        "request_fields_emitted": False,
        "request_adapter_registered": False,
        "trainer_wiring_allowed": False,
        "runtime_activation_enabled": False,
        "training_launch_allowed": False,
        "runs_dispatched": False,
        "ab_dispatch_allowed": False,
        "ab_execution_allowed": False,
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "promotion_ready": False,
        "default_enable_allowed": False,
        "default_rollout_allowed": False,
        "auto_rollout_allowed": False,
        "parsed_field_keys": sorted(parsed.keys()),
        "parsed_reducer_count": len(selected),
        "parsed_threshold_keys": sorted(thresholds.keys()),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "prepare request-adapter registration boundary while keeping DiT compute reducers default-off"
            if ready
            else "fix DiT compute-reducer request normalization replay before request-adapter registration"
        ),
    }


def _field_names(value: Any) -> tuple[str, ...]:
    if isinstance(value, str):
        return tuple(item.strip() for item in value.split(",") if item.strip())
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes)):
        return tuple(str(item).strip() for item in value if str(item).strip())
    return ()


def _items(value: Any) -> tuple[str, ...]:
    if isinstance(value, str):
        return tuple(item.strip() for item in value.split(",") if item.strip())
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes)):
        return tuple(str(item).strip() for item in value if str(item).strip())
    return ()


def _unsafe_flags(*payloads: Mapping[str, Any]) -> bool:
    unsafe_keys = (
        "training_path_enabled",
        "default_behavior_changed",
        "promotion_ready",
        "default_enable_allowed",
        "request_fields_emitted",
        "request_adapter_registered",
        "trainer_wiring_allowed",
        "runtime_activation_enabled",
        "runtime_activation_allowed",
        "ab_dispatch_allowed",
        "ab_execution_allowed",
        "training_launch_allowed",
        "runs_dispatched",
        "default_rollout_allowed",
        "auto_rollout_allowed",
    )
    return any(bool(payload.get(key, False)) for payload in payloads for key in unsafe_keys)


__all__ = ["build_dit_compute_reducer_request_field_config_adapter_replay"]
