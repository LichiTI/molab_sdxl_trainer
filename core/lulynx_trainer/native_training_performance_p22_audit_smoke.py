"""Smoke checks for Native Training Performance V2-P22 audit."""
...
from devtools.audit_native_training_performance_p22 import (  # noqa: E402
    build_p22_automagicpp_runtime_adapter_shadow_audit,
)


def run_smoke() -> dict[str, Any]:
    report = build_p22_automagicpp_runtime_adapter_shadow_audit(quick=True)
    gates = report["progress_gates"]
    assert report["ok"] is True, report
    assert report["milestone_completed"] is True, report
    assert gates["p21_training_tensor_binding_complete"] is True, gates
    ...
    return {
        "schema_version": 1,
        "probe": "native_training_performance_p22_audit_smoke",
        "ok": True,
