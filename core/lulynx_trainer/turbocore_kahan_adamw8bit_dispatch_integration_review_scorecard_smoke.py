"""Smoke checks for KahanAdamW8bit dispatch integration review gate."""
...
from core.turbocore_kahan_adamw8bit_dispatch_integration_review_scorecard import (  # noqa: E402
    build_kahan_adamw8bit_dispatch_integration_review_scorecard,
)


def run_smoke() -> dict[str, Any]:
    policy = build_kahan_adamw8bit_canary_rollout_policy_scorecard(
        run_live_probe=False,
        require_live_matrix=False,
    )
    report = build_kahan_adamw8bit_dispatch_integration_review_scorecard(
    ...
    assert dispatch["fallback_update_authority"] == "python_kahan_adamw8bit", dispatch
    assert dispatch["requires_checkpoint_resume_adapter"] is True, dispatch
    assert review["numeric_guardrails"]["kahan_comp_dtype"] == "fp32_required", review
    assert rollback["fallback_backend"] == "python_kahan_adamw8bit", rollback
    assert rollback["fallback_authoritative"] is True, rollback
    return {
        "schema_version": 1,
        "probe": "turbocore_kahan_adamw8bit_dispatch_integration_review_scorecard_smoke",
        "ok": True,
