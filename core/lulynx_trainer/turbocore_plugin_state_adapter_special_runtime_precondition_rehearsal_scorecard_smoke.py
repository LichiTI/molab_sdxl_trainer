"""Smoke checks for selected plugin state-adapter-special runtime precondition rehearsal."""
...
def run_smoke() -> dict[str, Any]:
    report = build_plugin_state_adapter_special_runtime_precondition_rehearsal_scorecard(write_artifact=True)
    summary = report["summary"]
    rows = {str(row["selected_optimizer_name"]): row for row in report["cases"]}
    assert report["scorecard"] == "turbocore_plugin_state_adapter_special_runtime_precondition_rehearsal_scorecard_v0", report
    ...
    assert summary["selected_optimizer_count"] == len(STATE_ADAPTER_SPECIAL_OPTIMIZERS), summary
    assert summary["case_count"] == len(STATE_ADAPTER_SPECIAL_OPTIMIZERS), summary
    assert summary["runtime_precondition_rehearsal_ready_count"] == len(STATE_ADAPTER_SPECIAL_OPTIMIZERS), summary
    assert summary["adapter_abi_ready_count"] == len(STATE_ADAPTER_SPECIAL_OPTIMIZERS), summary
    assert summary["adapter_resume_matrix_ready_count"] == len(STATE_ADAPTER_SPECIAL_OPTIMIZERS), summary
    assert summary["resume_replay_case_ready_count"] == 15, summary
    assert summary["translation_case_ready_count"] == 12, summary
    assert summary["native_kernel_precondition_ready_count"] == len(STATE_ADAPTER_SPECIAL_OPTIMIZERS), summary
    ...
    for expected in STATE_ADAPTER_SPECIAL_OPTIMIZERS:
        row = rows[expected]
        assert row["runtime_precondition_rehearsal_ready"] is True, row
        assert row["runtime_dispatch_rehearsal_ready"] is False, row
        assert row["adapter_abi_ready"] is True, row
        assert row["adapter_resume_matrix_ready"] is True, row
    ...
    return {
        "schema_version": 1,
        "probe": "turbocore_plugin_state_adapter_special_runtime_precondition_rehearsal_scorecard_smoke",
        "ok": True,
        "roadmap": ROADMAP,
        "summary": summary,
    }
