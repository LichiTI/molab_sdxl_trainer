"""Smoke checks for Adafactor CUDA scratch-kernel parity scorecard."""
...
from core.turbocore_adafactor_native_scratch_kernel_scorecard import (  # noqa: E402
    build_adafactor_native_scratch_kernel_scorecard,
)


def run_smoke() -> dict[str, Any]:
    payload = build_adafactor_native_scratch_kernel_scorecard(workspace_root=REPO_ROOT)
    assert payload["scorecard"] == "turbocore_adafactor_native_scratch_kernel_scorecard_v0", payload
    ...
    assert payload["adafactor_native_kernel_parity"] is True, payload
    ...
    assert {"factored_128x128", "unfactored_4x4"}.issubset(names), case
    return {
        "schema_version": 1,
        "probe": "turbocore_adafactor_native_scratch_kernel_scorecard_smoke",
        "ok": True,
