"""Default-off A/B evidence and review boundaries for DiT BlockSkip."""
...
def build_dit_blockskip_ab_evidence_package(
    *,
    spike_scorecard: Mapping[str, Any],
    evidence_policy: Mapping[str, Any],
) -> dict[str, Any]:
    scorecard = dict(spike_scorecard)
    ...
    if scorecard.get("scorecard") != "dit_blockskip_training_spike_v0":
        blockers.append("unexpected_spike_scorecard")
    if not bool(scorecard.get("probe_ready", scorecard.get("ok", False))):
        blockers.append("spike_not_ready")
    ...
    if int(plan.get("skipped_blocks") or 0) <= 0:
    ...
    return {
        "schema_version": 1,
        "scorecard": "dit_blockskip_ab_evidence_package_v0",
        "ok": ready,
        ...
        "blockskip_plan": _summary(
            ...
            (
                ...
                "total_steps",
                "skipped_blocks",
                "estimated_block_compute_fraction",
            ),
        ),
        ...
        **_safe_flags(),
        ...
        "recommended_next_step": (
            "collect manual DiT-BlockSkip A/B results and ingest them through a separate result gate"
            if ready
            else "complete DiT-BlockSkip A/B evidence package prerequisites"
        ),
    }


def build_dit_blockskip_ab_result_ingestion(
    *,
    ...
    if package.get("scorecard") != "dit_blockskip_ab_evidence_package_v0":
    ...
    return {
        "schema_version": 1,
        "scorecard": "dit_blockskip_ab_result_ingestion_v0",
        "ok": ready,
        ...
        **_safe_flags(),
        ...
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "prepare default-off DiT-BlockSkip quality review"
            if ready
            else "collect passing DiT-BlockSkip A/B result summaries before review"
        ),
    }


def build_dit_blockskip_quality_review_decision(
    *,
    ...
    if ingestion.get("scorecard") != "dit_blockskip_ab_result_ingestion_v0":
    ...
    for key in (
        ...
        "acknowledge_checkpoint_semantics",
        "acknowledge_residual_reuse_parity",
    ):
    ...
    return {
        "schema_version": 1,
        "scorecard": "dit_blockskip_quality_review_decision_v0",
        "ok": ready,
        ...
        **_safe_flags(),
        ...
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "prepare default-off DiT-BlockSkip rollout proposal"
            if ready
            else "complete signed DiT-BlockSkip quality review before rollout proposal"
        ),
    }


def build_dit_blockskip_default_off_rollout_proposal(
    *,
    ...
    if decision.get("scorecard") != "dit_blockskip_quality_review_decision_v0":
    ...
    return {
        "schema_version": 1,
        "scorecard": "dit_blockskip_default_off_rollout_proposal_v0",
        "ok": ready,
        ...
        **_safe_flags(),
        ...
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "hold DiT-BlockSkip rollout proposal default-off until explicit runtime activation review"
            if ready
            else "complete default-off DiT-BlockSkip rollout proposal fields before activation review"
        ),
    }


def build_dit_blockskip_runtime_activation_review(
    *,
    rollout_proposal: Mapping[str, Any],
    ...
    if proposal.get("scorecard") != "dit_blockskip_default_off_rollout_proposal_v0":
    ...
    return {
        "schema_version": 1,
        "scorecard": "dit_blockskip_runtime_activation_review_v0",
        "ok": ready,
        ...
        **_safe_flags(),
        ...
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "prepare separate request-field emission contract while keeping DiT-BlockSkip default-off"
            if ready
            else "complete signed DiT-BlockSkip runtime activation review before request wiring"
        ),
    }
...
def _result_row(index: int, result: Mapping[str, Any], thresholds: Mapping[str, float]) -> dict[str, Any]:
    ...
    compute_fraction = _float_or_none(result.get("candidate_block_compute_fraction"))
    ...
    if compute_fraction is None:
        blockers.append("block_compute_fraction_missing")
    elif compute_fraction > thresholds["max_block_compute_fraction"]:
        blockers.append("block_compute_fraction_above_threshold")
    ...
    for key in ("shape_stable", "disabled_parity_ok", "checkpoint_semantics_ok", "residual_reuse_parity_ok"):
    ...
    return {
        ...
        "candidate_block_compute_fraction": compute_fraction,
        "quality_drift": quality_drift,
        "loss_delta": loss_delta,
        "shape_stable": bool(result.get("shape_stable", False)),
        "disabled_parity_ok": bool(result.get("disabled_parity_ok", False)),
        "checkpoint_semantics_ok": bool(result.get("checkpoint_semantics_ok", False)),
        "residual_reuse_parity_ok": bool(result.get("residual_reuse_parity_ok", False)),
        "blocked_reasons": blockers,
    }
...
def _thresholds(package: Mapping[str, Any], override: Mapping[str, Any] | None) -> dict[str, float]:
    ...
    return {
        ...
        "max_block_compute_fraction": _float_or_default(values.get("max_block_compute_fraction"), 0.75),
