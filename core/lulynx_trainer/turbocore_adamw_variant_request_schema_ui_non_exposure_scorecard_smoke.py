"""Smoke checks for AdamW variant request/schema/UI non-exposure evidence."""
...
def run_smoke() -> dict[str, Any]:
    hold = build_adamw_variant_owner_release_hold_scorecard(workspace_root=REPO_ROOT)
    payload = build_adamw_variant_request_schema_ui_non_exposure_scorecard(
    ...
    assert payload["scorecard"] == "turbocore_adamw_variant_request_schema_ui_non_exposure_scorecard_v0", payload
    ...
    assert summary["optimizer_count"] == 6, summary
    ...
    return {
        "schema_version": 1,
        "probe": "turbocore_adamw_variant_request_schema_ui_non_exposure_scorecard_smoke",
        "ok": True,
...
def _write_real_artifact(payload: dict[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_adamw_variant_request_schema_ui_non_exposure_scorecard.json"
