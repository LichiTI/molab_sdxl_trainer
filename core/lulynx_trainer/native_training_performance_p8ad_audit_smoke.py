"""Smoke checks for Native Training Performance V2-P8AD audit."""
...
from devtools.audit_native_training_performance_p8ad import (  # noqa: E402
    build_p8ad_kahan_adamw8bit_dispatch_review_audit,
)


def run_smoke() -> dict[str, Any]:
    report = build_p8ad_kahan_adamw8bit_dispatch_review_audit(quick=True)
    gates = report["progress_gates"]
    assert report["ok"] is True, report
    assert report["milestone_completed"] is True, report
    assert gates["p8ac_rollout_policy_complete"] is True, gates
    ...
    section = report["sections"]["kahan_adamw8bit_training_loop_canary"]
    assert section["ok"] is True, section
    assert section["summary"]["native_step_count"] == 1, section
    assert section["summary"]["native_kernel_launch_count"] == 1, section
    assert section["training_path_enabled"] is False, section
    assert section["default_behavior_changed"] is False, section
    assert report["remaining_blockers"] == [], report
    return {
        "schema_version": 1,
        "probe": "native_training_performance_p8ad_audit_smoke",
        "ok": True,
