"""Smoke checks for selected plugin fused-backward runtime precondition rehearsal."""
...
def run_smoke() -> dict[str, Any]:
    report = build_plugin_fused_backward_runtime_precondition_rehearsal_scorecard(write_artifact=True)
    summary = report["summary"]
    rows = {str(row["selected_optimizer_name"]): row for row in report["cases"]}
    assert report["scorecard"] == "turbocore_plugin_fused_backward_runtime_precondition_rehearsal_scorecard_v0", report
    ...
    assert summary["fused_backward_abi_ready_count"] == len(TARGET_PLUGIN_OPTIMIZERS), summary
    assert summary["resume_parity_matrix_ready_count"] == len(TARGET_PLUGIN_OPTIMIZERS), summary
    assert summary["replay_case_ready_count"] == 10, summary
    ...
    for expected in TARGET_PLUGIN_OPTIMIZERS:
        ...
        assert row["fused_backward_abi_ready"] is True, row
        assert row["resume_parity_matrix_ready"] is True, row
        assert row["native_kernel_precondition_ready"] is True, row
        assert row["requires_backward_hook_ownership_token"] is True, row
    ...
    return {
        "schema_version": 1,
        "probe": "turbocore_plugin_fused_backward_runtime_precondition_rehearsal_scorecard_smoke",
        "ok": True,
        "roadmap": ROADMAP,
        "summary": summary,
    }
