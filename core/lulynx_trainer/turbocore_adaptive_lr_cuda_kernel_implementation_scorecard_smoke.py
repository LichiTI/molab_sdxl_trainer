"""Smoke checks for adaptive-LR CUDA scratch-kernel implementation evidence."""
...
def run_smoke() -> dict[str, Any]:
    contract = build_adaptive_lr_cuda_kernel_contract_plan_scorecard()
    payload = build_adaptive_lr_cuda_kernel_implementation_scorecard(
    ...
    assert payload["scorecard"] == "turbocore_adaptive_lr_cuda_kernel_implementation_scorecard_v0", payload
    assert payload["ok"] is True, payload
    assert payload["promotion_ready"] is False, payload
    assert payload["cuda_kernel_implementation_ready"] is True, payload
    ...
    assert summary["cuda_kernel_implementation_ready_count"] == len(TARGET_CASES), summary
    assert summary["state_machine_abi_implementation_ready_count"] == len(TARGET_CASES), summary
    assert summary["native_kernel_preconditions_implementation_ready_count"] == len(TARGET_CASES), summary
    assert summary["kernel_executed_count"] == len(TARGET_CASES), summary
    assert summary["contract_plan_ready_count"] == len(TARGET_CASES), summary
    ...
    for case in TARGET_CASES:
        ...
        assert row["state_machine_status"] == "cuda_kernel_implementation_ready", row
        ...
        assert row["reduce_kernel_executed"] is True, row
        assert row["apply_kernel_executed"] is True, row
        assert row["parameters_mutated"] is True, row
    ...
    return {
        "schema_version": 1,
        "probe": "turbocore_adaptive_lr_cuda_kernel_implementation_scorecard_smoke",
        "ok": True,
...
def _write_real_artifact(payload: dict[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_adaptive_lr_cuda_kernel_implementation_scorecard.json"
