"""Runtime activation review boundary for profiled adapter target proposals."""
...
EXPECTED_SCOPE = "adapter_target_runtime_activation_review"


def build_adapter_target_runtime_activation_review(
    *,
    ...
    if proposal.get("scorecard") != "adapter_target_default_off_rollout_proposal_v0":
    ...
    return {
        "schema_version": 1,
        "scorecard": "adapter_target_runtime_activation_review_v0",
        "ok": ready,
        ...
        "selected_count": int(proposal.get("selected_count") or 0),
        "review": _summary(
            review,
            (
                "signed_review_id",
        ...
        "recommended_next_step": (
            "prepare separate request-field emission contract while keeping profiled adapter targets default-off"
            if ready
...
__all__ = ["build_adapter_target_runtime_activation_review"]
