"""Request submission boundary for T-LoRA A/B request-adapter plans."""
...
REQUIRED_PAYLOAD_FIELDS = ("tlora_ab_case_id", "tlora_ab_arm", "adapter_type", "tlora_rank_schedule")


def build_tlora_ab_request_submission_boundary(
    *,
    ...
    if boundary.get("scorecard") != "tlora_ab_request_adapter_registration_boundary_v0":
    ...
    return {
        "schema_version": 1,
        "scorecard": "tlora_ab_request_submission_boundary_v0",
        "ok": ready,
        ...
        "recommended_next_step": (
            "prepare execution-job creation boundary while keeping T-LoRA request submission disabled"
            if ready
            else "complete default-off T-LoRA request submission boundary"
        ),
    }
...
__all__ = ["build_tlora_ab_request_submission_boundary"]
