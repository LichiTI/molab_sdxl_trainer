"""Smoke checks for built-in simple schedule-free dispatch review scorecard."""
...
def run_smoke() -> dict[str, Any]:
    rollout = build_simple_optimizer_schedulefree_rollout_policy_scorecard()
    assert rollout["canary_rollout_policy_ready"] is True, rollout
    payload = build_simple_optimizer_schedulefree_dispatch_integration_review_scorecard(
    ...
    assert payload["scorecard"] == "turbocore_simple_optimizer_schedulefree_dispatch_integration_review_scorecard_v0", payload
    ...
    assert review["optimizer_types"] == ["RAdamScheduleFree", "SGDScheduleFree"], review
    assert review["allowed_initial_modes"] == ["off", "observe"], review
    assert review["blocked_modes_until_review"] == ["canary", "auto"], review
    assert hooks["optimizer_train_mode_hook"].endswith(".train"), hooks
    assert hooks["optimizer_eval_mode_hook"].endswith(".eval"), hooks
    ...
    return {
        "schema_version": 1,
        "probe": "turbocore_simple_optimizer_schedulefree_dispatch_integration_review_scorecard_smoke",
        "ok": True,
...
def _write_real_artifact(payload: dict[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_simple_optimizer_schedulefree_dispatch_integration_review_scorecard.json"
