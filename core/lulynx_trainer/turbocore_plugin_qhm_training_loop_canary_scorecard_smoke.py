"""Smoke checks for selected plugin QHM TrainingLoop native canary."""
...
from core.turbocore_plugin_qhm_training_loop_canary_scorecard import (  # noqa: E402
    build_plugin_qhm_training_loop_canary_scorecard,
)


def run_smoke() -> dict[str, Any]:
    report = build_plugin_qhm_training_loop_canary_scorecard()
    assert report["scorecard"] == "turbocore_plugin_qhm_training_loop_canary_scorecard_v0", report
    assert report["training_path_enabled"] is False, report
    assert report["default_behavior_changed"] is False, report
    assert report["runtime_dispatch_ready"] is False, report
    assert report["native_dispatch_allowed"] is False, report
    assert report["selected_optimizer_name"] == "qhm", report
    assert report["optimizer_family"] == "simple_formula", report
    assert report["selected_native_canary_ready"] is True, report
    assert report["ok"] is True, report
    case = report["case"]
    assert case["optimizer_class"] == "QHM", case
    ...
    assert case["executor_optimizer_kind"] == "qhm", case
    assert "momentum_buffer" in case["state_keys"], case
    return {
        "schema_version": 1,
        "probe": "turbocore_plugin_qhm_training_loop_canary_scorecard_smoke",
        "ok": True,
