"""CUDA scratch-kernel parity scorecard for Adafactor."""
...
ENTRYPOINT = "probe_adafactor_cuda_scratch_launch_py"
REPO_ROOT = Path(__file__).resolve().parents[2]


def build_adafactor_native_scratch_kernel_scorecard(
    *,
    ...
    if native is None:
        return _blocked("adafactor_scratch_probe_entrypoint_missing")
    ...
    return {
        "schema_version": 1,
        "scorecard": "turbocore_adafactor_native_scratch_kernel_scorecard_v0",
        "gate": "adafactor_native_scratch_kernel_parity",
        "ok": ready,
        "promotion_ready": False,
        "kernel_parity_stage_ready": ready,
        "adafactor_native_kernel_parity": ready,
        ...
        "promotion_blockers": [] if ready else ["adafactor_native_kernel_parity_missing"],
        "blocked_reasons": _case_blockers(case),
        "recommended_next_step": (
            "extend Adafactor native kernel to live tensor binding canary"
            if ready
            else "fix Adafactor scratch CUDA kernel parity"
        ),
    }
...
def _run_case(native: Any, workspace_root: str, arch: str) -> dict[str, Any]:
    try:
        probe = native.probe_adafactor_cuda_scratch_launch_py(workspace_root, arch)
        ...
        return {
            ...
            "hard_failure": True,
            "reason": f"adafactor_cuda_probe_failed:{type(exc).__name__}: {exc}",
            "kernel_executed": False,
            "native_kernel_parity_ready": False,
            "training_path_enabled": False,
            "native_dispatch_allowed": False,
            "blocked_reasons": ["adafactor_native_kernel_parity_missing"],
        }
    ...
    parity_ready = (
        ...
        and _has_case(cases, "factored_128x128")
    ...
    return {
        ...
        "kernel_name": str(probe.get("kernel_name", "adafactor_flat_fp32_cuda_v0")),
        ...
        "cases": cases,
        "probe": probe,
        "blocked_reasons": [] if parity_ready else ["adafactor_native_kernel_parity_missing"],
    }
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "adafactor_native_kernel_parity": False,
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "native_dispatch_allowed": False,
        ...
        "promotion_blockers": [reason, "adafactor_native_kernel_parity_missing"],
        "blocked_reasons": [reason],
        "recommended_next_step": "build or load lulynx_native with Adafactor scratch probe entrypoint",
    }


__all__ = ["build_adafactor_native_scratch_kernel_scorecard"]
