"""Default-off SimplifiedAdEMAMix Triton TrainingLoop executor for selected plugin canaries."""
...
class SimplifiedAdEMAMixTrainingExecutorConfig:
    optimizer_kind: str = "simplifiedademamix"
    lr: float = 1.0e-4
    beta1: float = 0.99
    beta2: float = 0.95
    alpha: float = 0.0
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _simplified_ademamix_dense_kernel(
        param_ptr,
        grad_ptr,
        exp_avg_ptr,
        exp_avg_sq_ptr,
        ...
        denom = tl.sqrt(v_new) + eps_scaled
        update = ((g * alpha) + m_new) / denom / sqrt_den_sum
        p_new = p - lr * update
...
class SimplifiedAdEMAMixTrainingExecutor:
    """Launch a real SimplifiedAdEMAMix fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: SimplifiedAdEMAMixTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("SimplifiedAdEMAMixTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("simplifiedademamix_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("simplifiedademamix_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("simplifiedademamix_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"simplifiedademamix_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("simplifiedademamix_maximize_not_supported")
        ...
        for group in self.optimizer.param_groups:
            ...
            for param in group["params"]:
                if param.grad is None:
                    continue
                cases.append(self._step_param(param, group_cfg))
        blockers = _dedupe(reason for case in cases for reason in case.get("blocked_reasons", []) or [])
        if not cases:
            blockers.append("simplifiedademamix_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_simplifiedademamix_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "simplifiedademamix_native_step_failed"),
    ...
    def _step_param(self, param: torch.nn.Parameter, group_cfg: Mapping[str, Any]) -> dict[str, Any]:
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("simplifiedademamix_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("simplifiedademamix_training_executor_requires_contiguous_tensors")
        ...
        num_sum = beta1 * float(state.get("num_sum", 0.0) or 0.0) + 1.0
        den_sum = beta2 * float(state.get("den_sum", 0.0) or 0.0) + (1.0 - beta2)
        sqrt_den_sum = math.sqrt(den_sum)
        if sqrt_den_sum == 0.0:
            return _case_failed("simplifiedademamix_den_sum_invalid")
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _simplified_ademamix_dense_kernel[grid](
            param,
            param.grad.detach(),
            exp_avg,
            exp_avg_sq,
            ...
            float(sqrt_den_sum * float(group_cfg["eps"])),
            float(sqrt_den_sum),
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["simplifiedademamix_training_executor_parameters_not_mutated"],
        }


def build_plugin_simplifiedademamix_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: SimplifiedAdEMAMixTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> SimplifiedAdEMAMixTrainingExecutor:
    return SimplifiedAdEMAMixTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: SimplifiedAdEMAMixTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> SimplifiedAdEMAMixTrainingExecutorConfig:
    if isinstance(value, SimplifiedAdEMAMixTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.99, 0.95)))
    beta1_warmup = payload.get("beta1_warmup", group.get("beta1_warmup", None))
    return SimplifiedAdEMAMixTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "simplifiedademamix"),
        lr=float(payload.get("lr", group.get("lr", 1.0e-4)) or 1.0e-4),
        beta1=float(betas[0]),
        beta2=float(betas[1]),
        alpha=float(payload.get("alpha", group.get("alpha", 0.0)) or 0.0),
        beta1_warmup=None if beta1_warmup is None else int(beta1_warmup),
        min_beta1=float(payload.get("min_beta1", group.get("min_beta1", 0.9)) or 0.9),
...
def _group_config(group: Mapping[str, Any], config: SimplifiedAdEMAMixTrainingExecutorConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "beta1_warmup": group.get("beta1_warmup", config.beta1_warmup),
        "min_beta1": float(group.get("min_beta1", config.min_beta1) or config.min_beta1),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if group_cfg.get("beta1_warmup") is not None:
        blockers.append("simplifiedademamix_beta1_warmup_not_supported_for_canary")
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("simplifiedademamix_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("fixed_decay", False)):
        blockers.append("simplifiedademamix_fixed_decay_not_supported_for_canary")
    return blockers
