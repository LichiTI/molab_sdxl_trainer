"""Operator activation execution contract boundary for TurboCore V5-P46.

P46 records a signed boundary review for future operator activation execution
after P45. It does not execute activation requests, enable runtime/native
...
P46_BLOCKED_DECISION = "operator_activation_execution_contract_boundary_blocked_default_off"
P46_HOLD_DECISION = "operator_activation_execution_contract_boundary_hold_for_signed_review_default_off"
P46_REJECTED_DECISION = "operator_activation_execution_contract_boundary_rejected_default_off"
P46_SCOPE = "operator_activation_execution_contract_boundary"
...
def build_v5_operator_activation_execution_contract_boundary(
    *,
    p45_operator_activation_request_boundary: Mapping[str, Any] | None = None,
    operator_activation_execution_evidence: Mapping[str, Any] | None = None,
    operator_activation_execution_review: Mapping[str, Any] | None = None,
    failure_history: Sequence[Any] | None = None,
    rollback_history: Sequence[Any] | None = None,
) -> dict[str, Any]:
    """Record operator activation execution evidence without executing it."""

    p45 = _as_dict(p45_operator_activation_request_boundary)
    evidence = _as_dict(operator_activation_execution_evidence)
    review = _as_dict(operator_activation_execution_review)
    failure_events = _event_list(failure_history)
    rollback_events = _event_list(rollback_history)
    p45_summary = _p45_summary(p45)
    evidence_summary = _evidence_summary(evidence)
    review_summary = _review_summary(review)
    evidence_blockers = _evidence_blockers(p45_summary, evidence_summary, failure_events, rollback_events)
    ...
    if decision == P46_HOLD_DECISION:
        blockers.append("v5_p46_signed_operator_activation_execution_review_missing")
    blockers = _dedupe(blockers)
    ready = decision == P46_READY_DECISION
    rejected = decision == P46_REJECTED_DECISION
    decision_record_ready = ready or rejected
    return {
        "schema_version": 1,
        "package": "turbocore_v5_operator_activation_execution_contract_boundary_v0",
        "gate": "v5_operator_activation_execution_contract_boundary",
        "ok": decision_record_ready,
        "decision_record_ready": decision_record_ready,
        "operator_activation_execution_review_recorded": decision_record_ready,
        "operator_activation_execution_review_signed": decision_record_ready,
        "operator_activation_execution_contract_boundary_ready": ready,
        "operator_activation_execution_evidence_recorded": ready,
        ...
        **{field: False for field in UNSAFE_TRUE_FIELDS},
        "post_p46_request_fields": {},
        "p45_operator_activation_request_boundary_summary": p45_summary,
        "operator_activation_execution_evidence_summary": evidence_summary,
        "operator_activation_execution_review": review_summary,
        "operator_activation_execution_review_template": _review_template(evidence_summary),
        "progress_gates": _progress_gates(p45_summary, evidence_summary, review_summary),
        ...
        "notes": [
            "P46 records operator activation execution boundary evidence only.",
            "P46 does not execute activation requests, enable runtime/native dispatch, emit request fields, or launch training.",
            "A later explicit runtime enablement contract is still required before execution can become active.",
        ],
    }


def _p45_summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "operator_activation_request_boundary_ready": report.get("operator_activation_request_boundary_ready") is True,
        "operator_activation_request_evidence_recorded": report.get("operator_activation_request_evidence_recorded")
        is True,
        "operator_activation_request_review_signed": report.get("operator_activation_request_review_signed") is True,
        "decision": decision,
        ...
        "operator_activation_request_submitted": report.get("operator_activation_request_submitted"),
        "operator_activation_requested": report.get("operator_activation_requested"),
...
def _evidence_summary(evidence: Mapping[str, Any]) -> dict[str, Any]:
    ...
    execution_rows = _rows(evidence, "operator_activation_execution_plan_inventory", "execution_plan_inventory")
    precondition_rows = _rows(evidence, "execution_precondition_inventory", "precondition_inventory")
    blockers = _evidence_blocker_list(evidence, missing_sections, execution_rows, precondition_rows)
    return {
        ...
        "ready": not blockers,
        "operator_activation_execution_contract_boundary_ready": evidence.get(
        ...
        "requires_later_runtime_enablement_contract": evidence.get("requires_later_runtime_enablement_contract") is True,
        ...
        "unsafe_claims": _unsafe_claims(evidence, "operator_activation_execution"),
        "blockers": blockers,
    }
...
def _review_summary(review: Mapping[str, Any]) -> dict[str, Any]:
    summary = {
        ...
        "approve_operator_activation_execution_boundary": review.get("approve_operator_activation_execution_boundary")
        is True,
    }
...
def _progress_gates(p45_summary: Mapping[str, Any], evidence_summary: Mapping[str, Any], review_summary: Mapping[str, Any]) -> dict[str, bool]:
    return {
        "p45_operator_request_boundary_present": bool(p45_summary.get("present", False)),
        "p45_operator_request_boundary_ready": _p45_ready(p45_summary),
        "operator_activation_execution_evidence_present": bool(evidence_summary.get("present", False)),
        "operator_activation_execution_evidence_ready": bool(evidence_summary.get("ready", False)),
        "signed_operator_activation_execution_review_present": bool(review_summary.get("present", False)),
        "reviewer_present": bool(review_summary.get("reviewer")),
        "reviewed_at_present": bool(review_summary.get("reviewed_at")),
        "requested_scope_valid": review_summary.get("requested_scope") == P46_SCOPE,
...
def _decision(evidence_blockers: list[str], review: Mapping[str, Any], review_blockers: list[str]) -> str:
    if evidence_blockers:
        return P46_BLOCKED_DECISION
    if not review:
        return P46_HOLD_DECISION
    if review_blockers:
        return P46_BLOCKED_DECISION
    if review.get("approve_operator_activation_execution_boundary") is True:
        return P46_READY_DECISION
    return P46_REJECTED_DECISION


def _evidence_blockers(
    p45_summary: Mapping[str, Any],
    evidence_summary: Mapping[str, Any],
    failure_events: list[str],
    rollback_events: list[str],
) -> list[str]:
    blocked: list[str] = []
    if not bool(p45_summary.get("present", False)):
        blocked.append("v5_p46_p45_operator_activation_request_boundary_missing")
    elif not _p45_ready(p45_summary):
        blocked.append("v5_p46_p45_operator_activation_request_boundary_not_ready")
        blocked.extend(_string_list(p45_summary.get("blocked_reasons")))
        blocked.extend(_string_list(p45_summary.get("promotion_blockers")))
        blocked.extend(_string_list(p45_summary.get("unsafe_claims")))
    if not bool(evidence_summary.get("present", False)):
        blocked.append("v5_p46_operator_activation_execution_evidence_missing")
    elif not bool(evidence_summary.get("ready", False)):
        blocked.extend(_string_list(evidence_summary.get("blockers")))
    for event in failure_events:
        blocked.append(f"v5_p46_failure_history_not_clear:{event}")
    for event in rollback_events:
        blocked.append(f"v5_p46_rollback_history_not_clear:{event}")
...
def _review_blockers(review: Mapping[str, Any]) -> list[str]:
    ...
    if not review.get("reviewer"):
        blocked.append("v5_p46_reviewer_missing")
    if not review.get("reviewed_at"):
        blocked.append("v5_p46_reviewed_at_missing")
    if review.get("requested_scope") != P46_SCOPE:
        blocked.append("v5_p46_requested_scope_invalid")
    for field in UNSAFE_REVIEW_APPROVAL_FIELDS:
        if review.get(field):
            blocked.append(f"v5_p46_unsafe_review_approval:{field}")
    for field in REQUIRED_REVIEW_ACKS:
        if not review.get(field):
            blocked.append(f"v5_p46_review_ack_missing:{field}")
    return _dedupe(blocked)


def _p45_ready(summary: Mapping[str, Any]) -> bool:
    return bool(
        summary.get("present")
        and summary.get("ok")
        and summary.get("decision_record_ready")
        and summary.get("operator_activation_request_boundary_ready")
        and summary.get("operator_activation_request_evidence_recorded")
        and summary.get("operator_activation_request_review_signed")
        and summary.get("decision") == P45_READY_DECISION
        ...
        and summary.get("operator_activation_request_submitted") is False
        and summary.get("operator_activation_requested") is False
...
def _evidence_blocker_list(
    ...
    if evidence.get("ok") is not True:
        blocked.append("v5_p46_operator_execution_not_ok")
    if evidence.get("operator_activation_execution_contract_boundary_ready") is not True:
        blocked.append("v5_p46_operator_execution_evidence_not_ready")
    for field in (
        "report_only",
        "boundary_only",
        "execution_only",
        ...
        "manual_only",
        "internal_only",
        "requires_later_runtime_enablement_contract",
        "requires_explicit_owner_approval",
        "requires_explicit_operator_opt_in",
    ):
        if evidence.get(field) is not True:
            blocked.append(f"v5_p46_operator_execution_{field}_missing")
    if evidence.get("default_off") is not True or not _default_off_confirmed(evidence):
        blocked.append("v5_p46_operator_execution_default_off_violation")
    if evidence.get("request_adapter_off") is not True or not _request_adapter_off(evidence):
        blocked.append("v5_p46_operator_execution_request_adapter_violation")
    if not _digest(evidence):
        blocked.append("v5_p46_operator_execution_digest_missing")
    if not _source(evidence):
        blocked.append("v5_p46_operator_execution_source_missing")
    for section in missing_sections:
        blocked.append(f"v5_p46_operator_execution_section_missing:{section}")
    blocked.extend(_execution_plan_blockers(execution_rows))
    blocked.extend(_precondition_blockers(precondition_rows))
    blocked.extend(_unsafe_claims(evidence, "operator_activation_execution"))
...
def _execution_plan_blockers(rows: list[Mapping[str, Any]]) -> list[str]:
    blocked: list[str] = []
    if not rows:
        return ["v5_p46_operator_activation_execution_plan_inventory_missing"]
    for index, row in enumerate(rows):
        ...
        for field in (
            ...
            if row.get(field) is True:
                blocked.append(f"v5_p46_operator_execution_claim:{plan_id}:{field}")
...
def _precondition_blockers(rows: list[Mapping[str, Any]]) -> list[str]:
    blocked: list[str] = []
    if not rows:
        return ["v5_p46_execution_precondition_inventory_missing"]
    for index, row in enumerate(rows):
        ...
        for field in ("execution_precondition_active", "execution_check_registered", "execution_check_enabled"):
            if row.get(field) is True:
                blocked.append(f"v5_p46_execution_precondition_claim:{check_id}:{field}")
...
def _unsafe_claims(value: Mapping[str, Any], owner: str) -> list[str]:
    ...
    for field in UNSAFE_TRUE_FIELDS:
        if value.get(field) is True:
            blocked.append(f"v5_p46_unsafe_claim:{owner}:{field}")
...
def _review_template(evidence_summary: Mapping[str, Any]) -> dict[str, Any]:
    template = {"reviewer": "", "reviewed_at": "", "requested_scope": P46_SCOPE, "approve_operator_activation_execution_boundary": False}
    for field in UNSAFE_REVIEW_APPROVAL_FIELDS:
...
def _allowed_next_actions(decision: str, blockers: list[str]) -> list[str]:
    if decision == P46_READY_DECISION:
        return ["archive_p46_operator_activation_execution_contract_boundary"]
    if decision == P46_REJECTED_DECISION:
        return ["record_p46_default_off_rejection_or_repair_operator_execution_evidence"]
    if decision == P46_HOLD_DECISION:
        return ["collect_signed_operator_activation_execution_review"]
    if any("p45" in item for item in blockers):
        return ["repair_p45_operator_activation_request_boundary"]
    if any("operator_execution" in item or "precondition" in item for item in blockers):
        return ["repair_operator_activation_execution_evidence"]
    return ["clear_failure_or_rollback_history_before_operator_activation_execution_boundary"]


def _recommended_next_step(decision: str, blockers: list[str]) -> str:
    if decision == P46_READY_DECISION:
        return "archive P46 boundary; runtime enablement still requires a later explicit contract"
    if decision == P46_REJECTED_DECISION:
        return "record the signed rejection and keep operator activation execution default-off for repair"
    if decision == P46_HOLD_DECISION:
        return "collect a signed operator activation execution review over P45 evidence"
    if any("p45" in item for item in blockers):
        return "repair the P45 operator activation request boundary before P46"
    if any("digest_missing" in item or "source_missing" in item for item in blockers):
        return "collect replayable operator activation execution source and digest evidence"
    return "hold P46 until operator execution evidence, review acknowledgements, and histories are clear"


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build V5 P46 operator activation execution boundary.")
    parser.add_argument("--p45-operator-activation-request-boundary", default="", help="P45 boundary JSON.")
    parser.add_argument("--operator-activation-execution-evidence", default="", help="P46 execution evidence JSON.")
    parser.add_argument("--operator-activation-execution-review", default="", help="Signed P46 review JSON.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    report = build_v5_operator_activation_execution_contract_boundary(
        p45_operator_activation_request_boundary=load_json(args.p45_operator_activation_request_boundary)
        if args.p45_operator_activation_request_boundary
        else None,
        operator_activation_execution_evidence=load_json(args.operator_activation_execution_evidence)
        if args.operator_activation_execution_evidence
        else None,
        operator_activation_execution_review=load_json(args.operator_activation_execution_review)
        if args.operator_activation_execution_review
        else None,
