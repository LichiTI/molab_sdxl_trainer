"""TrainingLoop canary for Automagic++ native dispatch."""
...
def build_automagicpp_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_automagicpp_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = list(case.get("blocked_reasons", []) or [])
    return {
        "schema_version": 1,
        "scorecard": "turbocore_automagicpp_training_loop_canary_scorecard_v0",
        "gate": "automagicpp_training_loop_native_canary",
        "ok": ok,
        ...
        "recommended_next_step": (
            "add Automagic++ e2e shadow matrix and rollout policy"
            if ok
            else "fix Automagic++ TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "automagicpp_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["automagicpp_training_loop_did_not_emit_step"],
        }
    ...
    return {
        ...
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "blocked_reasons": [] if ok else _dedupe(list(runtime.get("blocked_reasons", []) or []) + ["automagicpp_training_loop_native_step_missing"]),
    }
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    loop = TrainingLoop(
        ...
        turbocore_native_update_quantized_optimizer_kind="automagicpp",
    )
    loop.total_steps = 1
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run Automagic++ TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_automagicpp_training_loop_canary_scorecard"]
