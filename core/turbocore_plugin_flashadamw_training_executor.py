"""Default-off training executor for selected plugin FlashAdamW native dispatch."""
...
ENTRYPOINT = "probe_flashadamw_training_tensor_binding_canary_py"
REPO_ROOT = Path(__file__).resolve().parents[2]


@dataclass(frozen=True)
class PluginFlashAdamWTrainingExecutorConfig:
    lr: float = 1e-3
    ...
    quantize: bool = False
    master_bytewidth: int = 0
    maximize: bool = False
    max_numel: int = 1_048_576
    require_native_cuda: bool = True


class PluginFlashAdamWTrainingExecutor:
    """Launch selected FlashAdamW native steps against unquantized live state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: PluginFlashAdamWTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("PluginFlashAdamWTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("plugin_flashadamw_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("plugin_flashadamw_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("plugin_flashadamw_training_dispatch_entrypoint_missing")
        ...
        if not cases:
            blockers.append("plugin_flashadamw_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_flashadamw_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "plugin_flashadamw_native_step_failed"),
            "optimizer_kind": "flashadamw",
    ...
    def _step_param(
        ...
        if type(self.optimizer).__name__.lower() != "flashadamw":
            return _case_failed("plugin_flashadamw_optimizer_not_flashadamw", "plugin_flashadamw_optimizer_class_unsupported")
        if bool(group_cfg.get("quantize", True)):
            return _case_failed("plugin_flashadamw_quantized_state_unsupported", "plugin_flashadamw_quantized_state_unsupported")
        if int(group_cfg.get("master_bytewidth", 0) or 0) != 0:
            return _case_failed("plugin_flashadamw_master_weight_bits_unsupported", "plugin_flashadamw_master_weight_bits_unsupported")
        if param.dtype != torch.float32 or param.grad is None or param.grad.dtype != torch.float32:
            return _case_failed("plugin_flashadamw_native_step_requires_float32", "plugin_flashadamw_native_step_dtype_unsupported")
        if not param.is_contiguous() or not param.grad.is_contiguous():
            return _case_failed("plugin_flashadamw_native_step_requires_contiguous", "plugin_flashadamw_native_step_layout_unsupported")
        state = self.optimizer.state[param]
        if not all(name in state for name in ("exp_avg", "exp_avg_sq")):
            return _case_failed("plugin_flashadamw_unquantized_state_missing", "plugin_flashadamw_unquantized_state_missing")
        exp_avg, exp_avg_sq = state["exp_avg"], state["exp_avg_sq"]
        state_tensors = (exp_avg, exp_avg_sq)
        if not all(torch.is_tensor(item) for item in state_tensors):
            return _case_failed("plugin_flashadamw_live_state_tensors_missing", "plugin_flashadamw_live_state_missing")
        if any(item.dtype != torch.float32 for item in state_tensors):
            return _case_failed("plugin_flashadamw_native_state_requires_float32", "plugin_flashadamw_native_state_dtype_unsupported")
        if not all(item.device.type == "cuda" for item in state_tensors):
            return _case_failed("plugin_flashadamw_native_state_requires_cuda", "plugin_flashadamw_native_state_device_unsupported")
        if not all(item.is_contiguous() and item.numel() == param.numel() for item in state_tensors):
            return _case_failed("plugin_flashadamw_native_state_layout_unsupported", "plugin_flashadamw_native_state_layout_unsupported")
        launch_config = {
            **dict(group_cfg),
        ...
        try:
            ...
            return _case_failed(f"{type(exc).__name__}: {exc}", "plugin_flashadamw_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(
                str(launch.get("reason") or "plugin_flashadamw_native_step_failed"),
                "plugin_flashadamw_native_step_failed",
                launch=launch,
            )
        return {
            "schema_version": 1,
...
def build_plugin_flashadamw_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: PluginFlashAdamWTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> PluginFlashAdamWTrainingExecutor:
    return PluginFlashAdamWTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: PluginFlashAdamWTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> PluginFlashAdamWTrainingExecutorConfig:
    if isinstance(value, PluginFlashAdamWTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return PluginFlashAdamWTrainingExecutorConfig(
        lr=float(payload.get("lr", group.get("lr", 1e-3))),
        betas=(float(betas[0]), float(betas[1])),
        eps=float(payload.get("eps", group.get("eps", 1e-8))),
        weight_decay=float(payload.get("weight_decay", group.get("weight_decay", 1e-2))),
        decouple_lr=bool(payload.get("decouple_lr", group.get("decouple_lr", False))),
        quantize=bool(payload.get("quantize", group.get("quantize", False))),
        master_bytewidth=int(payload.get("master_bytewidth", group.get("master_bytewidth", 0)) or 0),
...
def _group_config(group: Mapping[str, Any], config: PluginFlashAdamWTrainingExecutorConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "initial_lr": float(group.get("initial_lr", group.get("lr", config.lr)) or config.lr),
        "decouple_lr": bool(group.get("decouple_lr", config.decouple_lr)),
        "quantize": bool(group.get("quantize", config.quantize)),
        "master_bytewidth": int(group.get("master_bytewidth", config.master_bytewidth) or 0),
