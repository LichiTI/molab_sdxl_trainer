"""Default-off AdaSmooth Triton TrainingLoop executor for selected plugin canaries."""
...
class AdaSmoothTrainingExecutorConfig:
    optimizer_kind: str = "adasmooth"
    lr: float = 1.0e-3
    beta1: float = 0.5
    beta2: float = 0.99
...
if triton is not None and tl is not None:
    ...
    def _adasmooth_dense_kernel(
        ...
        prev = tl.load(prev_param_ptr + offsets, mask=mask, other=0.0).to(tl.float32)
        ...
        p_diff = p - prev
        s_new = s + p_diff
        n_new = n + tl.abs(p_diff)
        v_new = v * (1.0 - c_p2) + g * g * c_p2
        p_new = p - lr * g / tl.sqrt(v_new + eps)

        tl.store(s_ptr + offsets, s_new, mask=mask)
        tl.store(n_ptr + offsets, n_new, mask=mask)
        tl.store(exp_avg_sq_ptr + offsets, v_new, mask=mask)
        tl.store(prev_param_ptr + offsets, p_new, mask=mask)
        tl.store(param_ptr + offsets, p_new, mask=mask)


class AdaSmoothTrainingExecutor:
    """Launch a real AdaSmooth fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: AdaSmoothTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AdaSmoothTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("adasmooth_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("adasmooth_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("adasmooth_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"adasmooth_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("adasmooth_maximize_not_supported")
        ...
        if not cases:
            blockers.append("adasmooth_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_adasmooth_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "adasmooth_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("adasmooth_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("adasmooth_training_executor_requires_contiguous_tensors")
        state = self.optimizer.state[param]
        prev_param = _state_tensor(state, "prev_param", param)
        s = _state_tensor(state, "s", param)
        n = _state_tensor(state, "n", param)
        exp_avg_sq = _state_tensor(state, "exp_avg_sq", param)
        p_diff = param.detach().float() - prev_param.detach().float()
        s_sum = torch.sum(s.detach().float() + p_diff)
        n_sum = torch.sum(n.detach().float() + p_diff.abs())
        n_sum_value = float(n_sum.detach().cpu().item())
        if n_sum_value <= 0.0:
            return _case_failed("adasmooth_training_executor_requires_nonzero_param_delta")
        c = abs(float(s_sum.detach().cpu().item())) / n_sum_value
        c = c * (float(group_cfg["beta2"]) - float(group_cfg["beta1"])) + (1.0 - float(group_cfg["beta2"]))
        c_p2 = c * c
        before = param.detach().clone()
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _adasmooth_dense_kernel[grid](
            param,
            param.grad.detach(),
            prev_param,
            s,
        ...
        return {
            ...
            "c": float(c),
            "blocked_reasons": [] if mutated else ["adasmooth_training_executor_parameters_not_mutated"],
        }


def build_plugin_adasmooth_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AdaSmoothTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> AdaSmoothTrainingExecutor:
    return AdaSmoothTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AdaSmoothTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AdaSmoothTrainingExecutorConfig:
    if isinstance(value, AdaSmoothTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.5, 0.99)))
    return AdaSmoothTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "adasmooth"),
...
def _group_config(group: Mapping[str, Any], config: AdaSmoothTrainingExecutorConfig) -> dict[str, Any]:
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("adasmooth_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("fixed_decay", False)):
        blockers.append("adasmooth_fixed_decay_not_supported_for_canary")
    return blockers
...
__all__ = ["AdaSmoothTrainingExecutor", "AdaSmoothTrainingExecutorConfig", "build_plugin_adasmooth_training_executor"]
