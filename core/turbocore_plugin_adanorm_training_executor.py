"""Default-off AdaNorm Triton TrainingLoop executor for selected plugin canaries."""
...
class AdaNormTrainingExecutorConfig:
    optimizer_kind: str = "adanorm"
    lr: float = 1.0e-3
    beta1: float = 0.9
    beta2: float = 0.99
    r: float = 0.95
...
if triton is not None and tl is not None:
    ...
    def _adanorm_dense_kernel(
        ...
        v = tl.load(exp_avg_var_ptr + offsets, mask=mask, other=0.0).to(tl.float32)

        s_grad = g * adanorm_scale
        m_new = m * beta1 + s_grad * (1.0 - beta1)
        v_new = v * beta2 + g * g * (1.0 - beta2)
        denom = (tl.sqrt(v_new + 1.0e-15) + eps) / bias_correction2_sq
        p_new = p - step_size * m_new / denom

        tl.store(exp_avg_ptr + offsets, m_new, mask=mask)
        tl.store(exp_avg_var_ptr + offsets, v_new, mask=mask)
        tl.store(param_ptr + offsets, p_new, mask=mask)


class AdaNormTrainingExecutor:
    """Launch a real AdaNorm fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: AdaNormTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AdaNormTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("adanorm_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("adanorm_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("adanorm_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"adanorm_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("adanorm_maximize_not_supported")
        ...
        if not cases:
            blockers.append("adanorm_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_adanorm_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "adanorm_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("adanorm_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("adanorm_training_executor_requires_contiguous_tensors")
        state = self.optimizer.state[param]
        exp_avg = _state_tensor(state, "exp_avg", param)
        exp_avg_var = _state_tensor(state, "exp_avg_var", param)
        exp_grad_norm = _state_scalar_tensor(state, "exp_grad_norm", param)
        before = param.detach().clone()
        step = int(group.get("step", 0) or 0)
        grad_norm = torch.linalg.norm(param.grad.detach())
        scale = _adanorm_scale(exp_grad_norm, grad_norm)
        bias_correction1 = 1.0 - math.pow(float(group_cfg["beta1"]), step)
        bias_correction2_sq = math.sqrt(1.0 - math.pow(float(group_cfg["beta2"]), step))
        ...
        state["exp_avg_var"] = exp_avg_var
        state["exp_grad_norm"] = exp_grad_norm
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["adanorm_training_executor_parameters_not_mutated"],
        }


def build_plugin_adanorm_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AdaNormTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> AdaNormTrainingExecutor:
    return AdaNormTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AdaNormTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AdaNormTrainingExecutorConfig:
    if isinstance(value, AdaNormTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.99)))
    return AdaNormTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "adanorm"),
        lr=float(payload.get("lr", group.get("lr", 1.0e-3)) or 1.0e-3),
        beta1=float(betas[0]),
        beta2=float(betas[1]),
        r=float(payload.get("r", group.get("r", 0.95)) or 0.95),
...
def _group_config(group: Mapping[str, Any], config: AdaNormTrainingExecutorConfig) -> dict[str, Any]:
    betas = group.get("betas", (config.beta1, config.beta2))
    return {
        "lr": float(group.get("lr", config.lr) or config.lr),
        "beta1": float(betas[0]),
        "beta2": float(betas[1]),
        "r": float(group.get("r", config.r) or config.r),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("adanorm_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("fixed_decay", False)):
        blockers.append("adanorm_fixed_decay_not_supported_for_canary")
    if bool(group_cfg.get("ams_bound", False)):
        blockers.append("adanorm_ams_bound_not_supported_for_canary")
    if bool(group_cfg.get("adam_debias", False)):
        blockers.append("adanorm_adam_debias_not_supported_for_canary")
    return blockers
...
def _adanorm_scale(exp_grad_norm: torch.Tensor, grad_norm: torch.Tensor) -> float:
    grad_norm_value = float(grad_norm.detach().cpu().item())
    exp_norm_value = float(exp_grad_norm.detach().reshape(-1)[0].cpu().item())
    if grad_norm_value <= 0.0 or exp_norm_value <= grad_norm_value:
        return 1.0
    return exp_norm_value / grad_norm_value
...
__all__ = ["AdaNormTrainingExecutor", "AdaNormTrainingExecutorConfig", "build_plugin_adanorm_training_executor"]
