"""TrainingLoop canary for selected plugin AdamW native dispatch."""
...
def build_plugin_adamw_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_adamw_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = list(case.get("blocked_reasons", []) or [])
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_adamw_training_loop_canary_scorecard_v0",
        "gate": "plugin_adamw_training_loop_native_canary",
        "ok": ok,
        ...
        + ["selected_adamlike_dedicated_kernel_matrix_missing", "product_rollout_review_missing"],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "batch dedicated native kernels for adam-like plugin variants"
            if ok
            else "fix selected plugin AdamW TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_adamw_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_adamw_training_loop_did_not_emit_step"],
        }
    ...
    ok = bool(
        ...
        and step_after == 2
        and type(optimizer).__name__ == "AdamW"
    )
    return {
        "schema_version": 1,
        "ok": ok,
        ...
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "blocked_reasons": [] if ok else _dedupe(list(runtime.get("blocked_reasons", []) or []) + ["plugin_adamw_training_loop_native_step_missing"]),
    }


def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    device = torch.device("cuda")
    param = torch.nn.Parameter(torch.linspace(-0.25, 0.35, steps=4096, device=device, dtype=torch.float32))
    optimizer = _create_selected_adamw_plugin_optimizer(param)
    loop = TrainingLoop(
...
def _create_selected_adamw_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
    trainer = LulynxTrainer.__new__(LulynxTrainer)
    case = plugin_resume_case("adamw")
...
def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> None:
    param.grad = None
    loss = ((param.float() * 0.29) ** 2).mean() + param.float().mean() * 0.005
    loss.backward()
    optimizer.step()
...
def _step_to_int(step: Any) -> int:
    if torch.is_tensor(step) and step.numel() > 0:
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin AdamW TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_adamw_training_loop_canary_scorecard"]
