"""TrainingLoop canary for selected plugin AdamS native dispatch."""
...
def build_plugin_adams_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_adams_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = _strings(case.get("blocked_reasons"))
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_adams_training_loop_canary_scorecard_v0",
        "gate": "plugin_adams_training_loop_native_canary",
        "ok": ok,
        ...
        "summary": {
            ...
            "ams_bound_enabled": case.get("ams_bound_enabled"),
            "adanorm_enabled": case.get("adanorm_enabled"),
        },
        "promotion_blockers": blockers
        + [
            "adams_e2e_shadow_matrix_refresh_required",
            "adams_canary_rollout_policy_refresh_required",
            "product_rollout_review_missing",
        ],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "refresh selected Adam-like e2e shadow matrix and rollout policy for adams"
            if ok
            else "fix selected plugin AdamS TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    def _fake_train_step(
        ...
        for item in params:
            item.grad = None
        loss = sum(((item.float() * 0.18) ** 2).mean() + item.float().mean() * 0.005 for item in params)
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_adams_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_adams_training_loop_did_not_emit_step"],
        }
    ...
    ams_bound_enabled = bool(group.get("ams_bound", False))
    ...
    ok = bool(
        ...
        and step_after == 2
        and type(optimizer).__name__ == "AdamS"
        and str(executor_result.get("optimizer_kind") or "") == "adams"
        and {"exp_avg", "exp_avg_sq"}.issubset(state_keys)
        and not ams_bound_enabled
    ...
    return {
        ...
        "ams_bound_enabled": ams_bound_enabled,
        ...
        "blocked_reasons": []
        if ok
        else _dedupe(_strings(runtime.get("blocked_reasons")) + ["plugin_adams_training_loop_native_step_missing"]),
    }


def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    device = torch.device("cuda")
    param = torch.nn.Parameter(torch.linspace(-0.09, 0.19, steps=4096, device=device, dtype=torch.float32))
    optimizer = _create_selected_adams_plugin_optimizer(param)
    loop = TrainingLoop(
...
def _create_selected_adams_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
    ...
    trainer.config = LulynxConfig(
        ...
        learning_rate=1e-3,
        weight_decay=0.01,
        optimizer_args=f"{case.optimizer_args},ams_bound=False,adanorm=False,weight_decouple=True",
...
def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> None:
    param.grad = None
    loss = ((param.float() * 0.14) ** 2).mean() + param.float().mean() * 0.004
    loss.backward()
    optimizer.step()
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin AdamS TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_adams_training_loop_canary_scorecard"]
