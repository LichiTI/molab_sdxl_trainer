"""Default-off training executor for selected plugin ScheduleFreeRAdam."""
...
ENTRYPOINT = "probe_schedulefree_radam_training_tensor_binding_canary_py"
REPO_ROOT = Path(__file__).resolve().parents[2]


@dataclass(frozen=True)
class ScheduleFreeRAdamTrainingExecutorConfig:
    lr: float = 2.5e-3
...
class ScheduleFreeRAdamTrainingExecutor:
    """Launch native ScheduleFreeRAdam steps against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: ScheduleFreeRAdamTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("ScheduleFreeRAdamTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("schedulefree_radam_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("schedulefree_radam_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("schedulefree_radam_training_dispatch_entrypoint_missing")
        ...
        if not cases:
            blockers.append("schedulefree_radam_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_schedulefree_radam_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "schedulefree_radam_native_step_failed"),
            "optimizer_kind": "schedulefree_radam",
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed(
                "schedulefree_radam_native_step_requires_float32",
                "schedulefree_radam_native_step_dtype_unsupported",
            )
        if group.get("train_mode") is False:
            return _case_failed(
                "schedulefree_radam_native_step_requires_train_mode",
                "schedulefree_radam_training_mode_not_active",
            )
        state = self.optimizer.state[param]
        missing = [name for name in ("z", "exp_avg_sq") if name not in state]
        if missing:
            return _case_failed(
                f"schedulefree_radam_live_state_missing:{','.join(missing)}",
                "schedulefree_radam_live_state_missing",
            )
        z = state["z"].contiguous()
        exp_avg_sq = state["exp_avg_sq"].contiguous()
        if z.dtype != torch.float32 or exp_avg_sq.dtype != torch.float32:
            return _case_failed(
                "schedulefree_radam_state_requires_float32",
                "schedulefree_radam_state_dtype_unsupported",
            )
        ...
        try:
            ...
            return _case_failed(f"{type(exc).__name__}: {exc}", "schedulefree_radam_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(
                str(launch.get("reason") or "native_step_failed"),
                "schedulefree_radam_native_step_failed",
                launch,
            )
...
def build_plugin_schedulefree_radam_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: ScheduleFreeRAdamTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> ScheduleFreeRAdamTrainingExecutor:
    return ScheduleFreeRAdamTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: ScheduleFreeRAdamTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> ScheduleFreeRAdamTrainingExecutorConfig:
    if isinstance(value, ScheduleFreeRAdamTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return ScheduleFreeRAdamTrainingExecutorConfig(
        lr=float(payload.get("lr", group.get("lr", 2.5e-3))),
        betas=(float(betas[0]), float(betas[1])),
        eps=float(payload.get("eps", group.get("eps", 1e-8))),
        weight_decay=float(payload.get("weight_decay", group.get("weight_decay", 0.0))),
        silent_sgd_phase=bool(payload.get("silent_sgd_phase", group.get("silent_sgd_phase", True))),
...
def _group_config(group: Mapping[str, Any], config: ScheduleFreeRAdamTrainingExecutorConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "silent_sgd_phase": bool(group.get("silent_sgd_phase", config.silent_sgd_phase)),
