"""Default-off EmoNavi Triton TrainingLoop executor for selected plugin canaries."""
...
class EmoNaviTrainingExecutorConfig:
    optimizer_kind: str = "emonavi"
    lr: float = 1.0e-3
    beta1: float = 0.9
    beta2: float = 0.999
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _emonavi_first_step_kernel(
        param_ptr,
        grad_ptr,
        exp_avg_ptr,
        exp_avg_sq_ptr,
        ...
        param_new = param - lr * exp_avg / (tl.sqrt(exp_avg_sq) + eps)
...
class EmoNaviTrainingExecutor:
    """Launch a real EmoNavi no-shadow first-step fp32 Triton kernel."""
    ...
    def __init__(
        ...
        *,
        ...
        config: EmoNaviTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("EmoNaviTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("emonavi_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("emonavi_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("emonavi_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"emonavi_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("emonavi_maximize_not_supported")
        ...
        if not cases:
            blockers.append("emonavi_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_emonavi_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "emonavi_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("emonavi_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("emonavi_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("emonavi_training_executor_only_first_step_supported_for_canary")
        state = self.optimizer.state[param]
        exp_avg = _state_tensor(state, "exp_avg", param)
        exp_avg_sq = _state_tensor(state, "exp_avg_sq", param)
        if exp_avg is None or exp_avg_sq is None:
            return _case_failed("emonavi_training_executor_requires_fp32_state")
        if not exp_avg.is_contiguous() or not exp_avg_sq.is_contiguous():
            return _case_failed("emonavi_training_executor_requires_contiguous_state")
        before = param.detach().clone()
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _emonavi_first_step_kernel[grid](
            param,
            param.grad.detach(),
            exp_avg,
            exp_avg_sq,
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["emonavi_training_executor_parameters_not_mutated"],
        }


def build_plugin_emonavi_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: EmoNaviTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> EmoNaviTrainingExecutor:
    return EmoNaviTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: EmoNaviTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> EmoNaviTrainingExecutorConfig:
    if isinstance(value, EmoNaviTrainingExecutorConfig):
        return value
    ...
    if not isinstance(betas, (tuple, list)):
        betas = (0.9, 0.999)
    return EmoNaviTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "emonavi"),
...
def _group_config(group: Mapping[str, Any], config: EmoNaviTrainingExecutorConfig) -> dict[str, Any]:
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if bool(group_cfg.get("use_shadow", False)):
        blockers.append("emonavi_shadow_not_supported_for_canary")
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("emonavi_weight_decay_not_supported_for_canary")
    if float(group_cfg.get("lr", 0.0) or 0.0) <= 0.0:
        blockers.append("emonavi_lr_must_be_positive")
    ...
    if beta1 < 0.0 or beta1 >= 1.0 or beta2 < 0.0 or beta2 >= 1.0:
        blockers.append("emonavi_betas_out_of_range")
    return blockers
...
__all__ = [
    "EmoNaviTrainingExecutor",
    "EmoNaviTrainingExecutorConfig",
    "build_plugin_emonavi_training_executor",
]
