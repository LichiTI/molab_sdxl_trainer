"""TrainingLoop canary for selected plugin AdamP native dispatch."""
...
def build_plugin_adamp_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_adamp_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = list(case.get("blocked_reasons", []) or [])
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_adamp_training_loop_canary_scorecard_v0",
        "gate": "plugin_adamp_training_loop_native_canary",
        "ok": ok,
        ...
        "selected_optimizer_name": "adamp",
        "native_route": "rust_cuda_plugin_adamp_v0",
        "case": case,
        "summary": {
            ...
            "projection_shape": case.get("projection_shape"),
        },
        ...
        "recommended_next_step": (
            "add selected AdamP e2e shadow matrix and rollout policy"
            if ok
            else "fix selected plugin AdamP TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_adamp_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_adamp_training_loop_did_not_emit_step"],
        }
    ...
    ok = bool(
        ...
        and step_after == 2
        and type(optimizer).__name__ == "AdamP"
        and _has_adamp_state(param, optimizer)
    )
    return {
        "schema_version": 1,
        "ok": ok,
        ...
        "projection_shape": list(param.shape),
        "executor_case": first_case,
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "blocked_reasons": []
        if ok
        else _dedupe(list(runtime.get("blocked_reasons", []) or []) + ["plugin_adamp_training_loop_native_step_missing"]),
    }
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    param = torch.nn.Parameter(
        torch.linspace(-0.14, 0.24, steps=4096, device=device, dtype=torch.float32).reshape(64, 64)
    )
    optimizer = _create_selected_adamp_plugin_optimizer(param)
    loop = TrainingLoop(
        ...
        turbocore_native_update_quantized_optimizer_kind="adamp",
    )
    loop.total_steps = 1
    return loop, param, optimizer


def _create_selected_adamp_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
    trainer = LulynxTrainer.__new__(LulynxTrainer)
    case = plugin_resume_case("adamp")
...
def _has_adamp_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> bool:
    state = optimizer.state[param]
    return torch.is_tensor(state.get("exp_avg")) and torch.is_tensor(state.get("exp_avg_sq"))
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin AdamP TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_adamp_training_loop_canary_scorecard"]
