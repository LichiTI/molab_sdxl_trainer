"""Default-off training tensor binding canary for selected plugin AdamC."""
...
def build_plugin_adamc_training_tensor_binding_canary_scorecard(
    *,
    ...
    scratch = dict(
        scratch_report
        or build_plugin_adamc_native_scratch_kernel_scorecard(workspace_root=workspace_root, arch=arch)
    )
    ...
    return {
        ...
        "promotion_blockers": _dedupe(
            blockers
            + [
                "adamc_runtime_dispatch_shadow_missing",
                "adamc_training_loop_canary_missing",
                "adamc_rollback_review_missing",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "add selected AdamC runtime dispatch shadow before TrainingLoop canary"
            if ready
            else "fix selected AdamC training tensor binding canary blockers"
        ),
    }
...
def _live_probe(*, workspace_root: str | Path | None, arch: str | None) -> dict[str, Any]:
    ...
    if native is None:
        return _failed("lulynx_native_entrypoint_missing", ["adamc_training_tensor_binding_entrypoint_missing"])
    ...
    reference = _reference_adamc(role_tensors, config)
    ...
    if not bool(session.get("ok", False)):
        return _failed("tensor_binding_session_create_failed", ["adamc_tensor_binding_session_create_failed"], session=session)
    session_id = int(session["session_id"])
    try:
        launch = dict(native.tensor_binding_session_cuda_adamc_tensor_probe(session_id, json.dumps(config)))
        if not bool(launch.get("ok", False)):
            return _failed(
                f"adamc_tensor_probe_launch_failed:{launch.get('reason', 'unknown')}",
                ["adamc_tensor_probe_launch_failed"],
                session=session,
                launch=launch,
            )
        ...
        return {
            ...
            "probe_kind": "plugin_adamc_training_tensor_binding_canary_v0",
            "ok": ok,
            ...
            "session": session,
            "launch": launch,
            "blocked_reasons": [] if ok else ["adamc_training_tensor_binding_parity_failed"],
        }
    finally:
...
def _make_cuda_request() -> tuple[dict[str, Any], dict[str, torch.Tensor], dict[str, torch.Tensor]]:
    ...
    registry = TurboCoreTensorHandleRegistry(namespace="plugin_adamc_tensor_binding_canary")
    ...
    exp_avg = (torch.arange(24, dtype=torch.float32, device=device) * 0.00027 - 0.001).contiguous()
    exp_avg_sq = (torch.arange(24, dtype=torch.float32, device=device) * 0.000012 + 0.00021).contiguous()
...
def _reference_adamc(role_tensors: dict[str, torch.Tensor], config: Mapping[str, Any]) -> dict[str, torch.Tensor]:
    ...
    denom = exp_avg_sq.add(1e-15).sqrt().div_(bias_correction2**0.5).add_(eps)
...
def _validations(scratch: Mapping[str, Any], live_probe: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        _validation(
            "p51_adamc_scratch_kernel_ready",
            bool(scratch.get("adamc_native_kernel_parity", False)),
            "adamc_native_scratch_kernel_missing",
        ),
        _validation(
            "training_tensor_binding_parity",
            bool(live_probe.get("training_tensor_binding_parity_passed", False)),
            "adamc_training_tensor_binding_canary_failed",
        ),
        _validation(
            ...
            "adamc_training_tensor_binding_enabled_dispatch",
        ),
    ]
...
__all__ = ["build_plugin_adamc_training_tensor_binding_canary_scorecard"]
