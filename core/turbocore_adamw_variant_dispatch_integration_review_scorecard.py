"""Manual dispatch review package for AdamW variant native canaries."""

from __future__ import annotations

from typing import Any, Mapping, Sequence

from core.turbocore_adamw_variant_canary_rollout_policy_scorecard import (
    FALLBACK_BACKEND,
...
REVIEW_KIND = "adamw_variant_dispatch_integration_review_v0"


def build_adamw_variant_dispatch_integration_review_scorecard(
    *,
    ...
    policy_report = _as_dict(rollout_policy_report or _default_policy_report())
    ...
    return {
        "schema_version": 1,
        "scorecard": "turbocore_adamw_variant_dispatch_integration_review_scorecard_v0",
        "gate": "adamw_variant_dispatch_integration_review",
        "ok": ready,
        ...
        "recommended_next_step": (
            "requires explicit owner approval before wiring AdamW variant canary dispatch"
            if ready
            else "fix AdamW variant dispatch integration review blockers"
        ),
        "notes": [
            "This gate prepares a real-dispatch integration review only.",
            "Existing Python/third-party optimizers remain the training update authority.",
            "Canary and auto modes stay blocked until manual review approves wiring.",
        ],
    }


def _default_policy_report() -> dict[str, Any]:
    shadow = build_adamw_variant_e2e_shadow_matrix_scorecard(include_live_canaries=True)
    return build_adamw_variant_canary_rollout_policy_scorecard(shadow_matrix_report=shadow)
...
def _review_package(policy_report: Mapping[str, Any], mode: str) -> dict[str, Any]:
    ...
    return {
        ...
        "dispatch_contract": {
            ...
            "requires_variant_native_canary": True,
...
def _validations(policy_report: Mapping[str, Any], review: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _validation(
            ...
            len(optimizer_types) == 6 and "AdamWScheduleFree" in optimizer_types,
            "adamw_variant_dispatch_review_optimizer_set_incomplete",
        ),
        _validation(
            ...
            "adamw_variant_dispatch_review_hook_contract_missing",
        ),
        _validation(
            "dispatch_contract_requires_prior_evidence",
            bool(dispatch.get("requires_variant_native_canary"))
            ...
            "adamw_variant_dispatch_review_prior_evidence_missing",
        ),
        _validation(
            ...
            "adamw_variant_dispatch_review_numeric_guardrails_missing",
        ),
        _validation(
            ...
            "adamw_variant_dispatch_review_missing_rollback",
        ),
        _validation(
            ...
            "adamw_variant_dispatch_review_allows_dispatch_before_review",
        ),
        _validation(
...
__all__ = ["REVIEW_KIND", "build_adamw_variant_dispatch_integration_review_scorecard"]
