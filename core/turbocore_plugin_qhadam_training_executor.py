"""Default-off training executor for selected plugin QHAdam native dispatch."""
...
ENTRYPOINT = "probe_qhadam_training_tensor_binding_canary_py"
REPO_ROOT = Path(__file__).resolve().parents[2]


@dataclass(frozen=True)
class PluginQHAdamTrainingExecutorConfig:
    lr: float = 1e-3
...
class PluginQHAdamTrainingExecutor:
    """Launch selected QHAdam native steps against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: PluginQHAdamTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("PluginQHAdamTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("plugin_qhadam_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("plugin_qhadam_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("plugin_qhadam_training_dispatch_entrypoint_missing")
        ...
        if not cases:
            blockers.append("plugin_qhadam_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_qhadam_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "plugin_qhadam_native_step_failed"),
            "optimizer_kind": "qhadam",
    ...
    def _step_param(
        ...
        if type(self.optimizer).__name__.lower() != "qhadam":
            return _case_failed("plugin_qhadam_training_executor_optimizer_not_qhadam", "plugin_qhadam_optimizer_class_unsupported")
        if param.dtype != torch.float32 or param.grad is None or param.grad.dtype != torch.float32:
            return _case_failed("plugin_qhadam_native_step_requires_float32", "plugin_qhadam_native_step_dtype_unsupported")
        if not param.is_contiguous() or not param.grad.is_contiguous():
            return _case_failed("plugin_qhadam_native_step_requires_contiguous_param_grad", "plugin_qhadam_native_step_layout_unsupported")
        state = self.optimizer.state[param]
        required = ("exp_avg", "exp_avg_sq", "beta1_weight", "beta2_weight")
        if not all(name in state for name in required):
            return _case_failed("plugin_qhadam_live_state_missing", "plugin_qhadam_live_state_missing")
        exp_avg = state["exp_avg"]
        exp_avg_sq = state["exp_avg_sq"]
        beta1_weight = state["beta1_weight"]
        beta2_weight = state["beta2_weight"]
        state_tensors = (exp_avg, exp_avg_sq, beta1_weight, beta2_weight)
        if not all(torch.is_tensor(item) for item in state_tensors):
            return _case_failed("plugin_qhadam_live_state_tensors_missing", "plugin_qhadam_live_state_missing")
        if any(item.dtype != torch.float32 for item in state_tensors):
            return _case_failed("plugin_qhadam_native_state_requires_float32", "plugin_qhadam_native_state_dtype_unsupported")
        if not exp_avg.is_contiguous() or not exp_avg_sq.is_contiguous():
            return _case_failed("plugin_qhadam_native_state_requires_contiguous", "plugin_qhadam_native_state_layout_unsupported")
        if beta1_weight.numel() != 1 or beta2_weight.numel() != 1:
            return _case_failed("plugin_qhadam_beta_weight_requires_scalar_tensor", "plugin_qhadam_live_state_shape_unsupported")
        launch_config = {
            **dict(group_cfg),
        ...
        try:
            ...
            return _case_failed(f"{type(exc).__name__}: {exc}", "plugin_qhadam_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(
                str(launch.get("reason") or "plugin_qhadam_native_step_failed"),
                "plugin_qhadam_native_step_failed",
                launch=launch,
            )
        return {
            "schema_version": 1,
...
def build_plugin_qhadam_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: PluginQHAdamTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> PluginQHAdamTrainingExecutor:
    return PluginQHAdamTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: PluginQHAdamTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> PluginQHAdamTrainingExecutorConfig:
    if isinstance(value, PluginQHAdamTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    nus = payload.get("nus", group.get("nus", (1.0, 1.0)))
    return PluginQHAdamTrainingExecutorConfig(
        lr=float(payload.get("lr", group.get("lr", 1e-3))),
        betas=(float(betas[0]), float(betas[1])),
        nus=(float(nus[0]), float(nus[1])),
...
def _group_config(group: Mapping[str, Any], config: PluginQHAdamTrainingExecutorConfig) -> dict[str, Any]:
    betas = group.get("betas", config.betas)
    nus = group.get("nus", config.nus)
    return {
...
__all__ = [
    "PluginQHAdamTrainingExecutor",
    "PluginQHAdamTrainingExecutorConfig",
    "build_plugin_qhadam_training_executor",
]
