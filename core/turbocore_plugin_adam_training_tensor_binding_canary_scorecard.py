"""Default-off training tensor binding canary for selected plugin Adam."""
...
def build_plugin_adam_training_tensor_binding_canary_scorecard(
    *,
    ...
    scratch = dict(
        scratch_report
        or build_plugin_adam_native_scratch_kernel_scorecard(workspace_root=workspace_root, arch=arch)
    )
    ...
    return {
        ...
        "promotion_blockers": _dedupe(
            blockers
            + [
                "adam_runtime_dispatch_shadow_missing",
                "adam_training_loop_canary_missing",
                "adam_rollback_review_missing",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "add selected Adam runtime dispatch shadow before TrainingLoop canary"
            if ready
            else "fix selected Adam training tensor binding canary blockers"
        ),
    }
...
def _live_probe(*, workspace_root: str | Path | None, arch: str | None) -> dict[str, Any]:
    ...
    if native is None:
        return _failed("lulynx_native_entrypoint_missing", ["adam_training_tensor_binding_entrypoint_missing"])
    ...
    reference = _reference_adam(role_tensors, config)
    ...
    if not bool(session.get("ok", False)):
        return _failed("tensor_binding_session_create_failed", ["adam_tensor_binding_session_create_failed"], session=session)
    session_id = int(session["session_id"])
    try:
        launch = dict(native.tensor_binding_session_cuda_adam_tensor_probe(session_id, json.dumps(config)))
        if not bool(launch.get("ok", False)):
            return _failed(
                f"adam_tensor_probe_launch_failed:{launch.get('reason', 'unknown')}",
                ["adam_tensor_probe_launch_failed"],
                session=session,
                launch=launch,
            )
        ...
        return {
            ...
            "probe_kind": "plugin_adam_training_tensor_binding_canary_v0",
            "ok": ok,
            ...
            "session": session,
            "launch": launch,
            "blocked_reasons": [] if ok else ["adam_training_tensor_binding_parity_failed"],
        }
    finally:
...
def _make_cuda_request() -> tuple[dict[str, Any], dict[str, torch.Tensor], dict[str, torch.Tensor]]:
    ...
    registry = TurboCoreTensorHandleRegistry(namespace="plugin_adam_tensor_binding_canary")
    ...
    exp_avg = (torch.arange(24, dtype=torch.float32, device=device) * 0.0003 - 0.001).contiguous()
    exp_avg_sq = (torch.arange(24, dtype=torch.float32, device=device) * 0.00001 + 0.0002).contiguous()
...
def _reference_adam(role_tensors: dict[str, torch.Tensor], config: Mapping[str, Any]) -> dict[str, torch.Tensor]:
...
def _validations(scratch: Mapping[str, Any], live_probe: Mapping[str, Any]) -> list[dict[str, Any]]:
    return [
        _validation(
            "p43_adam_scratch_kernel_ready",
            bool(scratch.get("adam_native_kernel_parity", False)),
            "adam_native_scratch_kernel_missing",
        ),
        _validation(
            "training_tensor_binding_parity",
            bool(live_probe.get("training_tensor_binding_parity_passed", False)),
            "adam_training_tensor_binding_canary_failed",
        ),
        _validation(
            ...
            "adam_training_tensor_binding_enabled_dispatch",
        ),
    ]
...
__all__ = ["build_plugin_adam_training_tensor_binding_canary_scorecard"]
