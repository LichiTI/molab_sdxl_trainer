"""Kernel launch execution contract for TurboCore V5-P62.

P62 records future kernel launch execution contract evidence after P61. It
checks kernel launch plan, authorization, preconditions, artifact, parameter,
stream, parity, tensor-transfer, rollback, and observability evidence only; it
does not approve or execute kernel launches, parity, tensor transfer, training
steps, request fields, UI exposure, or training.
"""
...
from core.turbocore_v5_native_dispatch_execution_contract import (
    P61_READY_DECISION,
    UNSAFE_NON_EMPTY_FIELDS as _P61_UNSAFE_NON_EMPTY_FIELDS,
    UNSAFE_TRUE_FIELDS as _P61_UNSAFE_TRUE_FIELDS,
)
from core.turbocore_v5_owner_review_evidence_package import load_json


P62_READY_DECISION = "kernel_launch_execution_contract_recorded_default_off"
P62_BLOCKED_DECISION = "kernel_launch_execution_contract_blocked_default_off"
P62_HOLD_DECISION = "kernel_launch_execution_contract_hold_for_signed_review_default_off"
P62_REJECTED_DECISION = "kernel_launch_execution_contract_rejected_default_off"
P62_SCOPE = "kernel_launch_execution_contract"
DEFAULT_REQUIRED_SECTIONS = (
    "p61_native_dispatch_execution_reference", "kernel_launch_execution_plan_inventory",
    "kernel_launch_authorization_boundary", "kernel_launch_precondition_inventory",
    "kernel_build_artifact_boundary", "kernel_launch_parameter_boundary",
    "kernel_stream_synchronization_boundary", "parity_boundary", "tensor_transfer_boundary",
    ...
    "no_tensor_transfer_execution_boundary", "no_training_step_boundary",
...
REQUIRED_REVIEW_ACKS = (
    "acknowledge_p61_native_dispatch_execution_contract_recorded", "acknowledge_default_off_boundary",
    "acknowledge_no_training_launch", "acknowledge_no_ui_exposure",
    "acknowledge_no_kernel_launch_approved", "acknowledge_no_kernel_launch_executed",
    "acknowledge_no_parity_executed", "acknowledge_no_tensor_transfer_executed",
    "acknowledge_no_training_step_executed", "acknowledge_no_native_dispatch_executed",
    ...
    "acknowledge_kernel_launch_execution_contract_evidence_replayable",
    "acknowledge_later_parity_execution_contract_required", "acknowledge_manual_review_only",
)
UNSAFE_TRUE_FIELDS = tuple(
    dict.fromkeys(
        (
            *_P61_UNSAFE_TRUE_FIELDS,
            "kernel_launch_approved", "kernel_launch_execution_allowed",
            "kernel_launch_execution_contract_allowed", "kernel_launch_execution_contract_enabled",
            "kernel_launch_execution_contract_executed", "kernel_launch_execution_contract_applied",
            "kernel_launch_plan_activated", "kernel_launch_plan_executed",
            "kernel_launch_request_submitted", "kernel_launch_request_executed",
            "kernel_build_artifact_loaded", "kernel_parameters_applied",
            "kernel_stream_synchronization_executed", "parity_execution_allowed",
            "parity_execution_executed", "tensor_transfer_allowed", "kernel_execution_result_recorded",
        )
    )
)
UNSAFE_NON_EMPTY_FIELDS = tuple(
    dict.fromkeys(
        (
            *_P61_UNSAFE_NON_EMPTY_FIELDS,
            "post_p62_request_fields", "post_kernel_launch_execution_contract_fields",
            "kernel_launch_execution_contract_request", "kernel_launch_execution_contract_payload",
            "kernel_launch_execution_plan_request", "kernel_launch_execution_plan_payload",
            "kernel_launch_authorization_request", "kernel_launch_authorization_payload",
            "kernel_build_artifact_payload", "kernel_launch_parameter_payload",
            "kernel_stream_synchronization_payload", "kernel_execution_result_payload",
            "parity_execution_payload", "tensor_transfer_payload", "training_step_payload",
        )
    )
)
...
P61_REQUIRED_FALSE_FIELDS = _P61_UNSAFE_TRUE_FIELDS


def build_v5_kernel_launch_execution_contract(
    *,
    p61_native_dispatch_execution: Mapping[str, Any] | None = None,
    ...
    """Record kernel launch execution evidence without executing it."""

    p61 = _as_dict(p61_native_dispatch_execution)
    ...
    p61_summary = _p61_summary(p61)
    evidence_summary = _evidence_summary(evidence)
    review_summary = _review_summary(review)
    evidence_blockers = _evidence_blockers(p61_summary, evidence_summary, failure_events, rollback_events)
    ...
    if decision == P62_HOLD_DECISION:
        blockers.append("v5_p62_signed_kernel_launch_execution_review_missing")
    blockers = _dedupe(blockers)
    ready = decision == P62_READY_DECISION
    rejected = decision == P62_REJECTED_DECISION
    decision_record_ready = ready or rejected
    return {
        "schema_version": 1,
        "package": "turbocore_v5_kernel_launch_execution_contract_v0",
        "gate": "v5_kernel_launch_execution_contract",
        ...
        **{field: False for field in UNSAFE_TRUE_FIELDS},
        "post_p62_request_fields": {},
        "p61_native_dispatch_execution_summary": p61_summary,
        ...
        "progress_gates": _progress_gates(p61_summary, evidence_summary, review_summary),
        ...
        "notes": [
            "P62 records kernel launch execution contract evidence only.",
            "P62 does not approve or execute kernel launches, parity, tensor transfer, training steps, request fields, UI exposure, or training.",
            "A later parity execution contract is still required before kernel behavior can become active.",
        ],
    }


def _p61_summary(report: Mapping[str, Any]) -> dict[str, Any]:
...
def _evidence_summary(evidence: Mapping[str, Any]) -> dict[str, Any]:
    ...
    plan_rows = _rows(evidence, "kernel_launch_execution_plan_inventory", "kernel_launch_plan_inventory")
    auth_rows = _rows(evidence, "kernel_launch_authorization_boundary", "kernel_authorization_inventory")
    precondition_rows = _rows(evidence, "kernel_launch_precondition_inventory", "kernel_precondition_inventory")
    artifact_rows = _rows(evidence, "kernel_build_artifact_boundary", "kernel_artifact_inventory")
    parameter_rows = _rows(evidence, "kernel_launch_parameter_boundary", "kernel_parameter_inventory")
    stream_rows = _rows(evidence, "kernel_stream_synchronization_boundary", "kernel_stream_inventory")
    ...
    blockers = _evidence_blocker_list(
        evidence, missing_sections, plan_rows, auth_rows, precondition_rows, artifact_rows, parameter_rows,
        stream_rows, parity_rows, tensor_rows, rollback_rows, observability_rows,
    )
    return {
        ...
        "artifact_boundary_count": len(artifact_rows),
        "parameter_boundary_count": len(parameter_rows),
        "stream_boundary_count": len(stream_rows),
...
def _progress_gates(p61: Mapping[str, Any], evidence: Mapping[str, Any], review: Mapping[str, Any]) -> dict[str, bool]:
    return {
        "p61_native_dispatch_execution_contract_ready": _p61_ready(p61),
        ...
        "requested_scope_valid": review.get("requested_scope") == P62_SCOPE,
...
def _decision(evidence_blockers: list[str], review: Mapping[str, Any], review_blockers: list[str]) -> str:
    if evidence_blockers:
        return P62_BLOCKED_DECISION
    if not review:
        return P62_HOLD_DECISION
    if review_blockers:
        return P62_BLOCKED_DECISION
    if review.get("approve_kernel_launch_execution_contract") is True:
        return P62_READY_DECISION
    return P62_REJECTED_DECISION


def _evidence_blockers(
    p61_summary: Mapping[str, Any],
    evidence_summary: Mapping[str, Any],
    failure_events: list[str],
    rollback_events: list[str],
) -> list[str]:
    blocked: list[str] = []
    if not bool(p61_summary.get("present", False)):
        blocked.append("v5_p62_p61_native_dispatch_execution_missing")
    elif not _p61_ready(p61_summary):
        blocked.append("v5_p62_p61_native_dispatch_execution_not_ready")
        blocked.extend(_string_list(p61_summary.get("blocked_reasons")))
        blocked.extend(_string_list(p61_summary.get("promotion_blockers")))
        blocked.extend(_string_list(p61_summary.get("unsafe_claims")))
    if not bool(evidence_summary.get("present", False)):
        blocked.append("v5_p62_kernel_launch_execution_evidence_missing")
    elif not bool(evidence_summary.get("ready", False)):
        blocked.extend(_string_list(evidence_summary.get("blockers")))
    for event in failure_events:
        blocked.append(f"v5_p62_failure_history_not_clear:{event}")
    for event in rollback_events:
        blocked.append(f"v5_p62_rollback_history_not_clear:{event}")
...
def _review_blockers(review: Mapping[str, Any]) -> list[str]:
    ...
    if not review.get("reviewer"):
        blocked.append("v5_p62_reviewer_missing")
    if not review.get("reviewed_at"):
        blocked.append("v5_p62_reviewed_at_missing")
    if review.get("requested_scope") != P62_SCOPE:
        blocked.append("v5_p62_requested_scope_invalid")
    for field in UNSAFE_REVIEW_APPROVAL_FIELDS:
        if review.get(field):
            blocked.append(f"v5_p62_unsafe_review_approval:{field}")
    for field in REQUIRED_REVIEW_ACKS:
        if not review.get(field):
            blocked.append(f"v5_p62_review_ack_missing:{field}")
    return _dedupe(blocked)


def _p61_ready(summary: Mapping[str, Any]) -> bool:
    return bool(
        ...
        and summary.get("decision") == P61_READY_DECISION
        and summary.get("manual_review_required")
        and all(summary.get(field) is False for field in P61_REQUIRED_FALSE_FIELDS)
...
def _evidence_blocker_list(
    ...
    missing: list[str],
    plan_rows: list[dict[str, Any]], auth_rows: list[dict[str, Any]],
    precondition_rows: list[dict[str, Any]], artifact_rows: list[dict[str, Any]],
    parameter_rows: list[dict[str, Any]], stream_rows: list[dict[str, Any]],
    parity_rows: list[dict[str, Any]], tensor_rows: list[dict[str, Any]],
    rollback_rows: list[dict[str, Any]], observability_rows: list[dict[str, Any]],
) -> list[str]:
    blocked: list[str] = []
    if evidence.get("ok") is not True:
        blocked.append("v5_p62_kernel_launch_execution_evidence_not_ok")
    if evidence.get("kernel_launch_execution_contract_ready") is not True:
        blocked.append("v5_p62_kernel_launch_execution_evidence_not_ready")
    for field in (
        "report_only", "boundary_only", "contract_only", "kernel_launch_execution_contract_only",
        "records_evidence_only", "manual_only", "internal_only", "requires_later_parity_execution_contract",
        "requires_explicit_owner_approval", "requires_explicit_operator_opt_in",
    ):
        if evidence.get(field) is not True:
            blocked.append(f"v5_p62_kernel_launch_execution_evidence_{field}_missing")
    if evidence.get("default_off") is not True or not _default_off_confirmed(evidence):
        blocked.append("v5_p62_kernel_launch_execution_evidence_default_off_violation")
    if evidence.get("request_adapter_off") is not True or not _request_adapter_off(evidence):
        blocked.append("v5_p62_kernel_launch_execution_evidence_request_adapter_violation")
    if not _digest(evidence):
        blocked.append("v5_p62_kernel_launch_execution_evidence_digest_missing")
    if not _source(evidence):
        blocked.append("v5_p62_kernel_launch_execution_evidence_source_missing")
    for section in missing:
        blocked.append(f"v5_p62_kernel_launch_execution_evidence_section_missing:{section}")
    for rows, kind in (
        (plan_rows, "kernel_launch_execution_plan"), (auth_rows, "kernel_launch_authorization"),
        (precondition_rows, "kernel_launch_precondition"), (artifact_rows, "kernel_build_artifact"),
        (parameter_rows, "kernel_launch_parameter"), (stream_rows, "kernel_stream_synchronization"),
...
def _row_blockers(rows: list[Mapping[str, Any]], kind: str) -> list[str]:
    if not rows:
        return [f"v5_p62_{kind}_inventory_missing"]
    blocked: list[str] = []
    for index, row in enumerate(rows):
        item_id = str(row.get("plan_id") or row.get("check_id") or row.get("id") or f"{kind}_{index}")
        if row.get("ready") is not True:
            blocked.append(f"v5_p62_{kind}_not_ready:{item_id}")
        if not _source(row):
            blocked.append(f"v5_p62_{kind}_source_missing:{item_id}")
...
def _unsafe_claims(value: Mapping[str, Any], owner: str) -> list[str]:
    ...
    for field in UNSAFE_TRUE_FIELDS:
        if value.get(field) is True:
            blocked.append(f"v5_p62_unsafe_claim:{owner}:{field}")
...
def _review_template() -> dict[str, Any]:
    template = {"reviewer": "", "reviewed_at": "", "requested_scope": P62_SCOPE, "approve_kernel_launch_execution_contract": False}
    for field in UNSAFE_REVIEW_APPROVAL_FIELDS:
...
def _allowed_next_actions(decision: str, blockers: list[str]) -> list[str]:
    if decision == P62_READY_DECISION:
        return ["archive_p62_kernel_launch_execution_contract"]
    if decision == P62_REJECTED_DECISION:
        return ["record_p62_default_off_rejection_or_repair_kernel_launch_execution"]
    if decision == P62_HOLD_DECISION:
        return ["collect_signed_kernel_launch_execution_review"]
    if any("p61" in item for item in blockers):
        return ["repair_p61_native_dispatch_execution_contract"]
    if any("kernel" in item or "parity" in item or "tensor" in item for item in blockers):
        return ["repair_kernel_launch_execution_evidence"]
    return ["clear_failure_or_rollback_history_before_p62_contract"]


def _recommended_next_step(decision: str, blockers: list[str]) -> str:
    if decision == P62_READY_DECISION:
        return "archive P62 contract; parity execution still requires a later explicit contract"
    if decision == P62_REJECTED_DECISION:
        return "record the signed rejection and keep kernel launch execution default-off for repair"
    if decision == P62_HOLD_DECISION:
        return "collect a signed kernel launch execution review over P61 evidence"
    if any("p61" in item for item in blockers):
        return "repair the P61 native dispatch execution contract before P62"
    return "hold P62 until evidence, review acknowledgements, and histories are clear"


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build V5 P62 kernel launch execution contract.")
    parser.add_argument("--p61-native-dispatch-execution", default="", help="P61 native dispatch execution JSON.")
    parser.add_argument("--kernel-launch-execution-evidence", default="", help="P62 evidence JSON.")
    parser.add_argument("--kernel-launch-execution-review", default="", help="Signed P62 review JSON.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    report = build_v5_kernel_launch_execution_contract(
        p61_native_dispatch_execution=load_json(args.p61_native_dispatch_execution)
        if args.p61_native_dispatch_execution else None,
...
__all__ = ["build_v5_kernel_launch_execution_contract"]
