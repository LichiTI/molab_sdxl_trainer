"""Default-off BCOS Triton TrainingLoop executor for selected plugin canaries."""
...
class BcosTrainingExecutorConfig:
    optimizer_kind: str = "bcos"
    lr: float = 1.0e-3
    beta: float = 0.9
...
class BcosTrainingExecutor:
    """Launch a real BCOS mode-g fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: BcosTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("BcosTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("bcos_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("bcos_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("bcos_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"bcos_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("bcos_maximize_not_supported")
        ...
        if not cases:
            blockers.append("bcos_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_bcos_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "bcos_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("bcos_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("bcos_training_executor_requires_contiguous_tensors")
        state = self.optimizer.state[param]
        v = _state_tensor(state, "v", param, fill_from=param.grad.detach().square())
        before = param.detach().clone()
        beta_v = _beta_v(group_cfg)
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _bcos_g_dense_kernel[grid](
            param,
            param.grad.detach(),
            v,
            int(param.numel()),
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["bcos_training_executor_parameters_not_mutated"],
        }


def build_plugin_bcos_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: BcosTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> BcosTrainingExecutor:
    return BcosTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: BcosTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> BcosTrainingExecutorConfig:
    if isinstance(value, BcosTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    beta2 = payload.get("beta2", group.get("beta2", None))
    return BcosTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "bcos"),
        ...
        mode=str(payload.get("mode", getattr(optimizer, "mode", "g")) or "g"),
        simple_cond=bool(payload.get("simple_cond", getattr(optimizer, "simple_cond", False))),
...
def _group_config(
    group: Mapping[str, Any],
    config: BcosTrainingExecutorConfig,
    optimizer: torch.optim.Optimizer,
) -> dict[str, Any]:
    beta2 = group.get("beta2", config.beta2)
    return {
        "lr": float(group.get("lr", config.lr) or config.lr),
        "beta": float(group.get("beta", config.beta) or config.beta),
        "beta2": None if beta2 is None else float(beta2),
        "mode": str(getattr(optimizer, "mode", config.mode) or config.mode),
        "simple_cond": bool(getattr(optimizer, "simple_cond", config.simple_cond)),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if str(group_cfg.get("mode", "g")) != "g":
        blockers.append("bcos_only_mode_g_supported_for_canary")
    if bool(group_cfg.get("simple_cond", False)):
        blockers.append("bcos_simple_cond_not_supported_for_canary")
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("bcos_weight_decay_not_supported_for_canary")
    return blockers


def _beta_v(group_cfg: Mapping[str, Any]) -> float:
    beta2 = group_cfg.get("beta2", None)
    return float(group_cfg.get("beta", 0.9) if beta2 is None else beta2)
...
__all__ = ["BcosTrainingExecutor", "BcosTrainingExecutorConfig", "build_plugin_bcos_training_executor"]
