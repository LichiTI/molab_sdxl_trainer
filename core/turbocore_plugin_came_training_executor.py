"""Default-off CAME Triton TrainingLoop executor for selected plugin canaries."""
...
class CameTrainingExecutorConfig:
    optimizer_kind: str = "came"
    lr: float = 2.0e-4
    beta1: float = 0.9
    beta2: float = 0.999
    beta3: float = 0.9999
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _came_first_step_1d_kernel(
        param_ptr,
        grad_ptr,
        exp_avg_ptr,
        exp_avg_sq_ptr,
        ...
        squared_grad = grad * grad + eps1
        exp_avg_sq_new = exp_avg_sq * beta2 + squared_grad * (1.0 - beta2)
        update = tl.rsqrt(exp_avg_sq_new) * grad
        rms = tl.sqrt(tl.sum(tl.where(mask, update * update, 0.0), axis=0) / n_elements)
        clip_denom = tl.maximum(rms / clip_threshold, 1.0)
        update = update / clip_denom
        exp_avg_new = exp_avg * beta1 + update * (1.0 - beta1)
        exp_avg_after_step = exp_avg_new * lr
        param_new = param - exp_avg_after_step

        tl.store(param_ptr + offsets, param_new, mask=mask)
        tl.store(exp_avg_ptr + offsets, exp_avg_after_step, mask=mask)
        tl.store(exp_avg_sq_ptr + offsets, exp_avg_sq_new, mask=mask)


class CameTrainingExecutor:
    """Launch a real CAME first-step 1D fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: CameTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("CameTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("came_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("came_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("came_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"came_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("came_maximize_not_supported")
        ...
        if not cases:
            blockers.append("came_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_came_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "came_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.ndim != 1:
            return _case_failed("came_training_executor_requires_1d_non_factored_param")
        if int(param.numel()) > int(group_cfg["block_size"]):
            return _case_failed("came_training_executor_requires_single_block_canary")
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("came_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("came_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("came_training_executor_only_first_step_supported_for_canary")
        state = self.optimizer.state[param]
        exp_avg = _state_tensor(state, "exp_avg", param)
        exp_avg_sq = _state_tensor(state, "exp_avg_sq", param)
        state["RMS"] = _rms(param.detach())
        before = param.detach().clone()
        _came_first_step_1d_kernel[(1,)](
            param,
            param.grad.detach(),
            exp_avg,
            exp_avg_sq,
            ...
            float(group_cfg["clip_threshold"]),
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["came_training_executor_parameters_not_mutated"],
        }


def build_plugin_came_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: CameTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> CameTrainingExecutor:
    return CameTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: CameTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> CameTrainingExecutorConfig:
    if isinstance(value, CameTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999, 0.9999)))
    beta1, beta2, beta3 = _three_betas(betas)
    return CameTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "came"),
        lr=float(payload.get("lr", group.get("lr", 2.0e-4)) or 2.0e-4),
        beta1=beta1,
        beta2=beta2,
        beta3=beta3,
        ...
        clip_threshold=float(payload.get("clip_threshold", group.get("clip_threshold", 1.0)) or 1.0),
        ams_bound=bool(payload.get("ams_bound", group.get("ams_bound", False))),
        eps1=float(payload.get("eps1", group.get("eps1", 1.0e-30)) or 1.0e-30),
        eps2=float(payload.get("eps2", group.get("eps2", 1.0e-16)) or 1.0e-16),
        block_size=int(payload.get("block_size", 1024) or 1024),
        require_native_cuda=bool(payload.get("require_native_cuda", True)),
    )


def _group_config(group: Mapping[str, Any], config: CameTrainingExecutorConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "clip_threshold": float(group.get("clip_threshold", config.clip_threshold) or config.clip_threshold),
...
def _three_betas(value: Any) -> tuple[float, float, float]:
    if not isinstance(value, (tuple, list)):
        value = (0.9, 0.999, 0.9999)
    return (
        float(value[0] if len(value) > 0 else 0.9),
        float(value[1] if len(value) > 1 else 0.999),
        float(value[2] if len(value) > 2 else 0.9999),
    )
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if bool(group_cfg.get("ams_bound", False)):
        blockers.append("came_ams_bound_not_supported_for_canary")
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("came_weight_decay_not_supported_for_canary")
    if float(group_cfg.get("clip_threshold", 1.0) or 1.0) <= 0.0:
        blockers.append("came_clip_threshold_must_be_positive")
    return blockers
...
def _rms(value: torch.Tensor) -> torch.Tensor:
    return value.norm(2) / math.sqrt(value.numel())
...
__all__ = [
    "CameTrainingExecutor",
    "CameTrainingExecutorConfig",
    "build_plugin_came_training_executor",
]
