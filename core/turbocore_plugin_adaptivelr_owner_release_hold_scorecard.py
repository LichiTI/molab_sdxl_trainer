"""Owner/release hold for selected adaptive-LR plugin optimizers."""
...
HOLD_KIND = "plugin_adaptivelr_owner_release_hold_v0"


def build_plugin_adaptivelr_owner_release_hold_scorecard(
    *,
    family_batch_report: Mapping[str, Any] | None = None,
    write_artifact: bool = False,
) -> dict[str, Any]:
    """Record a default-off hold after adaptive-LR state-machine matrices are ready."""

    batch = _as_dict(family_batch_report or build_plugin_adaptivelr_family_batch_scorecard())
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_adaptivelr_owner_release_hold_scorecard_v0",
        "gate": "plugin_adaptivelr_owner_release_hold",
        "ok": ready,
        "promotion_ready": False,
        "owner_release_hold_ready": ready,
        "family_batch_ready": batch.get("selected_adaptivelr_family_batch_ready") is True,
        ...
        "promotion_blockers": _dedupe(
            blockers
            + [
                "plugin_adaptivelr_owner_approval_missing",
                "plugin_adaptivelr_release_approval_missing",
                "plugin_adaptivelr_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "add adaptive-LR request/schema/UI non-exposure guard before product dispatch wiring"
            if ready
            else "fix adaptive-LR owner/release hold blockers"
        ),
        "notes": [
            "This gate records a hold state only; it does not approve native dispatch.",
            "Adaptive-LR evidence remains report-only until owner/release approval.",
...
def _hold_manifest(batch: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "hold_kind": HOLD_KIND,
        "optimizer_family": "adaptive_lr_state_machine",
        ...
        "required_approval_artifacts": [
            ...
            "adaptive_lr_state_machine_review",
...
def _validations(batch: Mapping[str, Any], hold: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _validation(
            "family_batch_ready",
            batch.get("selected_adaptivelr_family_batch_ready") is True,
            "plugin_adaptivelr_family_batch_missing",
        ),
        _validation(
            "optimizer_set_complete",
            len(hold.get("selected_optimizer_names", [])) == optimizer_count == 6,
            "plugin_adaptivelr_optimizer_set_incomplete",
        ),
        _validation(
            "implementation_matrices_ready",
            int(summary.get("selected_state_machine_reference_ready_count", 0) or 0) == optimizer_count
            and int(summary.get("selected_state_machine_abi_implementation_ready_count", 0) or 0) == optimizer_count
            and int(summary.get("selected_native_kernel_preconditions_implementation_ready_count", 0) or 0)
            == optimizer_count
            and int(summary.get("selected_state_machine_replay_matrix_implementation_ready_count", 0) or 0)
            == optimizer_count
            and int(summary.get("selected_state_machine_replay_case_implementation_ready_count", 0) or 0)
            == int(summary.get("selected_state_machine_replay_case_planned_count", 0) or 0)
            and int(summary.get("selected_state_machine_replay_resume_case_implementation_ready_count", 0) or 0)
            == int(summary.get("selected_state_machine_replay_resume_case_planned_count", 0) or 0),
            "plugin_adaptivelr_implementation_matrices_not_ready",
        ),
        _validation(
        ...
        _validation(
            ...
            "plugin_adaptivelr_batch_enabled_dispatch",
        ),
    ]
...
def _write_artifact(report: Mapping[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_plugin_adaptivelr_owner_release_hold_scorecard.json"
...
__all__ = ["build_plugin_adaptivelr_owner_release_hold_scorecard"]
