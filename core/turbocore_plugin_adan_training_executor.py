"""Default-off Adan Triton TrainingLoop executor for selected plugin canaries."""
...
class AdanTrainingExecutorConfig:
    optimizer_kind: str = "adan"
    lr: float = 1.0e-3
    beta1: float = 0.98
    beta2: float = 0.92
    beta3: float = 0.99
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _adan_first_step_dense_kernel(
        param_ptr,
        grad_ptr,
        exp_avg_ptr,
        exp_avg_sq_ptr,
        ...
        d = tl.load(exp_avg_diff_ptr + offsets, mask=mask, other=0.0).to(tl.float32)
        prev = tl.load(previous_grad_ptr + offsets, mask=mask, other=0.0).to(tl.float32)

        grad_diff = prev + g
        m_new = m * beta1 + g * (1.0 - beta1)
        d_new = d * beta2 + grad_diff * (1.0 - beta2)
        grad_diff_for_v = grad_diff * beta2 + g
        v_new = v * beta3 + grad_diff_for_v * grad_diff_for_v * (1.0 - beta3)
        denom = tl.sqrt(v_new) / bias_correction3_sq + eps
        p_new = p - step_size1 * m_new / denom - step_size2 * d_new / denom

        tl.store(exp_avg_ptr + offsets, m_new, mask=mask)
        tl.store(exp_avg_sq_ptr + offsets, v_new, mask=mask)
        tl.store(exp_avg_diff_ptr + offsets, d_new, mask=mask)
        tl.store(previous_grad_ptr + offsets, -g, mask=mask)
        tl.store(param_ptr + offsets, p_new, mask=mask)


class AdanTrainingExecutor:
    """Launch a real Adan first-step fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: AdanTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AdanTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("adan_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("adan_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("adan_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"adan_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("adan_maximize_not_supported")
        ...
        if not cases:
            blockers.append("adan_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_adan_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "adan_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("adan_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("adan_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("adan_training_executor_only_first_step_supported_for_canary")
        state = self.optimizer.state[param]
        exp_avg = _state_tensor(state, "exp_avg", param)
        exp_avg_sq = _state_tensor(state, "exp_avg_sq", param)
        exp_avg_diff = _state_tensor(state, "exp_avg_diff", param)
        previous_grad = _previous_grad_tensor(state, param)
        before = param.detach().clone()
        beta1 = float(group_cfg["beta1"])
        beta2 = float(group_cfg["beta2"])
        beta3 = float(group_cfg["beta3"])
        bias_correction1 = 1.0 - math.pow(beta1, step)
        bias_correction2 = 1.0 - math.pow(beta2, step)
        bias_correction3_sq = math.sqrt(1.0 - math.pow(beta3, step))
        step_size1 = float(group_cfg["lr"]) / bias_correction1
        step_size2 = float(group_cfg["lr"]) * beta2 / bias_correction2
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _adan_first_step_dense_kernel[grid](
            param,
            param.grad.detach(),
            exp_avg,
            exp_avg_sq,
        ...
        state["exp_avg_diff"] = exp_avg_diff
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["adan_training_executor_parameters_not_mutated"],
        }


def build_plugin_adan_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AdanTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> AdanTrainingExecutor:
    return AdanTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AdanTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AdanTrainingExecutorConfig:
    if isinstance(value, AdanTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.98, 0.92, 0.99)))
    beta3 = betas[2] if len(betas) > 2 else 0.99
    return AdanTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "adan"),
        ...
        max_grad_norm=float(payload.get("max_grad_norm", group.get("max_grad_norm", 0.0)) or 0.0),
        use_gc=bool(payload.get("use_gc", group.get("use_gc", False))),
        adanorm=bool(payload.get("adanorm", group.get("adanorm", False))),
...
def _group_config(group: Mapping[str, Any], config: AdanTrainingExecutorConfig) -> dict[str, Any]:
    betas = group.get("betas", (config.beta1, config.beta2, config.beta3))
    beta3 = betas[2] if len(betas) > 2 else config.beta3
    return {
        ...
        "max_grad_norm": float(group.get("max_grad_norm", config.max_grad_norm) or config.max_grad_norm),
        "use_gc": bool(group.get("use_gc", config.use_gc)),
        "adanorm": bool(group.get("adanorm", config.adanorm)),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("adan_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("weight_decouple", False)):
        blockers.append("adan_weight_decouple_not_supported_for_canary")
    if abs(float(group_cfg.get("max_grad_norm", 0.0) or 0.0)) > 0.0:
        blockers.append("adan_max_grad_norm_not_supported_for_canary")
    if bool(group_cfg.get("use_gc", False)):
        blockers.append("adan_gradient_centralization_not_supported_for_canary")
    if bool(group_cfg.get("adanorm", False)):
        blockers.append("adan_adanorm_not_supported_for_canary")
    return blockers
...
def _previous_grad_tensor(state: dict[str, Any], param: torch.nn.Parameter) -> torch.Tensor:
    existing = state.get("previous_grad")
    if torch.is_tensor(existing) and tuple(existing.shape) == tuple(param.shape):
        if existing.device == param.device and existing.dtype == torch.float32 and existing.is_contiguous():
            return existing
        converted = existing.detach().to(device=param.device, dtype=torch.float32).contiguous()
        state["previous_grad"] = converted
        return converted
    grad = param.grad.detach().to(dtype=torch.float32).contiguous()
    state["previous_grad"] = -grad
    return state["previous_grad"]
...
__all__ = ["AdanTrainingExecutor", "AdanTrainingExecutorConfig", "build_plugin_adan_training_executor"]
