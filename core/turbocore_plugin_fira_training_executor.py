"""Default-off Fira Triton TrainingLoop executor for selected plugin canaries."""
...
class FiraTrainingExecutorConfig:
    optimizer_kind: str = "fira"
    lr: float = 1.0e-3
    beta1: float = 0.9
    beta2: float = 0.999
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _fira_first_step_dense_kernel(
        param_ptr,
        grad_ptr,
        exp_avg_ptr,
        exp_avg_sq_ptr,
...
class FiraTrainingExecutor:
    """Launch a real Fira first-step fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: FiraTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("FiraTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("fira_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("fira_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("fira_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"fira_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("fira_maximize_not_supported")
        ...
        if not cases:
            blockers.append("fira_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_fira_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "fira_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("fira_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("fira_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("fira_training_executor_only_first_step_supported_for_canary")
        ...
        _fira_first_step_dense_kernel[grid](
            param,
            param.grad.detach(),
            exp_avg,
            exp_avg_sq,
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["fira_training_executor_parameters_not_mutated"],
        }


def build_plugin_fira_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: FiraTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> FiraTrainingExecutor:
    return FiraTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: FiraTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> FiraTrainingExecutorConfig:
    if isinstance(value, FiraTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return FiraTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "fira"),
...
def _group_config(group: Mapping[str, Any], config: FiraTrainingExecutorConfig) -> dict[str, Any]:
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("fira_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("has_rank", False)):
        blockers.append("fira_galore_rank_not_supported_for_canary")
    return blockers
...
__all__ = ["FiraTrainingExecutor", "FiraTrainingExecutorConfig", "build_plugin_fira_training_executor"]
