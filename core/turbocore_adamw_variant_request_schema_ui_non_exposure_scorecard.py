"""Request/schema/UI non-exposure audit for AdamW variant native canaries."""
...
from core.turbocore_adamw_variant_owner_release_hold_scorecard import (
    build_adamw_variant_owner_release_hold_scorecard,
)


AUDIT_KIND = "adamw_variant_request_schema_ui_non_exposure_v0"
FORBIDDEN_BOUNDARY_TOKENS = (
    "adamw_variant_native",
    "adamw_variant_canary",
    "adamw_variant_dispatch",
    "adamw_variant_owner_release",
    "turbocore_adamw_variant",
    "enable_adamw_variant_kernel",
    "enable_adamw_variant_native",
    "adamw8bit_native",
    "paged_adamw_native",
    "paged_adamw8bit_native",
    "kahan_adamw8bit_native",
    "adamw_schedule_free_native",
)
BOUNDARY_PATHS = (
...
def build_adamw_variant_request_schema_ui_non_exposure_scorecard(
    *,
    ...
    hold_report = _as_dict(owner_release_hold_report or build_adamw_variant_owner_release_hold_scorecard())
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_adamw_variant_request_schema_ui_non_exposure_scorecard_v0",
        "gate": "adamw_variant_request_schema_ui_non_exposure",
        "ok": ready,
        ...
        "promotion_blockers": _dedupe(
            ...
            + [
                ...
                "adamw_variant_request_schema_ui_exposure_not_approved",
                "adamw_variant_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "record explicit owner and release approval before AdamW variant request/schema/UI or dispatch wiring"
            if ready
            else "fix AdamW variant request/schema/UI non-exposure blockers"
        ),
        "notes": [
            "This audit is report-only and does not register request fields, schema fields, UI controls, or routes.",
            "Boundary scanning checks product-facing paths for AdamW variant native exposure tokens.",
...
def _validations(
    ...
    return [
        _validation(
            ...
            "adamw_variant_owner_release_hold_missing",
        ),
        _validation(
            ...
            "adamw_variant_request_schema_ui_unexpected_approval",
        ),
        _validation(
            ...
            "adamw_variant_request_schema_ui_hold_enabled_boundary",
        ),
        _validation(
            "boundary_inventory_present",
            len(boundary.get("present_paths", [])) > 0,
            "adamw_variant_request_schema_ui_boundary_inventory_missing",
        ),
        _validation(
            "forbidden_tokens_absent",
            not findings.get("forbidden_token_hits"),
            "adamw_variant_request_schema_ui_forbidden_boundary_token_found",
        ),
    ]
...
__all__ = ["AUDIT_KIND", "build_adamw_variant_request_schema_ui_non_exposure_scorecard"]
