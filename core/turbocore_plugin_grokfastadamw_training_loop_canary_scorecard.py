"""TrainingLoop canary for selected plugin GrokFastAdamW native dispatch."""
...
SELECTED_OPTIMIZER = "grokfastadamw"
...
def build_plugin_grokfastadamw_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_grokfastadamw_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = _strings(case.get("blocked_reasons"))
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_grokfastadamw_training_loop_canary_scorecard_v0",
        "gate": "plugin_grokfastadamw_training_loop_native_canary",
        "ok": ok,
        ...
        "summary": {
            ...
            "grokfast": case.get("grokfast"),
        },
        "promotion_blockers": blockers
        + [
            "grokfastadamw_e2e_shadow_matrix_refresh_required",
            "grokfastadamw_canary_rollout_policy_refresh_required",
            "product_rollout_review_missing",
        ],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "refresh selected Adam-like e2e shadow matrix and rollout policy for grokfastadamw"
            if ok
            else "fix selected plugin GrokFastAdamW TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    def _fake_train_step(
        ...
        for item in params:
            item.grad = None
        loss = sum(((item.float() * 0.27) ** 2).mean() + item.float().mean() * 0.005 for item in params)
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_grokfastadamw_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_grokfastadamw_training_loop_did_not_emit_step"],
        }
    ...
    ok = bool(
        ...
        and step_after == 2
        and type(optimizer).__name__ == "GrokFastAdamW"
        and str(executor_result.get("optimizer_kind") or "") == "grokfastadamw"
        and group.get("foreach") is False
        and group.get("grokfast") is True
    )
    return {
        "schema_version": 1,
        "ok": ok,
        ...
        "foreach": bool(group.get("foreach", True)),
        "grokfast": bool(group.get("grokfast", False)),
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "runtime_dispatch_ready": False,
        "native_dispatch_allowed": False,
        ...
        if ok
            ...
            _strings(runtime.get("blocked_reasons")) + ["plugin_grokfastadamw_training_loop_native_step_missing"]
        ),
    }
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    param = torch.nn.Parameter(torch.linspace(-0.12, 0.22, steps=4096, device=device, dtype=torch.float32))
    optimizer = _create_selected_grokfastadamw_plugin_optimizer(param)
    loop = TrainingLoop(
...
def _create_selected_grokfastadamw_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
    ...
    trainer.config = LulynxConfig(
        ...
        learning_rate=1e-3,
        weight_decay=0.01,
        optimizer_args=f"{case.optimizer_args},foreach=False,grokfast=True,grokfast_after_step=0",
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin GrokFastAdamW TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_grokfastadamw_training_loop_canary_scorecard"]
