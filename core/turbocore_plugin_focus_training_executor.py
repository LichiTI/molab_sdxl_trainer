"""Default-off FOCUS Triton TrainingLoop executor for selected plugin canaries."""
...
class FocusTrainingExecutorConfig:
    optimizer_kind: str = "focus"
    lr: float = 1.0e-2
    beta1: float = 0.9
    beta2: float = 0.0
    gamma: float = 0.1
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _focus_first_step_dense_kernel(
        param_ptr,
        grad_ptr,
        exp_avg_ptr,
        pbar_ptr,
        ...
        pbar = tl.load(pbar_ptr + offsets, mask=mask, other=0.0).to(tl.float32)

        exp_avg_new = exp_avg * beta1 + g * (1.0 - beta1)
        pbar_new = pbar * beta2 + p * (1.0 - beta2)
        pbar_hat = pbar_new / bias_correction2

        attraction_raw = p - pbar_hat
        attraction_sign = tl.where(attraction_raw > 0.0, 1.0, tl.where(attraction_raw < 0.0, -1.0, 0.0))
        grad_sign = tl.where(exp_avg_new > 0.0, 1.0, tl.where(exp_avg_new < 0.0, -1.0, 0.0))
        update = attraction_sign * gamma + grad_sign
        p_new = p - lr * update

        tl.store(exp_avg_ptr + offsets, exp_avg_new, mask=mask)
        tl.store(pbar_ptr + offsets, pbar_new, mask=mask)
        tl.store(param_ptr + offsets, p_new, mask=mask)


class FocusTrainingExecutor:
    """Launch a real FOCUS first-step fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: FocusTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("FocusTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("focus_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("focus_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("focus_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"focus_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("focus_maximize_not_supported")
        ...
        if not cases:
            blockers.append("focus_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_focus_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "focus_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("focus_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("focus_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("focus_training_executor_only_first_step_supported_for_canary")
        state = self.optimizer.state[param]
        exp_avg = _state_tensor(state, "exp_avg", param)
        pbar = _state_tensor(state, "pbar", param)
        before = param.detach().clone()
        beta2 = float(group_cfg["beta2"])
        bias_correction2 = 1.0 - beta2**step
        if bias_correction2 == 0.0:
            return _case_failed("focus_training_executor_invalid_beta2_bias")
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _focus_first_step_dense_kernel[grid](
            param,
            param.grad.detach(),
            exp_avg,
            pbar,
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["focus_training_executor_parameters_not_mutated"],
        }


def build_plugin_focus_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: FocusTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> FocusTrainingExecutor:
    return FocusTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: FocusTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> FocusTrainingExecutorConfig:
    if isinstance(value, FocusTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.0)))
    return FocusTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "focus"),
        lr=float(payload.get("lr", group.get("lr", 1.0e-2)) or 1.0e-2),
        beta1=float(betas[0]),
        beta2=float(betas[1]),
        gamma=float(payload.get("gamma", group.get("gamma", 0.1)) or 0.1),
...
def _group_config(group: Mapping[str, Any], config: FocusTrainingExecutorConfig) -> dict[str, Any]:
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("focus_weight_decay_not_supported_for_canary")
    return blockers
...
__all__ = ["FocusTrainingExecutor", "FocusTrainingExecutorConfig", "build_plugin_focus_training_executor"]
