"""Runtime enablement execution contract boundary for TurboCore V5-P48.

P48 records future runtime enablement execution evidence after P47; it does not
execute enablement, enable runtime/native dispatch, emit fields, expose UI, or launch training.
"""
...
P47_READY_DECISION = "runtime_enablement_contract_boundary_recorded_default_off"
P48_READY_DECISION = "runtime_enablement_execution_contract_boundary_recorded_default_off"
P48_BLOCKED_DECISION = "runtime_enablement_execution_contract_boundary_blocked_default_off"
P48_HOLD_DECISION = "runtime_enablement_execution_contract_boundary_hold_for_signed_review_default_off"
P48_REJECTED_DECISION = "runtime_enablement_execution_contract_boundary_rejected_default_off"
P48_SCOPE = "runtime_enablement_execution_contract_boundary"
DEFAULT_REQUIRED_SECTIONS = (
    "p47_runtime_enablement_boundary_reference",
    "runtime_enablement_execution_plan_inventory",
    "runtime_enablement_execution_precondition_inventory",
    "runtime_execution_boundary",
    "runtime_adapter_boundary",
    "native_dispatch_boundary",
    "request_adapter_boundary",
    "no_runtime_enablement_execution_boundary",
...
REQUIRED_REVIEW_ACKS = (
    "acknowledge_p47_runtime_enablement_boundary_recorded",
    "acknowledge_default_off_boundary",
    "acknowledge_no_training_launch",
    "acknowledge_no_ui_exposure",
    "acknowledge_no_runtime_enablement_execution",
    "acknowledge_no_runtime_enablement_enabled",
    ...
    "acknowledge_runtime_enablement_execution_evidence_replayable",
...
UNSAFE_NON_EMPTY_FIELDS = (
    "post_p47_request_fields",
    "post_p48_request_fields",
    "post_runtime_enablement_execution_fields",
...
def build_v5_runtime_enablement_execution_contract_boundary(
    *,
    p47_runtime_enablement_contract_boundary: Mapping[str, Any] | None = None,
    runtime_enablement_execution_evidence: Mapping[str, Any] | None = None,
    runtime_enablement_execution_review: Mapping[str, Any] | None = None,
    failure_history: Sequence[Any] | None = None,
    rollback_history: Sequence[Any] | None = None,
) -> dict[str, Any]:
    """Record runtime enablement execution evidence without executing it."""

    p47 = _as_dict(p47_runtime_enablement_contract_boundary)
    evidence = _as_dict(runtime_enablement_execution_evidence)
    review = _as_dict(runtime_enablement_execution_review)
    failure_events = _event_list(failure_history)
    rollback_events = _event_list(rollback_history)
    p47_summary = _p47_summary(p47)
    evidence_summary = _evidence_summary(evidence)
    review_summary = _review_summary(review)
    evidence_blockers = _evidence_blockers(p47_summary, evidence_summary, failure_events, rollback_events)
    ...
    if decision == P48_HOLD_DECISION:
        blockers.append("v5_p48_signed_runtime_enablement_execution_review_missing")
    blockers = _dedupe(blockers)
    ready = decision == P48_READY_DECISION
    rejected = decision == P48_REJECTED_DECISION
    decision_record_ready = ready or rejected
    return {
        "schema_version": 1,
        "package": "turbocore_v5_runtime_enablement_execution_contract_boundary_v0",
        "gate": "v5_runtime_enablement_execution_contract_boundary",
        "ok": decision_record_ready,
        "decision_record_ready": decision_record_ready,
        "runtime_enablement_execution_review_recorded": decision_record_ready,
        "runtime_enablement_execution_review_signed": decision_record_ready,
        "runtime_enablement_execution_contract_boundary_ready": ready,
        "runtime_enablement_execution_evidence_recorded": ready,
        ...
        **{field: False for field in UNSAFE_TRUE_FIELDS},
        "post_p48_request_fields": {},
        "p47_runtime_enablement_boundary_summary": p47_summary,
        "runtime_enablement_execution_evidence_summary": evidence_summary,
        "runtime_enablement_execution_review": review_summary,
        "runtime_enablement_execution_review_template": _review_template(evidence_summary),
        "progress_gates": _progress_gates(p47_summary, evidence_summary, review_summary),
        ...
        "notes": [
            "P48 records runtime enablement execution boundary evidence only.",
            "P48 does not execute runtime enablement, enable runtime/native dispatch, emit request fields, or launch training.",
            "A later explicit runtime dispatch contract is still required before runtime behavior can become active.",
        ],
    }


def _p47_summary(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "runtime_enablement_contract_boundary_ready": report.get("runtime_enablement_contract_boundary_ready") is True,
        "runtime_enablement_evidence_recorded": report.get("runtime_enablement_evidence_recorded") is True,
        "runtime_enablement_review_signed": report.get("runtime_enablement_review_signed") is True,
        "decision": decision,
        ...
        "runtime_enablement_enabled": report.get("runtime_enablement_enabled"),
...
def _evidence_summary(evidence: Mapping[str, Any]) -> dict[str, Any]:
    ...
    plan_rows = _rows(evidence, "runtime_enablement_execution_plan_inventory", "execution_plan_inventory")
    precondition_rows = _rows(evidence, "runtime_enablement_execution_precondition_inventory", "execution_precondition_inventory")
    ...
    return {
        ...
        "ready": not blockers,
        "runtime_enablement_execution_contract_boundary_ready": evidence.get(
        ...
        "unsafe_claims": _unsafe_claims(evidence, "runtime_enablement_execution"),
        "blockers": blockers,
    }
...
def _review_summary(review: Mapping[str, Any]) -> dict[str, Any]:
    summary = {
        ...
        "approve_runtime_enablement_execution_boundary": review.get("approve_runtime_enablement_execution_boundary")
        is True,
    }
...
def _progress_gates(p47_summary: Mapping[str, Any], evidence_summary: Mapping[str, Any], review_summary: Mapping[str, Any]) -> dict[str, bool]:
    return {
        "p47_runtime_enablement_boundary_present": bool(p47_summary.get("present", False)),
        "p47_runtime_enablement_boundary_ready": _p47_ready(p47_summary),
        "runtime_enablement_execution_evidence_present": bool(evidence_summary.get("present", False)),
        "runtime_enablement_execution_evidence_ready": bool(evidence_summary.get("ready", False)),
        "signed_runtime_enablement_execution_review_present": bool(review_summary.get("present", False)),
        "reviewer_present": bool(review_summary.get("reviewer")),
        "reviewed_at_present": bool(review_summary.get("reviewed_at")),
        "requested_scope_valid": review_summary.get("requested_scope") == P48_SCOPE,
...
def _decision(evidence_blockers: list[str], review: Mapping[str, Any], review_blockers: list[str]) -> str:
    if evidence_blockers:
        return P48_BLOCKED_DECISION
    if not review:
        return P48_HOLD_DECISION
    if review_blockers:
        return P48_BLOCKED_DECISION
    if review.get("approve_runtime_enablement_execution_boundary") is True:
        return P48_READY_DECISION
    return P48_REJECTED_DECISION


def _evidence_blockers(
    p47_summary: Mapping[str, Any],
    evidence_summary: Mapping[str, Any],
    failure_events: list[str],
    rollback_events: list[str],
) -> list[str]:
    blocked: list[str] = []
    if not bool(p47_summary.get("present", False)):
        blocked.append("v5_p48_p47_runtime_enablement_boundary_missing")
    elif not _p47_ready(p47_summary):
        blocked.append("v5_p48_p47_runtime_enablement_boundary_not_ready")
        blocked.extend(_string_list(p47_summary.get("blocked_reasons")))
        blocked.extend(_string_list(p47_summary.get("promotion_blockers")))
        blocked.extend(_string_list(p47_summary.get("unsafe_claims")))
    if not bool(evidence_summary.get("present", False)):
        blocked.append("v5_p48_runtime_enablement_execution_evidence_missing")
    elif not bool(evidence_summary.get("ready", False)):
        blocked.extend(_string_list(evidence_summary.get("blockers")))
    for event in failure_events:
        blocked.append(f"v5_p48_failure_history_not_clear:{event}")
    for event in rollback_events:
        blocked.append(f"v5_p48_rollback_history_not_clear:{event}")
...
def _review_blockers(review: Mapping[str, Any]) -> list[str]:
    ...
    if not review.get("reviewer"):
        blocked.append("v5_p48_reviewer_missing")
    if not review.get("reviewed_at"):
        blocked.append("v5_p48_reviewed_at_missing")
    if review.get("requested_scope") != P48_SCOPE:
        blocked.append("v5_p48_requested_scope_invalid")
    for field in UNSAFE_REVIEW_APPROVAL_FIELDS:
        if review.get(field):
            blocked.append(f"v5_p48_unsafe_review_approval:{field}")
    for field in REQUIRED_REVIEW_ACKS:
        if not review.get(field):
            blocked.append(f"v5_p48_review_ack_missing:{field}")
    return _dedupe(blocked)


def _p47_ready(summary: Mapping[str, Any]) -> bool:
    return bool(
        summary.get("present")
        and summary.get("ok")
        and summary.get("decision_record_ready")
        and summary.get("runtime_enablement_contract_boundary_ready")
        and summary.get("runtime_enablement_evidence_recorded")
        and summary.get("runtime_enablement_review_signed")
        and summary.get("decision") == P47_READY_DECISION
        ...
        and summary.get("request_adapter_enabled") is False
        and summary.get("runtime_enablement_enabled") is False
...
def _evidence_blocker_list(
    ...
    if evidence.get("ok") is not True:
        blocked.append("v5_p48_runtime_enablement_execution_not_ok")
    if evidence.get("runtime_enablement_execution_contract_boundary_ready") is not True:
        blocked.append("v5_p48_runtime_enablement_execution_evidence_not_ready")
    for field in (
        "report_only",
        "boundary_only",
        "contract_only",
        ...
        if evidence.get(field) is not True:
            blocked.append(f"v5_p48_runtime_enablement_execution_{field}_missing")
    if evidence.get("default_off") is not True or not _default_off_confirmed(evidence):
        blocked.append("v5_p48_runtime_enablement_execution_default_off_violation")
    if evidence.get("request_adapter_off") is not True or not _request_adapter_off(evidence):
        blocked.append("v5_p48_runtime_enablement_execution_request_adapter_violation")
    if not _digest(evidence):
        blocked.append("v5_p48_runtime_enablement_execution_digest_missing")
    if not _source(evidence):
        blocked.append("v5_p48_runtime_enablement_execution_source_missing")
    for section in missing_sections:
        blocked.append(f"v5_p48_runtime_enablement_execution_section_missing:{section}")
    blocked.extend(_execution_plan_blockers(plan_rows))
    blocked.extend(_precondition_blockers(precondition_rows))
    blocked.extend(_unsafe_claims(evidence, "runtime_enablement_execution"))
...
def _execution_plan_blockers(rows: list[Mapping[str, Any]]) -> list[str]:
    blocked: list[str] = []
    if not rows:
        return ["v5_p48_runtime_enablement_execution_plan_inventory_missing"]
    for index, row in enumerate(rows):
        ...
        for field in (
            ...
            if row.get(field) is True:
                blocked.append(f"v5_p48_runtime_enablement_execution_claim:{plan_id}:{field}")
...
def _precondition_blockers(rows: list[Mapping[str, Any]]) -> list[str]:
    blocked: list[str] = []
    if not rows:
        return ["v5_p48_runtime_enablement_execution_precondition_inventory_missing"]
    for index, row in enumerate(rows):
        check_id = str(row.get("check_id") or row.get("id") or f"check_{index}")
        for field in (
            "runtime_enablement_execution_precondition_active",
            "runtime_enablement_execution_check_registered",
            "runtime_enablement_execution_check_enabled",
        ):
            if row.get(field) is True:
                blocked.append(f"v5_p48_runtime_enablement_execution_precondition_claim:{check_id}:{field}")
...
def _unsafe_claims(value: Mapping[str, Any], owner: str) -> list[str]:
    ...
    for field in UNSAFE_TRUE_FIELDS:
        if value.get(field) is True:
            blocked.append(f"v5_p48_unsafe_claim:{owner}:{field}")
...
def _review_template(evidence_summary: Mapping[str, Any]) -> dict[str, Any]:
    template = {"reviewer": "", "reviewed_at": "", "requested_scope": P48_SCOPE, "approve_runtime_enablement_execution_boundary": False}
    for field in UNSAFE_REVIEW_APPROVAL_FIELDS:
...
def _allowed_next_actions(decision: str, blockers: list[str]) -> list[str]:
    if decision == P48_READY_DECISION:
        return ["archive_p48_runtime_enablement_execution_contract_boundary"]
    if decision == P48_REJECTED_DECISION:
        return ["record_p48_default_off_rejection_or_repair_runtime_enablement_execution_evidence"]
    if decision == P48_HOLD_DECISION:
        return ["collect_signed_runtime_enablement_execution_review"]
    if any("p47" in item for item in blockers):
        return ["repair_p47_runtime_enablement_boundary"]
    if any("runtime_enablement_execution" in item or "precondition" in item for item in blockers):
        return ["repair_runtime_enablement_execution_evidence"]
    return ["clear_failure_or_rollback_history_before_runtime_enablement_execution_boundary"]


def _recommended_next_step(decision: str, blockers: list[str]) -> str:
    if decision == P48_READY_DECISION:
        return "archive P48 boundary; runtime dispatch still requires a later explicit contract"
    if decision == P48_REJECTED_DECISION:
        return "record the signed rejection and keep runtime enablement execution default-off for repair"
    if decision == P48_HOLD_DECISION:
        return "collect a signed runtime enablement execution review over P47 evidence"
    if any("p47" in item for item in blockers):
        return "repair the P47 runtime enablement boundary before P48"
    if any("digest_missing" in item or "source_missing" in item for item in blockers):
        return "collect replayable runtime enablement execution source and digest evidence"
    return "hold P48 until runtime enablement execution evidence, review acknowledgements, and histories are clear"


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build V5 P48 runtime enablement execution boundary.")
    parser.add_argument("--p47-runtime-enablement-contract-boundary", default="", help="P47 boundary JSON.")
    parser.add_argument("--runtime-enablement-execution-evidence", default="", help="P48 execution evidence JSON.")
    parser.add_argument("--runtime-enablement-execution-review", default="", help="Signed P48 review JSON.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    report = build_v5_runtime_enablement_execution_contract_boundary(
        p47_runtime_enablement_contract_boundary=load_json(args.p47_runtime_enablement_contract_boundary)
        if args.p47_runtime_enablement_contract_boundary
        else None,
        runtime_enablement_execution_evidence=load_json(args.runtime_enablement_execution_evidence)
        if args.runtime_enablement_execution_evidence
        else None,
        runtime_enablement_execution_review=load_json(args.runtime_enablement_execution_review)
        if args.runtime_enablement_execution_review
        else None,
