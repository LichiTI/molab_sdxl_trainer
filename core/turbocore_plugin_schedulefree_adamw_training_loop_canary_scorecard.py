"""TrainingLoop canary for selected plugin ScheduleFreeAdamW native dispatch."""
...
SELECTED_OPTIMIZER = "schedulefreeadamw"
...
def build_plugin_schedulefree_adamw_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_schedulefreeadamw_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = _strings(case.get("blocked_reasons"))
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_schedulefree_adamw_training_loop_canary_scorecard_v0",
        "gate": "plugin_schedulefreeadamw_training_loop_native_canary",
        "ok": ok,
        ...
        + [
            "schedulefreeadamw_e2e_shadow_matrix_refresh_required",
            "schedulefreeadamw_canary_rollout_policy_refresh_required",
            "product_rollout_review_missing",
        ],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "refresh selected schedule-free e2e shadow matrix and rollout policy for schedulefreeadamw"
            if ok
            else "fix selected plugin ScheduleFreeAdamW TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    def _fake_train_step(
        ...
        for item in params:
            item.grad = None
        loss = sum(((item.float() * 0.41) ** 2).mean() + item.float().mean() * 0.009 for item in params)
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_schedulefreeadamw_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_schedulefreeadamw_training_loop_did_not_emit_step"],
        }
    ...
    ok = bool(
        ...
        and step_after == 2
        and type(optimizer).__name__ == "ScheduleFreeAdamW"
        and str(executor_result.get("optimizer_kind") or "") == "adamw_schedule_free"
    )
    return {
        "schema_version": 1,
        "ok": ok,
        ...
        "blocked_reasons": [] if ok else _dedupe(_strings(runtime.get("blocked_reasons")) + [
            "plugin_schedulefreeadamw_training_loop_native_step_missing"
        ]),
    }
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    param = torch.nn.Parameter(torch.linspace(-0.21, 0.31, steps=4096, device=device, dtype=torch.float32))
    optimizer = _create_selected_schedulefreeadamw_plugin_optimizer(param)
    loop = TrainingLoop(
...
def _create_selected_schedulefreeadamw_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
...
def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> None:
    if hasattr(optimizer, "train"):
        optimizer.train()
    param.grad = None
    loss = ((param.float() * 0.29) ** 2).mean() + param.float().mean() * 0.006
    loss.backward()
    optimizer.step()
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin ScheduleFreeAdamW TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_schedulefree_adamw_training_loop_canary_scorecard"]
