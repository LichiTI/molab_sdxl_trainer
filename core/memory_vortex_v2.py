"""
Vortex Memory Manager V2 | [R&D]
负责管理模型权重在 CPU Pinned Memory 与 GPU 显存之间的动态流转。

V2 Improvements:
- Encapsulated State Management using `VortexState` dataclass
...
logger = logging.getLogger("VortexManagerV2")

# --- V2 State Encapsulation ---
@dataclass
class VortexState:
    """Encapsulated state for Vortex-managed layers"""
    managed: bool = True
    status: Literal["CPU", "GPU", "TRANSFERRING"] = "CPU"
    gpu_cache: Optional[torch.Tensor] = None
...
class VortexManagerV2:
    """
    Vortex 核心管理器 V2
    实现权重的同步/异步弹跳机制 (State Encapsulated)
    """
    _instance = None

    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(VortexManagerV2, cls).__new__(cls)
    ...
    def initialize(self, high_priority: bool = True):
        ...
        with self._lock:
            ...
            logger.info("Vortex Memory Manager V2 initialized with dual streams & LRU Cache.")
    ...
    def register_layer(self, layer: nn.Module):
        ...
        with torch.no_grad():
            if hasattr(layer, "weight") and layer.weight is not None:
                ...
                # [V2] Initialize Vortex State
                layer.vortex_state = VortexState()
    ...
    def evict_until_safe(self, required_bytes: int = 0):
        ...
        max_evictions = len(self.cache_pool) * 2
        eviction_ops = 0
        
        while (self.current_cache_bytes + required_bytes) > threshold_bytes:
            if eviction_ops > max_evictions:
                logger.warning("VortexV2: Eviction infinite loop detected.")
                break
            eviction_ops += 1
            ...
            if hasattr(layer, 'vortex_state') and layer.vortex_state.keep_alive:
                # 重新塞回尾部 (最近使用)，避免死循环
            ...
            # 更新状态
            if hasattr(layer, 'vortex_state'):
                layer.vortex_state.gpu_cache = None
                layer.vortex_state.status = "CPU"
    ...
    def cache_layer(self, layer: nn.Module):
        ...
        将层加入 LRU Cache
        """
        if not hasattr(layer, 'vortex_state') or layer.vortex_state.gpu_cache is None:
            return

        # 估算大小
        tensor = layer.vortex_state.gpu_cache
    ...
    def prefetch_layer(self, layer: nn.Module):
        ...
        """
        if not hasattr(layer, 'vortex_state'):
            return

        # 1. Cache Hit Check
        if layer.vortex_state.status == "GPU" or layer in self.cache_pool:
            if layer in self.cache_pool:
                self.cache_pool.move_to_end(layer)
            self.stats["cache_hits"] += 1
            return 
            
        if layer.vortex_state.status != "CPU":
            return
        ...
        layer.vortex_state.gpu_cache = gpu_weight
        layer.vortex_state.event = event
        layer.vortex_state.status = "TRANSFERRING"
    ...
    def ensure_layer_on_gpu(self, layer: nn.Module) -> torch.Tensor:
        ...
        """
        if not hasattr(layer, 'vortex_state'):
            # Fallback for unmanaged layers (shouldn't happen if hooks correct)
            return layer.weight.to(self.device)

        if layer in self.cache_pool:
            self.cache_pool.move_to_end(layer)
            layer.vortex_state.status = "GPU"   # Ensure status sync
            return self.cache_pool[layer]

        if layer.vortex_state.status == "GPU":
            return layer.vortex_state.gpu_cache

        elif layer.vortex_state.status == "TRANSFERRING":
            # 等待搬运完成
            self.compute_stream.wait_event(layer.vortex_state.event)
            layer.vortex_state.status = "GPU"
            return layer.vortex_state.gpu_cache
    ...
    class VortexFunction(torch.autograd.Function):
        ...
        """
        @staticmethod
        def forward(ctx, input_tensor, weight_cpu, bias, manager: 'VortexManagerV2', layer: nn.Module):
            ctx.manager = manager
            ctx.layer = layer
    ...
    def apply_linear(self, layer: nn.Module, input_tensor: torch.Tensor):
        ...
        替代 nn.Linear 的调用
        """
        if not hasattr(layer, 'vortex_state') or not layer.vortex_state.managed:
        ...
        if self._last_accessed_layer is not None and self._last_accessed_layer is not layer:
            # 记录链表关系: Last -> Current
            if hasattr(self._last_accessed_layer, 'vortex_state'):
                self._last_accessed_layer.vortex_state.next_layer = layer
        ...
        # ---------------------------
        if layer.vortex_state.next_layer:
            self.prefetch_layer(layer.vortex_state.next_layer)
...
# 全局单例
vortex_manager_v2 = VortexManagerV2()
