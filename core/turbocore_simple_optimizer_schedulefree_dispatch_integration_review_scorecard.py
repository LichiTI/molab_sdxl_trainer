"""Manual dispatch review package for built-in simple schedule-free canaries."""

from __future__ import annotations

from typing import Any, Mapping

from core.turbocore_simple_optimizer_schedulefree_rollout_policy_scorecard import (
    FALLBACK_BACKEND,
    build_simple_optimizer_schedulefree_rollout_policy_scorecard,
)


REVIEW_KIND = "simple_formula_schedulefree_dispatch_integration_review_v0"
EXPECTED_OPTIMIZERS = {"RAdamScheduleFree", "SGDScheduleFree"}


def build_simple_optimizer_schedulefree_dispatch_integration_review_scorecard(
    *,
    ...
    policy_report = _as_dict(rollout_policy_report or build_simple_optimizer_schedulefree_rollout_policy_scorecard())
    ...
    return {
        "schema_version": 1,
        "scorecard": "turbocore_simple_optimizer_schedulefree_dispatch_integration_review_scorecard_v0",
        "gate": "simple_formula_schedulefree_dispatch_integration_review",
        "ok": ready,
        ...
        "recommended_next_step": (
            "requires explicit owner approval before wiring simple schedule-free canary dispatch"
            if ready
            else "fix simple schedule-free dispatch integration review blockers"
        ),
        "notes": [
            "This gate prepares a real-dispatch integration review only.",
            "Existing schedule-free optimizers remain the training update authority.",
...
def _review_package(policy_report: Mapping[str, Any], mode: str) -> dict[str, Any]:
    ...
    return {
        ...
        "runtime_hook_contract": {
            ...
            "optimizer_train_mode_hook": "schedulefree_optimizer.train",
            "optimizer_eval_mode_hook": "schedulefree_optimizer.eval",
        ...
        "dispatch_contract": {
            ...
            "requires_schedulefree_native_canary": True,
...
def _validations(policy_report: Mapping[str, Any], review: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _validation(
            ...
            "simple_schedulefree_canary_rollout_policy_missing",
        ),
        _validation(
            "optimizer_set_complete",
            optimizer_types == EXPECTED_OPTIMIZERS,
            "simple_schedulefree_dispatch_review_optimizer_set_incomplete",
        ),
        _validation(
            ...
            "simple_schedulefree_dispatch_review_hook_contract_missing",
        ),
        _validation(
            "dispatch_contract_requires_prior_evidence",
            bool(dispatch.get("requires_schedulefree_native_canary"))
            and bool(dispatch.get("requires_train_eval_mode_transition"))
            and bool(dispatch.get("requires_rollout_policy"))
            and bool(dispatch.get("requires_manual_review_approval")),
            "simple_schedulefree_dispatch_review_prior_evidence_missing",
        ),
        _validation(
            ...
            and bool(numeric.get("rollback_on_train_eval_mode_mismatch")),
            "simple_schedulefree_dispatch_review_numeric_guardrails_missing",
        ),
        _validation(
            ...
            and bool(rollback.get("rollback_on_train_eval_mode_mismatch")),
            "simple_schedulefree_dispatch_review_missing_rollback",
        ),
        _validation(
            ...
            "simple_schedulefree_dispatch_review_allows_dispatch_before_review",
        ),
        _validation(
            ...
            "simple_schedulefree_dispatch_review_enabled_dispatch",
        ),
        _validation(
            ...
            "simple_schedulefree_dispatch_review_changed_request_schema_ui",
        ),
    ]
...
__all__ = ["REVIEW_KIND", "build_simple_optimizer_schedulefree_dispatch_integration_review_scorecard"]
