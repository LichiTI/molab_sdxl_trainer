"""Default-off AdaBound Triton TrainingLoop executor for selected plugin canaries."""
...
class AdaBoundTrainingExecutorConfig:
    optimizer_kind: str = "adabound"
    lr: float = 1.0e-3
    final_lr: float = 1.0e-1
    gamma: float = 1.0e-3
...
if triton is not None and tl is not None:
    ...
    def _adabound_dense_kernel(
        ...
        denom = tl.sqrt(v_new + 1.0e-15) + eps
        bounded_lr = tl.minimum(tl.maximum(step_size / denom, lower_bound), upper_bound)
        p_new = p - bounded_lr * m_new
...
class AdaBoundTrainingExecutor:
    """Launch a real AdaBound fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: AdaBoundTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AdaBoundTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("adabound_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("adabound_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("adabound_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"adabound_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("adabound_maximize_not_supported")
        ...
        if not cases:
            blockers.append("adabound_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_adabound_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "adabound_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("adabound_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("adabound_training_executor_requires_contiguous_tensors")
        ...
        step_size = float(group_cfg["lr"]) * bias_correction2_sq / bias_correction1
        final_lr = float(group_cfg["final_lr"]) * float(group_cfg["lr"]) / float(group_cfg["base_lr"])
        lower_bound = final_lr * (1.0 - 1.0 / (float(group_cfg["gamma"]) * step + 1.0))
        upper_bound = final_lr * (1.0 + 1.0 / (float(group_cfg["gamma"]) * step))
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _adabound_dense_kernel[grid](
            param,
            param.grad.detach(),
            exp_avg,
            exp_avg_sq,
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["adabound_training_executor_parameters_not_mutated"],
        }


def build_plugin_adabound_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AdaBoundTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> AdaBoundTrainingExecutor:
    return AdaBoundTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AdaBoundTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AdaBoundTrainingExecutorConfig:
    if isinstance(value, AdaBoundTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    base_lrs = getattr(optimizer, "base_lrs", None)
    base_lr = base_lrs[0] if isinstance(base_lrs, list) and base_lrs else group.get("lr", 1.0e-3)
    return AdaBoundTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "adabound"),
        lr=float(payload.get("lr", group.get("lr", 1.0e-3)) or 1.0e-3),
        final_lr=float(payload.get("final_lr", group.get("final_lr", 1.0e-1)) or 1.0e-1),
        gamma=float(payload.get("gamma", group.get("gamma", 1.0e-3)) or 1.0e-3),
        base_lr=float(payload.get("base_lr", base_lr) or 1.0e-3),
        beta1=float(betas[0]),
        beta2=float(betas[1]),
...
def _group_config(group: Mapping[str, Any], config: AdaBoundTrainingExecutorConfig) -> dict[str, Any]:
    betas = group.get("betas", (config.beta1, config.beta2))
    return {
        "lr": float(group.get("lr", config.lr) or config.lr),
        "final_lr": float(group.get("final_lr", config.final_lr) or config.final_lr),
        "gamma": float(group.get("gamma", config.gamma) or config.gamma),
        "base_lr": float(config.base_lr or config.lr),
        "beta1": float(betas[0]),
        "beta2": float(betas[1]),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("adabound_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("ams_bound", False)):
        blockers.append("adabound_ams_bound_not_supported_for_canary")
    if bool(group_cfg.get("adam_debias", False)):
        blockers.append("adabound_adam_debias_not_supported_for_canary")
    if float(group_cfg.get("base_lr", 0.0) or 0.0) <= 0.0:
        blockers.append("adabound_base_lr_invalid_for_canary")
    return blockers
...
__all__ = ["AdaBoundTrainingExecutor", "AdaBoundTrainingExecutorConfig", "build_plugin_adabound_training_executor"]
