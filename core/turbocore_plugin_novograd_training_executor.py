"""Default-off training executor for selected plugin NovoGrad native dispatch."""
...
ENTRYPOINT = "probe_novograd_training_tensor_binding_canary_py"
REPO_ROOT = Path(__file__).resolve().parents[2]


@dataclass(frozen=True)
class PluginNovoGradTrainingExecutorConfig:
    lr: float = 1e-3
...
class PluginNovoGradTrainingExecutor:
    """Launch selected NovoGrad native steps against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: PluginNovoGradTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("PluginNovoGradTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("plugin_novograd_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("plugin_novograd_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("plugin_novograd_training_dispatch_entrypoint_missing")
        ...
        if not cases:
            blockers.append("plugin_novograd_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_novograd_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "plugin_novograd_native_step_failed"),
    ...
    def _step_param(
        ...
        if type(self.optimizer).__name__.lower() != "novograd":
            return _case_failed("plugin_novograd_training_executor_optimizer_not_novograd", "plugin_novograd_optimizer_class_unsupported")
        if param.dtype != torch.float32 or param.grad is None or param.grad.dtype != torch.float32:
            return _case_failed("plugin_novograd_native_step_requires_float32", "plugin_novograd_native_step_dtype_unsupported")
        if not param.is_contiguous() or not param.grad.is_contiguous():
            return _case_failed("plugin_novograd_native_step_requires_contiguous_param_grad", "plugin_novograd_native_step_layout_unsupported")
        state = self.optimizer.state[param]
        required = ("moments", "grads_ema")
        if not all(name in state for name in required):
            return _case_failed("plugin_novograd_live_state_missing", "plugin_novograd_live_state_missing")
        moments = state["moments"]
        grads_ema = state["grads_ema"]
        state_tensors = (moments, grads_ema)
        if not all(torch.is_tensor(item) for item in state_tensors):
            return _case_failed("plugin_novograd_live_state_tensors_missing", "plugin_novograd_live_state_missing")
        if any(item.dtype != torch.float32 for item in state_tensors):
            return _case_failed("plugin_novograd_native_state_requires_float32", "plugin_novograd_native_state_dtype_unsupported")
        if not all(item.device.type == "cuda" for item in state_tensors):
            return _case_failed("plugin_novograd_native_state_requires_cuda", "plugin_novograd_native_state_device_unsupported")
        if not moments.is_contiguous():
            return _case_failed("plugin_novograd_native_state_requires_contiguous", "plugin_novograd_native_state_layout_unsupported")
        if grads_ema.numel() != 1:
            return _case_failed("plugin_novograd_grads_ema_requires_scalar_tensor", "plugin_novograd_live_state_shape_unsupported")
        launch_config = {
            **dict(group_cfg),
        ...
        try:
            ...
            return _case_failed(f"{type(exc).__name__}: {exc}", "plugin_novograd_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(
                str(launch.get("reason") or "plugin_novograd_native_step_failed"),
                "plugin_novograd_native_step_failed",
                launch=launch,
            )
        return {
            "schema_version": 1,
...
def build_plugin_novograd_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: PluginNovoGradTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> PluginNovoGradTrainingExecutor:
    return PluginNovoGradTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: PluginNovoGradTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> PluginNovoGradTrainingExecutorConfig:
    if isinstance(value, PluginNovoGradTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.95, 0.98)))
    return PluginNovoGradTrainingExecutorConfig(
        ...
        grad_averaging=bool(payload.get("grad_averaging", group.get("grad_averaging", False))),
...
def _group_config(group: Mapping[str, Any], config: PluginNovoGradTrainingExecutorConfig) -> dict[str, Any]:
...
__all__ = [
    "PluginNovoGradTrainingExecutor",
    "PluginNovoGradTrainingExecutorConfig",
    "build_plugin_novograd_training_executor",
]
