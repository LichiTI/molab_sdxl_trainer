"""Owner/release hold for selected fused-backward plugin optimizers."""
...
HOLD_KIND = "plugin_fused_backward_owner_release_hold_v0"


def build_plugin_fused_backward_owner_release_hold_scorecard(
    *,
    family_batch_report: Mapping[str, Any] | None = None,
    write_artifact: bool = False,
) -> dict[str, Any]:
    """Record a default-off hold after fused-backward ABI matrices are implementation-ready."""

    batch = _as_dict(family_batch_report or build_plugin_fused_backward_family_batch_scorecard())
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_fused_backward_owner_release_hold_scorecard_v0",
        "gate": "plugin_fused_backward_owner_release_hold",
        "ok": ready,
        "promotion_ready": False,
        "owner_release_hold_ready": ready,
        "family_batch_ready": batch.get("selected_fused_backward_family_batch_ready") is True,
        ...
        "promotion_blockers": _dedupe(
            blockers
            + [
                "plugin_fused_backward_owner_approval_missing",
                "plugin_fused_backward_release_approval_missing",
                "plugin_fused_backward_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "add fused-backward request/schema/UI non-exposure guard before product dispatch wiring"
            if ready
            else "fix fused-backward owner/release hold blockers"
        ),
        "notes": [
            "This gate records a hold state only; it does not approve native dispatch.",
            "Fused-backward evidence remains report-only until owner/release approval.",
...
def _hold_manifest(batch: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "hold_kind": HOLD_KIND,
        "optimizer_family": "fused_backward",
        ...
        "required_approval_artifacts": [
            ...
            "fused_backward_gradient_ownership_review",
...
def _validations(batch: Mapping[str, Any], hold: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    optimizer_count = int(summary.get("selected_optimizer_count", 0) or 0)
    replay_planned = int(summary.get("fused_backward_replay_case_planned_count", 0) or 0)
    replay_ready = int(summary.get("fused_backward_replay_case_implementation_ready_count", 0) or 0)
    return [
        _validation(
            "family_batch_ready",
            batch.get("selected_fused_backward_family_batch_ready") is True,
            "plugin_fused_backward_family_batch_missing",
        ),
        _validation(
            "optimizer_set_complete",
            len(hold.get("selected_optimizer_names", [])) == optimizer_count == 2,
            "plugin_fused_backward_optimizer_set_incomplete",
        ),
        _validation(
            "abi_resume_matrices_ready",
            int(summary.get("fused_backward_abi_implementation_ready_count", 0) or 0) == optimizer_count
            and int(summary.get("resume_parity_matrix_implementation_ready_count", 0) or 0) == optimizer_count
            and int(summary.get("native_kernel_preconditions_implementation_ready_count", 0) or 0)
            == optimizer_count
            and replay_planned > 0
            and replay_ready == replay_planned,
            "plugin_fused_backward_matrices_not_ready",
        ),
        _validation(
        ...
        _validation(
            ...
            "plugin_fused_backward_batch_enabled_dispatch",
        ),
    ]
...
def _write_artifact(report: Mapping[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_plugin_fused_backward_owner_release_hold_scorecard.json"
...
__all__ = ["build_plugin_fused_backward_owner_release_hold_scorecard"]
