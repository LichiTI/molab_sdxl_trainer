"""Owner/release hold for selected schedule-free plugin optimizers."""
...
HOLD_KIND = "plugin_schedulefree_owner_release_hold_v0"


def build_plugin_schedulefree_owner_release_hold_scorecard(
    *,
    family_batch_report: Mapping[str, Any] | None = None,
    write_artifact: bool = False,
) -> dict[str, Any]:
    """Record a default-off hold after schedule-free canary evidence is ready."""

    batch = _as_dict(family_batch_report or build_plugin_schedulefree_family_batch_scorecard())
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_schedulefree_owner_release_hold_scorecard_v0",
        "gate": "plugin_schedulefree_owner_release_hold",
        "ok": ready,
        "promotion_ready": False,
        "owner_release_hold_ready": ready,
        "family_batch_ready": batch.get("selected_schedulefree_family_batch_ready") is True,
        ...
        "summary": {
            ...
            "ready_stage_count": int(summary.get("ready_stage_count", 0) or 0),
        ...
        "promotion_blockers": _dedupe(
            blockers
            + [
                "plugin_schedulefree_owner_approval_missing",
                "plugin_schedulefree_release_approval_missing",
                "plugin_schedulefree_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "add schedule-free request/schema/UI non-exposure guard before product dispatch wiring"
            if ready
            else "fix schedule-free owner/release hold blockers"
        ),
        "notes": [
            "This gate records a hold state only; it does not approve native dispatch.",
            "Schedule-free evidence remains report-only until owner/release approval.",
...
def _hold_manifest(batch: Mapping[str, Any]) -> dict[str, Any]:
    ...
    selected = [
        name.replace("_training_loop_canary", "")
        for name in stage_names
        if name.startswith("schedulefree") and name.endswith("_training_loop_canary")
    ]
    return {
        "schema_version": 1,
        "hold_kind": HOLD_KIND,
        ...
        "required_approval_artifacts": [
            ...
            "schedule_free_checkpoint_adapter_review",
...
def _validations(batch: Mapping[str, Any], hold: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _validation(
            "family_batch_ready",
            batch.get("selected_schedulefree_family_batch_ready") is True,
            "plugin_schedulefree_family_batch_missing",
        ),
        _validation(
            "optimizer_set_complete",
            len(hold.get("selected_optimizer_names", [])) == optimizer_count == 3,
            "plugin_schedulefree_optimizer_set_incomplete",
        ),
        _validation(
            "all_stages_ready",
            int(summary.get("stage_count", 0) or 0) > 0
            and int(summary.get("ready_stage_count", 0) or 0) == int(summary.get("stage_count", 0) or 0)
            and int(summary.get("selected_native_canary_ready_count", 0) or 0) == optimizer_count
            and int(summary.get("e2e_shadow_case_count", 0) or 0) == 6
            and summary.get("dispatch_review_gate_ready") is True,
            "plugin_schedulefree_stages_not_ready",
        ),
        _validation(
        ...
        _validation(
            ...
            "plugin_schedulefree_batch_enabled_dispatch",
        ),
    ]
...
def _write_artifact(report: Mapping[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_plugin_schedulefree_owner_release_hold_scorecard.json"
...
__all__ = ["build_plugin_schedulefree_owner_release_hold_scorecard"]
