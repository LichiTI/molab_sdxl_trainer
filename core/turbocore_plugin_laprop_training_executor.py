"""Default-off LaProp Triton TrainingLoop executor for selected plugin canaries."""
...
class LaPropTrainingExecutorConfig:
    optimizer_kind: str = "laprop"
    lr: float = 4.0e-4
    beta1: float = 0.9
    beta2: float = 0.999
    eps: float = 1.0e-15
...
if triton is not None and tl is not None:
    ...
    def _laprop_dense_kernel(
        ...
        m_new = m * beta1 + (g / denom) * ((1.0 - beta1) * lr)
...
class LaPropTrainingExecutor:
    """Launch a real LaProp fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: LaPropTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("LaPropTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("laprop_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("laprop_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("laprop_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"laprop_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("laprop_maximize_not_supported")
        ...
        if not cases:
            blockers.append("laprop_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_laprop_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "laprop_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("laprop_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("laprop_training_executor_requires_contiguous_tensors")
        ...
        exp_avg_lr_1 = _next_exp_avg_lr_1(state, group_cfg)
        bias_correction1 = exp_avg_lr_1 / float(group_cfg["lr"]) if float(group_cfg["lr"]) != 0.0 else 1.0
        ...
        state["exp_avg_lr_1"] = exp_avg_lr_1
        state["exp_avg_lr_2"] = _next_exp_avg_lr_2(state, group_cfg)
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["laprop_training_executor_parameters_not_mutated"],
        }


def build_plugin_laprop_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: LaPropTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> LaPropTrainingExecutor:
    return LaPropTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: LaPropTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> LaPropTrainingExecutorConfig:
    if isinstance(value, LaPropTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return LaPropTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "laprop"),
        lr=float(payload.get("lr", group.get("lr", 4.0e-4)) or 4.0e-4),
        beta1=float(betas[0]),
        beta2=float(betas[1]),
        eps=float(payload.get("eps", group.get("eps", 1.0e-15)) or 1.0e-15),
        centered=bool(payload.get("centered", group.get("centered", False))),
...
def _group_config(group: Mapping[str, Any], config: LaPropTrainingExecutorConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "centered": bool(group.get("centered", config.centered)),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if bool(group_cfg.get("centered", False)):
        blockers.append("laprop_centered_not_supported_for_canary")
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("laprop_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("ams_bound", False)):
        blockers.append("laprop_ams_bound_not_supported_for_canary")
    if bool(group_cfg.get("cautious", False)):
        blockers.append("laprop_cautious_not_supported_for_canary")
    return blockers
...
def _next_exp_avg_lr_1(state: Mapping[str, Any], group_cfg: Mapping[str, Any]) -> float:
    return float(state.get("exp_avg_lr_1", 0.0) or 0.0) * float(group_cfg["beta1"]) + (
        1.0 - float(group_cfg["beta1"])
    ) * float(group_cfg["lr"])


def _next_exp_avg_lr_2(state: Mapping[str, Any], group_cfg: Mapping[str, Any]) -> float:
    return float(state.get("exp_avg_lr_2", 0.0) or 0.0) * float(group_cfg["beta2"]) + (1.0 - float(group_cfg["beta2"]))
...
__all__ = ["LaPropTrainingExecutor", "LaPropTrainingExecutorConfig", "build_plugin_laprop_training_executor"]
