"""Request/schema/UI non-exposure audit for selected adaptive-LR plugin native work."""
...
AUDIT_KIND = "plugin_adaptivelr_request_schema_ui_non_exposure_v0"
FORBIDDEN_BOUNDARY_TOKENS = (
    "plugin_adaptivelr_native",
    "plugin_adaptivelr_dispatch",
    "plugin_adaptivelr_canary",
    "enable_plugin_adaptivelr_native",
    "enable_adaptivelr_native",
    "adaptivelr_native",
    "adaptivelr_turbocore",
)
BOUNDARY_PATHS = (
...
def build_plugin_adaptivelr_request_schema_ui_non_exposure_scorecard(
    *,
    owner_release_hold_report: Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
    write_artifact: bool = False,
) -> dict[str, Any]:
    """Audit product boundaries while selected adaptive-LR native dispatch stays disabled."""

    root = Path(workspace_root).resolve() if workspace_root is not None else Path(__file__).resolve().parents[2]
    hold = _as_dict(owner_release_hold_report or build_plugin_adaptivelr_owner_release_hold_scorecard())
    ...
    report = {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_adaptivelr_request_schema_ui_non_exposure_scorecard_v0",
        "gate": "plugin_adaptivelr_request_schema_ui_non_exposure",
        "ok": ready,
        ...
        "promotion_blockers": _dedupe(
            ...
            + [
                ...
                "plugin_adaptivelr_request_schema_ui_exposure_not_approved",
                "plugin_adaptivelr_product_dispatch_not_approved",
            ]
        ),
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "await explicit owner/release approval before adaptive-LR request/schema/UI or dispatch wiring"
            if ready
        ...
        "notes": [
            ...
            "Boundary scanning checks product-facing paths for selected adaptive-LR native exposure tokens.",
...
def _validations(
    ...
    return [
        _validation(
            ...
            "plugin_adaptivelr_owner_release_hold_missing",
        ),
        _validation(
            "optimizer_set_complete",
            int(_as_dict(hold.get("summary")).get("optimizer_count", 0) or 0) == 6,
            "plugin_adaptivelr_request_schema_ui_optimizer_set_incomplete",
        ),
        _validation(
            ...
            "plugin_adaptivelr_request_schema_ui_unexpected_approval",
        ),
        _validation(
            ...
            "plugin_adaptivelr_request_schema_ui_hold_enabled_boundary",
        ),
        _validation(
            "boundary_inventory_present",
            len(boundary.get("present_paths", [])) > 0,
            "plugin_adaptivelr_request_schema_ui_boundary_inventory_missing",
        ),
        _validation(
            "forbidden_tokens_absent",
            not findings.get("forbidden_token_hits"),
            "plugin_adaptivelr_request_schema_ui_forbidden_boundary_token_found",
        ),
    ]
...
def _write_artifact(root: Path, report: Mapping[str, Any]) -> None:
    ...
    path = temp_dir / "turbocore_plugin_adaptivelr_request_schema_ui_non_exposure_scorecard.json"
...
__all__ = ["build_plugin_adaptivelr_request_schema_ui_non_exposure_scorecard"]
