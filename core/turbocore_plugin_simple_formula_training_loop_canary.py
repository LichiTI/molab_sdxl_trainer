"""Shared TrainingLoop canaries for selected plugin simple-formula optimizers."""

from __future__ import annotations

from dataclasses import dataclass
from typing import Any, Mapping
from unittest.mock import patch

import torch

from core.configs import SchedulerType
from core.lulynx_trainer.config import LulynxConfig, OptimizerType
from core.lulynx_trainer.optimizer_plugin_support import plugin_resume_case
from core.lulynx_trainer.trainer import LulynxTrainer
from core.lulynx_trainer.training_loop import TrainingLoop


@dataclass(frozen=True)
class SimpleFormulaPluginSpec:
    kind: str
    class_name: str
    native_kind: str
    native_route: str
    state_keys: tuple[str, ...]
    optimizer_extra_args: str
    weight_decay: float
    loss_scale: float
    loss_bias: float
    param_shape: tuple[int, ...] = (4096,)


SPECS: dict[str, SimpleFormulaPluginSpec] = {
    "lion": SimpleFormulaPluginSpec(
        kind="lion",
        class_name="Lion",
        native_kind="lion",
        native_route="rust_cuda_simple_formula_lion_v0",
        state_keys=("exp_avg",),
        optimizer_extra_args="foreach=False",
        weight_decay=0.01,
        loss_scale=0.31,
        loss_bias=0.003,
    ),
    "sgdw": SimpleFormulaPluginSpec(
        kind="sgdw",
        class_name="SGDW",
        native_kind="sgd_nesterov",
        native_route="rust_cuda_simple_formula_sgd_nesterov_v0",
        state_keys=("momentum_buffer",),
        optimizer_extra_args="momentum=0.9 nesterov=True foreach=False",
        weight_decay=0.01,
        loss_scale=0.27,
        loss_bias=0.002,
    ),
    "sgd": SimpleFormulaPluginSpec(
        kind="sgd",
        class_name="SGD",
        native_kind="sgd",
        native_route="rust_cuda_simple_formula_sgd_v0",
        state_keys=("momentum_buffer",),
        optimizer_extra_args="momentum=0.9 nesterov=False foreach=False",
        weight_decay=0.01,
        loss_scale=0.24,
        loss_bias=0.002,
    ),
    "signsgd": SimpleFormulaPluginSpec(
        kind="signsgd",
        class_name="SignSGD",
        native_kind="sign_momentum",
        native_route="rust_cuda_simple_formula_sign_momentum_v0",
        state_keys=("momentum_buffer",),
        optimizer_extra_args="momentum=0.9 foreach=False",
        weight_decay=0.0,
        loss_scale=0.25,
        loss_bias=0.002,
    ),
    "tiger": SimpleFormulaPluginSpec(
        kind="tiger",
        class_name="Tiger",
        native_kind="sign_momentum",
        native_route="rust_cuda_simple_formula_sign_momentum_v0",
        state_keys=("exp_avg",),
        optimizer_extra_args="beta=0.965 foreach=False",
        weight_decay=0.01,
        loss_scale=0.22,
        loss_bias=0.001,
    ),
    "qhm": SimpleFormulaPluginSpec(
        kind="qhm",
        class_name="QHM",
        native_kind="qhm",
        native_route="rust_cuda_simple_formula_qhm_v0",
        state_keys=("momentum_buffer",),
        optimizer_extra_args="momentum=0.9 nu=0.7",
        weight_decay=0.01,
        loss_scale=0.29,
        loss_bias=0.002,
    ),
    "accsgd": SimpleFormulaPluginSpec(
        kind="accsgd",
        class_name="AccSGD",
        native_kind="accsgd",
        native_route="rust_cuda_simple_formula_accsgd_v0",
        state_keys=("momentum_buffer",),
        optimizer_extra_args="kappa=1000.0 xi=10.0 constant=0.7",
        weight_decay=0.01,
        loss_scale=0.26,
        loss_bias=0.002,
    ),
    "fromage": SimpleFormulaPluginSpec(
        kind="fromage",
        class_name="Fromage",
        native_kind="fromage",
        native_route="rust_cuda_simple_formula_fromage_v0",
        state_keys=(),
        optimizer_extra_args="p_bound=None",
        weight_decay=0.0,
        loss_scale=0.23,
        loss_bias=0.001,
    ),
    "rmsprop": SimpleFormulaPluginSpec(
        kind="rmsprop",
        class_name="RMSprop",
        native_kind="rmsprop",
        native_route="rust_cuda_simple_formula_rmsprop_v0",
        state_keys=("square_avg",),
        optimizer_extra_args="alpha=0.99 eps=1e-8 momentum=0.0 centered=False foreach=False",
        weight_decay=0.01,
        loss_scale=0.21,
        loss_bias=0.001,
    ),
    "lars": SimpleFormulaPluginSpec(
        kind="lars",
        class_name="LARS",
        native_kind="lars",
        native_route="rust_cuda_simple_formula_lars_v0",
        state_keys=("momentum_buffer",),
        optimizer_extra_args="momentum=0.9 dampening=0.0 trust_coefficient=0.001 nesterov=False foreach=False",
        weight_decay=0.01,
        loss_scale=0.20,
        loss_bias=0.001,
        param_shape=(64, 64),
    ),
    "pid": SimpleFormulaPluginSpec(
        kind="pid",
        class_name="PID",
        native_kind="pid",
        native_route="rust_cuda_simple_formula_pid_v0",
        state_keys=(),
        optimizer_extra_args="momentum=0.0 dampening=0.0 derivative=10.0 integral=5.0 weight_decouple=False fixed_decay=False",
        weight_decay=0.01,
        loss_scale=0.19,
        loss_bias=0.001,
    ),
    "sgdp": SimpleFormulaPluginSpec(
        kind="sgdp",
        class_name="SGDP",
        native_kind="sgdp",
        native_route="rust_cuda_simple_formula_sgdp_v0",
        state_keys=("momentum",),
        optimizer_extra_args=(
            "momentum=0.0 dampening=0.0 weight_decay=0.0 weight_decouple=True "
            "fixed_decay=False delta=0.1 wd_ratio=0.1 nesterov=False"
        ),
        weight_decay=0.0,
        loss_scale=0.18,
        loss_bias=0.001,
    ),
    "gravity": SimpleFormulaPluginSpec(
        kind="gravity",
        class_name="Gravity",
        native_kind="gravity",
        native_route="rust_cuda_simple_formula_gravity_v0",
        state_keys=("v",),
        optimizer_extra_args="alpha=0.0 beta=0.9",
        weight_decay=0.0,
        loss_scale=0.17,
        loss_bias=0.001,
    ),
    "aggmo": SimpleFormulaPluginSpec(
        kind="aggmo",
        class_name="AggMo",
        native_kind="aggmo",
        native_route="rust_cuda_simple_formula_aggmo_v0",
        state_keys=("momentum_buffer",),
        optimizer_extra_args="betas=(0.0,0.9,0.99) weight_decouple=False fixed_decay=False",
        weight_decay=0.0,
        loss_scale=0.16,
        loss_bias=0.001,
    ),
    "asgd": SimpleFormulaPluginSpec(
        kind="asgd",
        class_name="ASGD",
        native_kind="asgd",
        native_route="rust_cuda_simple_formula_asgd_v0",
        state_keys=(),
        optimizer_extra_args="amplifier=0.02 theta=1.0 dampening=1.0 eps=1e-5 weight_decouple=True fixed_decay=False",
        weight_decay=0.0,
        loss_scale=0.15,
        loss_bias=0.001,
    ),
    "madgrad": SimpleFormulaPluginSpec(
        kind="madgrad",
        class_name="MADGRAD",
        native_kind="madgrad",
        native_route="rust_cuda_simple_formula_madgrad_v0",
        state_keys=("grad_sum_sq", "s", "x0"),
        optimizer_extra_args="momentum=0.9 eps=1e-6 weight_decouple=False",
        weight_decay=0.0,
        loss_scale=0.14,
        loss_bias=0.001,
    ),
    "nero": SimpleFormulaPluginSpec(
        kind="nero",
        class_name="Nero",
        native_kind="nero",
        native_route="rust_cuda_simple_formula_nero_v0",
        state_keys=("exp_avg_sq", "scale"),
        optimizer_extra_args="beta=0.999 constraints=False eps=1e-8",
        weight_decay=0.0,
        loss_scale=0.13,
        loss_bias=0.001,
    ),
    "vsgd": SimpleFormulaPluginSpec(
        kind="vsgd",
        class_name="VSGD",
        native_kind="vsgd",
        native_route="rust_cuda_simple_formula_vsgd_v0",
        state_keys=("bg", "bhg", "mug"),
        optimizer_extra_args="tau1=0.81 tau2=0.9 eps=1e-8 weight_decouple=True",
        weight_decay=0.0,
        loss_scale=0.12,
        loss_bias=0.001,
    ),
}


class _Injector:
    def __init__(self, params: list[torch.nn.Parameter]) -> None:
        self.params = params

    def get_trainable_params(self) -> list[torch.nn.Parameter]:
        return self.params


def build_plugin_simple_formula_training_loop_canary_scorecard(kind: str) -> dict[str, Any]:
    spec = _spec(kind)
    if not torch.cuda.is_available():
        return _blocked(spec, f"cuda_required_for_plugin_{spec.kind}_training_loop_canary")
    case = _run_case(spec)
    ok = bool(case.get("ok", False))
    blockers = _strings(case.get("blocked_reasons"))
    return {
        "schema_version": 1,
        "scorecard": f"turbocore_plugin_{spec.kind}_training_loop_canary_scorecard_v0",
        "gate": f"plugin_{spec.kind}_training_loop_native_canary",
        "ok": ok,
        "promotion_ready": False,
        "selected_native_canary_ready": ok,
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "runtime_dispatch_ready": False,
        "native_dispatch_allowed": False,
        "selector_type": OptimizerType.PYTORCH_OPTIMIZER.value,
        "selected_optimizer_name": spec.kind,
        "optimizer_family": "simple_formula",
        "native_route": spec.native_route,
        "case": case,
        "summary": {
            "native_step_count": 1 if case.get("native_step_executed") is True else 0,
            "native_kernel_launch_count": 1 if case.get("native_kernel_launched") is True else 0,
            "primed_pytorch_state": bool(case.get("primed_pytorch_state", False)),
            "optimizer_class": case.get("optimizer_class"),
            "executor_optimizer_kind": case.get("executor_optimizer_kind"),
            "state_keys": case.get("state_keys"),
        },
        "promotion_blockers": blockers
        + [
            f"plugin_{spec.kind}_formula_parity_matrix_refresh_required",
            f"plugin_{spec.kind}_canary_rollout_policy_missing",
            "product_rollout_review_missing",
        ],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            f"refresh selected simple-formula parity matrix and rollout policy for plugin {spec.class_name}"
            if ok
            else f"fix selected plugin {spec.class_name} TrainingLoop canary blockers"
        ),
    }


def _run_case(spec: SimpleFormulaPluginSpec) -> dict[str, Any]:
    loop, param, optimizer = _make_loop(spec)
    _prime_optimizer_state(param, optimizer, spec)
    _seed_previous_gate(loop)
    captured: list[dict[str, Any]] = []
    before = param.detach().clone()

    def _fake_train_step(
        self: TrainingLoop,
        _batch: dict[str, Any],
        accumulation_steps: int = 1,
        return_loss_tensor: bool = False,
    ) -> Any:
        params = self._get_trainable_params()
        for item in params:
            item.grad = None
        loss = sum(
            ((item.float() * spec.loss_scale) ** 2).mean() + item.float().mean() * spec.loss_bias
            for item in params
        )
        loss = loss / max(int(accumulation_steps or 1), 1)
        loss.backward()
        return loss.detach() if return_loss_tensor else float(loss.detach().item())

    loop.on_step_end = lambda _step, _loss, info: captured.append(dict(info))
    with patch.object(TrainingLoop, "train_step", new=_fake_train_step):
        result = loop.train_epoch([{}], 0)
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": f"plugin_{spec.kind}_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": [f"plugin_{spec.kind}_training_loop_did_not_emit_step"],
        }
    runtime = _as_dict(captured[0].get("turbocore_native_update_dispatch_runtime"))
    training_executor = _as_dict(runtime.get("training_executor"))
    executor_result = _as_dict(training_executor.get("result"))
    native_step = bool(runtime.get("native_step_executed", False))
    native_kernel = bool(runtime.get("native_kernel_launched", False))
    state_keys = sorted(str(key) for key in optimizer.state[param].keys())
    param_mutated = not torch.equal(before, param.detach())
    ok = bool(
        result.get("steps") == 1
        and native_step
        and native_kernel
        and param_mutated
        and not bool(runtime.get("should_call_pytorch_optimizer_step", True))
        and type(optimizer).__name__ == spec.class_name
        and str(executor_result.get("optimizer_kind") or "") == spec.native_kind
        and set(spec.state_keys).issubset(state_keys)
    )
    return {
        "schema_version": 1,
        "ok": ok,
        "probe": f"plugin_{spec.kind}_training_loop_native_canary_v0",
        "result": result,
        "captured_step_count": len(captured),
        "primed_pytorch_state": True,
        "optimizer_class": type(optimizer).__name__,
        "native_step_executed": native_step,
        "native_kernel_launched": native_kernel,
        "training_parameters_mutated": param_mutated,
        "should_call_pytorch_optimizer_step": bool(runtime.get("should_call_pytorch_optimizer_step", True)),
        "fallback_to_pytorch_required": bool(runtime.get("fallback_to_pytorch_required", True)),
        "training_executor_called": bool(training_executor.get("called", False)),
        "training_executor_ok": bool(training_executor.get("ok", False)),
        "executor_result_ok": bool(executor_result.get("ok", False)),
        "executor_optimizer_kind": str(executor_result.get("optimizer_kind") or ""),
        "param_dtype": str(param.dtype).replace("torch.", ""),
        "state_keys": state_keys,
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "runtime_dispatch_ready": False,
        "native_dispatch_allowed": False,
        "blocked_reasons": []
        if ok
        else _dedupe(_strings(runtime.get("blocked_reasons")) + [f"plugin_{spec.kind}_training_loop_native_step_missing"]),
    }


def _make_loop(spec: SimpleFormulaPluginSpec) -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    device = torch.device("cuda")
    numel = 1
    for dim in spec.param_shape:
        numel *= int(dim)
    param = torch.nn.Parameter(
        torch.linspace(-0.07, 0.17, steps=numel, device=device, dtype=torch.float32).view(spec.param_shape)
    )
    optimizer = _create_plugin_optimizer(param, spec)
    loop = TrainingLoop(
        unet=torch.nn.Identity(),
        text_encoder_1=torch.nn.Identity(),
        text_encoder_2=None,
        vae=torch.nn.Identity(),
        tokenizer_1=None,
        tokenizer_2=None,
        noise_scheduler=None,
        lora_injector=_Injector([param]),
        optimizer=optimizer,
        lr_scheduler=None,
        device="cuda",
        dtype=torch.float32,
        gradient_accumulation_steps=1,
        max_grad_norm=0.0,
        layer_monitor_enabled=False,
        vram_smart_sensing_enabled=False,
        turbocore_native_update_mode="native_experimental",
        turbocore_native_update_required_shadow_passes=1,
        turbocore_native_update_allow_missing_kernel=True,
        turbocore_native_update_dispatch_enabled=True,
        turbocore_native_update_training_path_enabled=True,
        turbocore_native_update_require_native_cuda=True,
        turbocore_native_update_simple_optimizer_kind=spec.native_kind,
    )
    loop.total_steps = 1
    return loop, param, optimizer


def _create_plugin_optimizer(param: torch.nn.Parameter, spec: SimpleFormulaPluginSpec) -> torch.optim.Optimizer:
    trainer = LulynxTrainer.__new__(LulynxTrainer)
    case = plugin_resume_case(spec.kind)
    trainer.config = LulynxConfig(
        optimizer_type=OptimizerType.PYTORCH_OPTIMIZER,
        learning_rate=1e-3,
        weight_decay=spec.weight_decay,
        optimizer_args=f"{case.optimizer_args} {spec.optimizer_extra_args}".strip(),
        lr_scheduler=SchedulerType.COSINE,
        warmup_ratio=0.0,
    )
    trainer.config.semantic_tuner_enabled = False
    trainer.lora_injector = _Injector([param])
    trainer.model = None
    trainer.trainable_params = []
    trainer._block_weight_manager = None
    trainer._easy_control = None
    trainer._ip_adapter = None
    trainer._repa_projector = None
    trainer._advanced_optimizer_strategy_profile = {}
    trainer._optimizer_backend_profile = {}
    trainer._log_messages = []
    trainer._log = lambda msg: trainer._log_messages.append(str(msg))
    return trainer._create_optimizer()


def _prime_optimizer_state(
    param: torch.nn.Parameter,
    optimizer: torch.optim.Optimizer,
    spec: SimpleFormulaPluginSpec,
) -> None:
    param.grad = None
    loss = ((param.float() * spec.loss_scale) ** 2).mean() + param.float().mean() * spec.loss_bias
    loss.backward()
    optimizer.step()
    optimizer.zero_grad(set_to_none=True)


def _seed_previous_gate(loop: TrainingLoop) -> None:
    loop._turbocore_native_update_dispatch_armer._last_gate_report = _explicit_gate()


def _explicit_gate() -> dict[str, Any]:
    return {
        "dispatch_request": {
            "requested": True,
            "dispatch_allowed": True,
            "training_path_enabled": True,
            "training_path_request": {"request_boundary_ready": True, "explicit_training_path_requested": True},
        },
        "dispatch_contract": {
            "dispatch_rehearsal_ready": True,
            "would_allow_native_dispatch": True,
            "rehearsal": {"would_launch_native_kernel": True},
            "recovery": {"default_off_recovery_bridge_ready": True, "training_dispatch_recovery_ready": True},
            "owner_gradient_sync": _ready_contract("sync_boundary_ready", "owner_gradient_sync_preconditions_ready"),
            "training_flat_owner": _ready_contract("owner_boundary_ready", "training_flat_owner_preconditions_ready"),
            "training_dispatch_kernel": _ready_contract("kernel_boundary_ready", "training_dispatch_kernel_preconditions_ready"),
            "training_executor": {"executor_boundary_ready": True, "training_executor_preconditions_ready": True},
            "stream_lifetime_ownership": {
                "ownership_boundary_ready": True,
                "stream_lifetime_ownership_preconditions_ready": True,
            },
            "evidence": {
                "owner_native_launch_ok": True,
                "copyback_dispatch_validated": True,
                "event_chain_verified": True,
                "stream_ordering_verified": True,
                "representative_performance_gate_ready": True,
            },
            "blocked_reasons": [],
        },
        "kernel_launch_plan": {
            "launch_allowed": True,
            "evidence": {"diagnostic_kernel_executed": True, "diagnostic_parity_ok": True},
        },
    }


def _ready_contract(boundary: str, precondition: str) -> dict[str, Any]:
    return {boundary: True, precondition: True, "native_supported": True, "training_lifecycle_integrated": True}


def _blocked(spec: SimpleFormulaPluginSpec, reason: str) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "scorecard": f"turbocore_plugin_{spec.kind}_training_loop_canary_scorecard_v0",
        "gate": f"plugin_{spec.kind}_training_loop_native_canary",
        "ok": False,
        "promotion_ready": False,
        "selected_native_canary_ready": False,
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "runtime_dispatch_ready": False,
        "native_dispatch_allowed": False,
        "selector_type": OptimizerType.PYTORCH_OPTIMIZER.value,
        "selected_optimizer_name": spec.kind,
        "optimizer_family": "simple_formula",
        "native_route": spec.native_route,
        "case": {},
        "summary": {"native_step_count": 0, "native_kernel_launch_count": 0, "primed_pytorch_state": False},
        "promotion_blockers": [reason],
        "blocked_reasons": [reason],
        "recommended_next_step": f"run selected plugin {spec.class_name} TrainingLoop canary on CUDA",
    }


def _spec(kind: str) -> SimpleFormulaPluginSpec:
    key = str(kind or "").strip().lower()
    if key not in SPECS:
        raise ValueError(f"Unsupported plugin simple-formula canary kind: {kind}")
    return SPECS[key]


def _as_dict(value: Any) -> dict[str, Any]:
    return dict(value) if isinstance(value, Mapping) else {}


def _strings(value: Any) -> list[str]:
    if isinstance(value, (list, tuple, set)):
        return [str(item) for item in value if str(item)]
    return []


def _dedupe(values: list[str]) -> list[str]:
    result: list[str] = []
    for value in values:
        text = str(value or "")
        if text and text not in result:
            result.append(text)
    return result


__all__ = ["SPECS", "build_plugin_simple_formula_training_loop_canary_scorecard"]
