"""TrainingLoop canary for selected plugin LAMB native dispatch."""
...
def build_plugin_lamb_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_lamb_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = _strings(case.get("blocked_reasons"))
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_lamb_training_loop_canary_scorecard_v0",
        "gate": "plugin_lamb_training_loop_native_canary",
        "ok": ok,
        ...
        "summary": {
            ...
            "rectify_enabled": case.get("rectify_enabled"),
            "pre_norm_enabled": case.get("pre_norm_enabled"),
            "adanorm_enabled": case.get("adanorm_enabled"),
            "trust_ratio_present": case.get("trust_ratio_present"),
        },
        "promotion_blockers": blockers
        + [
            "lamb_e2e_shadow_matrix_refresh_required",
            "lamb_canary_rollout_policy_refresh_required",
            "product_rollout_review_missing",
        ],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "refresh selected Adam-like e2e shadow matrix and rollout policy for lamb"
            if ok
            else "fix selected plugin LAMB TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    def _fake_train_step(
        ...
        for item in params:
            item.grad = None
        loss = sum(((item.float() * 0.12) ** 2).mean() + item.float().mean() * 0.006 for item in params)
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_lamb_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_lamb_training_loop_did_not_emit_step"],
        }
    ...
    rectify_enabled = bool(group.get("rectify", False))
    pre_norm_enabled = bool(getattr(optimizer, "pre_norm", False))
    adanorm_enabled = bool(group.get("adanorm", False))
    trust_ratio_present = "trust_ratio" in state and "weight_norm" in state and "adam_norm" in state
    ok = bool(
        result.get("steps") == 1
        and native_step
        and native_kernel
        ...
        and step_after == 2
        and type(optimizer).__name__ == "Lamb"
        and str(executor_result.get("optimizer_kind") or "") == "lamb"
        and {"exp_avg", "exp_avg_sq", "weight_norm", "adam_norm", "trust_ratio"}.issubset(state_keys)
        and not rectify_enabled
        and not pre_norm_enabled
        and not adanorm_enabled
    )
    return {
        "schema_version": 1,
        "ok": ok,
        ...
        "pre_norm_enabled": pre_norm_enabled,
        "adanorm_enabled": adanorm_enabled,
        "trust_ratio_present": trust_ratio_present,
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "runtime_dispatch_ready": False,
        "native_dispatch_allowed": False,
        "blocked_reasons": []
        if ok
        else _dedupe(_strings(runtime.get("blocked_reasons")) + ["plugin_lamb_training_loop_native_step_missing"]),
    }
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    optimizer = _create_selected_lamb_plugin_optimizer(param)
    loop = TrainingLoop(
...
def _create_selected_lamb_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
    ...
    trainer.config = LulynxConfig(
        ...
        learning_rate=1e-3,
        weight_decay=0.01,
        optimizer_args=f"{case.optimizer_args},rectify=False,pre_norm=False,adanorm=False,foreach=False",
...
def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> None:
    param.grad = None
    loss = ((param.float() * 0.13) ** 2).mean() + param.float().mean() * 0.004
    loss.backward()
    optimizer.step()
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin LAMB TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_lamb_training_loop_canary_scorecard"]
