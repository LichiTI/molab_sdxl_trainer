"""CUDA scratch-kernel parity scorecard for selected plugin adamg."""
...
ENTRYPOINT = "probe_adamg_cuda_scratch_launch_py"
REPO_ROOT = Path(__file__).resolve().parents[2]


def build_plugin_adamg_native_scratch_kernel_scorecard(
    *,
    ...
    if native is None:
        return _blocked("adamg_scratch_probe_entrypoint_missing")
    ...
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_adamg_native_scratch_kernel_scorecard_v0",
        "gate": "plugin_adamg_native_scratch_kernel_parity",
        "ok": ready,
        "promotion_ready": False,
        "kernel_parity_stage_ready": ready,
        "adamg_native_kernel_parity": ready,
        ...
        + [
            "adamg_training_tensor_binding_missing",
            "adamg_runtime_dispatch_shadow_missing",
            "adamg_training_loop_canary_missing",
        ],
        "blocked_reasons": _case_blockers(case),
        "recommended_next_step": (
            "extend selected adamg native kernel to training tensor binding canary"
            if ready
            else "fix selected adamg scratch CUDA kernel parity"
        ),
    }
...
def _run_case(native: Any, workspace_root: str, arch: str) -> dict[str, Any]:
    try:
        probe = native.probe_adamg_cuda_scratch_launch_py(workspace_root, arch)
        ...
        return {
            ...
            "hard_failure": True,
            "reason": f"adamg_cuda_probe_failed:{type(exc).__name__}: {exc}",
            "kernel_executed": False,
            "native_kernel_parity_ready": False,
            "training_path_enabled": False,
            "native_dispatch_allowed": False,
            "blocked_reasons": ["adamg_native_kernel_parity_missing"],
        }
    parity_ready = (
        ...
        and str(probe.get("optimizer_kind", "")) == "adamg"
    ...
    return {
        ...
        "kernel_name": str(probe.get("kernel_name", "adamg_flat_fp32_cuda_v0")),
        ...
        "probe": probe,
        "blocked_reasons": [] if parity_ready else ["adamg_native_kernel_parity_missing"],
    }
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "adamg_native_kernel_parity": False,
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "native_dispatch_allowed": False,
        ...
        "promotion_blockers": [reason, "adamg_native_kernel_parity_missing"],
        "blocked_reasons": [reason],
        "recommended_next_step": "build or load lulynx_native with selected adamg scratch probe entrypoint",
    }


__all__ = ["build_plugin_adamg_native_scratch_kernel_scorecard"]
