"""Default-off SWATS Triton TrainingLoop executor for selected plugin canaries."""
...
class SwatsTrainingExecutorConfig:
    optimizer_kind: str = "swats"
    lr: float = 1.0e-3
    beta1: float = 0.9
    beta2: float = 0.999
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _swats_first_step_scalar_kernel(
        param_ptr,
        grad_ptr,
        exp_avg_ptr,
        exp_avg_sq_ptr,
        ...
        m = tl.load(exp_avg_ptr).to(tl.float32)
        v = tl.load(exp_avg_sq_ptr).to(tl.float32)
        m2 = tl.load(exp_avg2_ptr).to(tl.float32)

        m_new = m * beta1 + g * (1.0 - beta1)
        v_new = v * beta2 + g * g * (1.0 - beta2)
        denom = tl.sqrt(v_new + 1.0e-15) + eps
        perturb = (m_new / denom) * (-step_size)
        p_new = p + perturb
        pg = perturb * g
        scaling = tl.where(pg != 0.0, (perturb * perturb) / (-pg), 0.0)
        m2_new = tl.where(pg != 0.0, m2 * beta2 + scaling * (1.0 - beta2), m2)

        tl.store(exp_avg_ptr, m_new)
        tl.store(exp_avg_sq_ptr, v_new)
        tl.store(exp_avg2_ptr, m2_new)
        tl.store(param_ptr, p_new)


class SwatsTrainingExecutor:
    """Launch a real SWATS first-step scalar fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: SwatsTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("SwatsTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("swats_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("swats_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("swats_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"swats_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("swats_maximize_not_supported")
        ...
        if not cases:
            blockers.append("swats_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_swats_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "swats_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("swats_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("swats_training_executor_requires_contiguous_tensors")
        if int(param.numel()) != 1:
            return _case_failed("swats_training_executor_scalar_canary_only")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("swats_training_executor_only_first_adam_step_supported_for_canary")
        state = self.optimizer.state[param]
        exp_avg = _state_tensor(state, "exp_avg", param)
        exp_avg_sq = _state_tensor(state, "exp_avg_sq", param)
        exp_avg2 = _exp_avg2_tensor(state, param)
        ...
        step_size = float(group_cfg["lr"]) * math.sqrt(bias_correction2)
        if not bool(group_cfg["adam_debias"]):
            step_size /= bias_correction1
        _swats_first_step_scalar_kernel[(1,)](
            param,
            param.grad.detach(),
            exp_avg,
            exp_avg_sq,
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["swats_training_executor_parameters_not_mutated"],
        }


def build_plugin_swats_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: SwatsTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> SwatsTrainingExecutor:
    return SwatsTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: SwatsTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> SwatsTrainingExecutorConfig:
    if isinstance(value, SwatsTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return SwatsTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "swats"),
...
def _group_config(group: Mapping[str, Any], config: SwatsTrainingExecutorConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "phase": str(group.get("phase", "adam") or "adam"),
    }


def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if str(group_cfg.get("phase", "adam")) != "adam":
        blockers.append("swats_sgd_phase_not_supported_for_canary")
    if bool(group_cfg.get("ams_bound", False)):
        blockers.append("swats_ams_bound_not_supported_for_canary")
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("swats_weight_decay_not_supported_for_canary")
    return blockers
...
def _exp_avg2_tensor(state: dict[str, Any], param: torch.nn.Parameter) -> torch.Tensor:
    existing = state.get("exp_avg2")
    if torch.is_tensor(existing) and tuple(existing.shape) == (1,):
        if existing.device == param.device and existing.dtype == torch.float32 and existing.is_contiguous():
            return existing
        converted = existing.detach().to(device=param.device, dtype=torch.float32).contiguous()
        state["exp_avg2"] = converted
        return converted
    state["exp_avg2"] = torch.zeros((1,), device=param.device, dtype=torch.float32)
    return state["exp_avg2"]
...
__all__ = ["SwatsTrainingExecutor", "SwatsTrainingExecutorConfig", "build_plugin_swats_training_executor"]
