"""Default-off AdaPNM Triton TrainingLoop executor for selected plugin canaries."""
...
class AdaPNMTrainingExecutorConfig:
    optimizer_kind: str = "adapnm"
    lr: float = 1.0e-3
    beta1: float = 0.9
    beta2: float = 0.999
    beta3: float = 1.0
...
if triton is not None and tl is not None:
    ...
    @triton.jit
    def _adapnm_first_step_dense_kernel(
        param_ptr,
        grad_ptr,
        exp_avg_ptr,
        neg_exp_avg_ptr,
        ...
        pos = tl.load(exp_avg_ptr + offsets, mask=mask, other=0.0).to(tl.float32)
        neg = tl.load(neg_exp_avg_ptr + offsets, mask=mask, other=0.0).to(tl.float32)
        v = tl.load(exp_avg_sq_ptr + offsets, mask=mask, other=0.0).to(tl.float32)

        pos_new = pos * beta1_p2 + g * (1.0 - beta1_p2)
        v_ema = v * beta2 + g * g * (1.0 - beta2)
        denom = (tl.sqrt(v_ema + 1.0e-15) + eps) / bias_correction2_sq
        pn_momentum = (pos_new * (1.0 + beta3) - neg * beta3) / noise_norm
        p_new = p - step_size * pn_momentum / denom

        tl.store(exp_avg_ptr + offsets, pos_new, mask=mask)
        tl.store(exp_avg_sq_ptr + offsets, v_ema, mask=mask)
        tl.store(param_ptr + offsets, p_new, mask=mask)


class AdaPNMTrainingExecutor:
    """Launch a real AdaPNM first-step fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: AdaPNMTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AdaPNMTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("adapnm_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("adapnm_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("adapnm_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"adapnm_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("adapnm_maximize_not_supported")
        ...
        if not cases:
            blockers.append("adapnm_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_adapnm_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "adapnm_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("adapnm_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("adapnm_training_executor_requires_contiguous_tensors")
        step = int(group.get("step", 0) or 0)
        if step != 1:
            return _case_failed("adapnm_training_executor_only_first_step_supported_for_canary")
        state = self.optimizer.state[param]
        exp_avg = _state_tensor(state, "exp_avg", param)
        exp_avg_sq = _state_tensor(state, "exp_avg_sq", param)
        neg_exp_avg = _state_tensor(state, "neg_exp_avg", param)
        ...
        step_size = float(group_cfg["lr"]) if bool(group_cfg["adam_debias"]) else float(group_cfg["lr"]) / bias_correction1
        noise_norm = math.sqrt((1.0 + beta3) ** 2 + beta3**2)
        grid = (triton.cdiv(int(param.numel()), int(group_cfg["block_size"])),)
        _adapnm_first_step_dense_kernel[grid](
            param,
            param.grad.detach(),
            exp_avg,
            neg_exp_avg,
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["adapnm_training_executor_parameters_not_mutated"],
        }


def build_plugin_adapnm_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AdaPNMTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> AdaPNMTrainingExecutor:
    return AdaPNMTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AdaPNMTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AdaPNMTrainingExecutorConfig:
    if isinstance(value, AdaPNMTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999, 1.0)))
    beta3 = betas[2] if len(betas) > 2 else 1.0
    return AdaPNMTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "adapnm"),
...
def _group_config(group: Mapping[str, Any], config: AdaPNMTrainingExecutorConfig) -> dict[str, Any]:
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("adapnm_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("fixed_decay", False)):
        blockers.append("adapnm_fixed_decay_not_supported_for_canary")
    if bool(group_cfg.get("ams_bound", False)):
        blockers.append("adapnm_ams_bound_not_supported_for_canary")
    if bool(group_cfg.get("adam_debias", False)):
        blockers.append("adapnm_adam_debias_not_supported_for_canary")
    if bool(group_cfg.get("adanorm", False)):
        blockers.append("adapnm_adanorm_not_supported_for_canary")
    return blockers
...
__all__ = ["AdaPNMTrainingExecutor", "AdaPNMTrainingExecutorConfig", "build_plugin_adapnm_training_executor"]
