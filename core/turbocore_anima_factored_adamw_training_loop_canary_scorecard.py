"""TrainingLoop canary for AnimaFactoredAdamW native dispatch."""
...
def build_anima_factored_adamw_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_anima_factored_adamw_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = list(case.get("blocked_reasons", []) or [])
    return {
        "schema_version": 1,
        "scorecard": "turbocore_anima_factored_adamw_training_loop_canary_scorecard_v0",
        "gate": "anima_factored_adamw_training_loop_native_canary",
        "ok": ok,
        ...
        "recommended_next_step": (
            "add AnimaFactoredAdamW e2e shadow matrix and rollout policy"
            if ok
            else "fix AnimaFactoredAdamW TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    def _fake_train_step(
        ...
        for item in params:
            item.grad = None
        loss = sum(((item.float() * 0.7) ** 2).mean() + item.float().mean() * 0.013 for item in params)
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "anima_factored_adamw_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["anima_factored_adamw_training_loop_did_not_emit_step"],
        }
    ...
    step_after = int(state.get("step").detach().cpu().item() if torch.is_tensor(state.get("step")) else state.get("step", 0))
    ...
    return {
        ...
        "training_path_enabled": False,
        "default_behavior_changed": False,
        "blocked_reasons": [] if ok else _dedupe(list(runtime.get("blocked_reasons", []) or []) + ["anima_factored_adamw_training_loop_native_step_missing"]),
    }


def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    device = torch.device("cuda")
    param = torch.nn.Parameter(torch.linspace(-0.15, 0.15, steps=256 * 256, device=device, dtype=torch.float32).reshape(256, 256))
    ...
    loop = TrainingLoop(
        ...
        turbocore_native_update_quantized_optimizer_kind="anima_factored_adamw",
    )
    loop.total_steps = 1
...
def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> None:
    param.grad = None
    loss = ((param.float() * 0.35) ** 2).mean() + param.float().mean() * 0.013
    loss.backward()
    optimizer.step()
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run AnimaFactoredAdamW TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_anima_factored_adamw_training_loop_canary_scorecard"]
