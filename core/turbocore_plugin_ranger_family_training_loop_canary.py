"""Shared TrainingLoop canary scorecards for Ranger21/Ranger25 native probes."""
...
from core.turbocore_plugin_ranger_family_training_support import RangerFamilySpec, _spec


def build_ranger_family_training_loop_canary_scorecard(kind: str) -> dict[str, Any]:
    spec = _spec(kind)
    if not torch.cuda.is_available():
        return _blocked_scorecard(spec, f"cuda_required_for_plugin_{kind}_training_loop_canary")
    case = _run_loop_case(spec)
    ok = bool(case.get("ok", False))
    blockers = _strings(case.get("blocked_reasons"))
    return {
        "schema_version": 1,
        "scorecard": f"turbocore_plugin_{kind}_training_loop_canary_scorecard_v0",
        "gate": f"plugin_{kind}_training_loop_native_canary",
        "ok": ok,
        ...
        + [
            f"{kind}_limited_formula_parity_probe",
            f"{kind}_e2e_shadow_matrix_refresh_required",
            f"{kind}_canary_rollout_policy_refresh_required",
            "product_rollout_review_missing",
        ],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            f"refresh selected Adam-like e2e shadow matrix and rollout policy for {kind}"
            if ok
...
def _run_loop_case(spec: RangerFamilySpec) -> dict[str, Any]:
    ...
    def _fake_train_step(
        ...
        for item in params:
            item.grad = None
        loss = sum(((item.float() * spec.loss_scale) ** 2).mean() + item.float().mean() * spec.loss_bias for item in params)
    ...
    ok = bool(
        ...
        and str(executor_result.get("optimizer_kind") or "") == spec.kind
...
def _make_loop(spec: RangerFamilySpec) -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    loop = TrainingLoop(
        ...
        turbocore_native_update_quantized_optimizer_kind=spec.kind,
    )
    loop.total_steps = 1
    return loop, param, optimizer


def _create_plugin_optimizer(param: torch.nn.Parameter, spec: RangerFamilySpec) -> torch.optim.Optimizer:
...
def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer, spec: RangerFamilySpec) -> None:
    param.grad = None
...
def _blocked_scorecard(spec: RangerFamilySpec, reason: str) -> dict[str, Any]:
    return {
        "schema_version": 1,
...
__all__ = ["build_ranger_family_training_loop_canary_scorecard"]
