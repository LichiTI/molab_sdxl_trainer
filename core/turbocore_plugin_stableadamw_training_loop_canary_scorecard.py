"""TrainingLoop canary for selected plugin StableAdamW native dispatch."""
...
SELECTED_OPTIMIZER = "stableadamw"
...
def build_plugin_stableadamw_training_loop_canary_scorecard() -> dict[str, Any]:
    if not torch.cuda.is_available():
        return _blocked("cuda_required_for_plugin_stableadamw_training_loop_canary")
    case = _run_case()
    ok = bool(case.get("ok", False))
    blockers = _strings(case.get("blocked_reasons"))
    return {
        "schema_version": 1,
        "scorecard": "turbocore_plugin_stableadamw_training_loop_canary_scorecard_v0",
        "gate": "plugin_stableadamw_training_loop_native_canary",
        "ok": ok,
        ...
        + [
            "stableadamw_e2e_shadow_matrix_refresh_required",
            "stableadamw_canary_rollout_policy_refresh_required",
            "product_rollout_review_missing",
        ],
        "blocked_reasons": blockers,
        "recommended_next_step": (
            "refresh selected Adam-like e2e shadow matrix and rollout policy for stableadamw"
            if ok
            else "fix selected plugin StableAdamW TrainingLoop canary blockers"
        ),
    }
...
def _run_case() -> dict[str, Any]:
    ...
    def _fake_train_step(
        ...
        for item in params:
            item.grad = None
        loss = sum(((item.float() * 0.21) ** 2).mean() + item.float().mean() * 0.004 for item in params)
    ...
    if not captured:
        return {
            "schema_version": 1,
            "ok": False,
            "probe": "plugin_stableadamw_training_loop_native_canary_v0",
            "result": result,
            "captured_step_count": 0,
            "blocked_reasons": ["plugin_stableadamw_training_loop_did_not_emit_step"],
        }
    ...
    ok = bool(
        ...
        and step_after == 2
        and type(optimizer).__name__ == "StableAdamW"
        and str(executor_result.get("optimizer_kind") or "") == "stableadamw"
        and {"exp_avg", "exp_avg_sq", "kahan_comp"}.issubset(state_keys)
    )
    return {
        "schema_version": 1,
        "ok": ok,
        ...
        "blocked_reasons": []
        if ok
        else _dedupe(_strings(runtime.get("blocked_reasons")) + ["plugin_stableadamw_training_loop_native_step_missing"]),
    }
...
def _make_loop() -> tuple[TrainingLoop, torch.nn.Parameter, torch.optim.Optimizer]:
    ...
    optimizer = _create_selected_stableadamw_plugin_optimizer(param)
    loop = TrainingLoop(
...
def _create_selected_stableadamw_plugin_optimizer(param: torch.nn.Parameter) -> torch.optim.Optimizer:
    ...
    trainer.config = LulynxConfig(
        ...
        learning_rate=1e-3,
        weight_decay=0.01,
        optimizer_args=f"{case.optimizer_args},foreach=False,kahan_sum=False",
...
def _prime_optimizer_state(param: torch.nn.Parameter, optimizer: torch.optim.Optimizer) -> None:
    param.grad = None
    loss = ((param.float() * 0.18) ** 2).mean() + param.float().mean() * 0.003
    loss.backward()
    optimizer.step()
...
def _blocked(reason: str) -> dict[str, Any]:
    return {
        ...
        "recommended_next_step": "run selected plugin StableAdamW TrainingLoop canary on CUDA",
    }
...
__all__ = ["build_plugin_stableadamw_training_loop_canary_scorecard"]
