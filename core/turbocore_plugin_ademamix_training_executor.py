"""Default-off AdEMAMix Triton TrainingLoop executor for selected plugin canaries."""
...
class AdEMAMixTrainingExecutorConfig:
    optimizer_kind: str = "ademamix"
    lr: float = 1.0e-3
    beta1: float = 0.9
    beta2: float = 0.999
    beta3: float = 0.9999
...
if triton is not None and tl is not None:
    ...
    def _ademamix_dense_kernel(
        ...
        slow = tl.load(exp_avg_slow_ptr + offsets, mask=mask, other=0.0).to(tl.float32)

        m_new = m * beta1 + g * (1.0 - beta1)
        v_new = v * beta2 + g * g * (1.0 - beta2)
        slow_new = slow * beta3_t + g * (1.0 - beta3_t)
        denom = tl.sqrt(v_new) / bias_correction2_sq + eps
        update = (m_new + slow_new * alpha_t) / denom
        p_new = p - step_size * update

        tl.store(exp_avg_ptr + offsets, m_new, mask=mask)
        tl.store(exp_avg_sq_ptr + offsets, v_new, mask=mask)
        tl.store(exp_avg_slow_ptr + offsets, slow_new, mask=mask)
        tl.store(param_ptr + offsets, p_new, mask=mask)


class AdEMAMixTrainingExecutor:
    """Launch a real AdEMAMix fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: AdEMAMixTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AdEMAMixTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("ademamix_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("ademamix_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("ademamix_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"ademamix_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("ademamix_maximize_not_supported")
        ...
        if not cases:
            blockers.append("ademamix_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_ademamix_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "ademamix_native_step_failed"),
    ...
    def _step_param(
        ...
        state["exp_avg_slow"] = exp_avg_slow
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["ademamix_training_executor_parameters_not_mutated"],
        }


def build_plugin_ademamix_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AdEMAMixTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> AdEMAMixTrainingExecutor:
    return AdEMAMixTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AdEMAMixTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AdEMAMixTrainingExecutorConfig:
    if isinstance(value, AdEMAMixTrainingExecutorConfig):
        return value
    ...
    t_alpha_beta3 = payload.get("t_alpha_beta3", group.get("t_alpha_beta3", None))
    return AdEMAMixTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "ademamix"),
        lr=float(payload.get("lr", group.get("lr", 1.0e-3)) or 1.0e-3),
        beta1=float(betas[0]),
        beta2=float(betas[1]),
        beta3=float(beta3),
        alpha=float(payload.get("alpha", group.get("alpha", 5.0)) or 5.0),
        t_alpha_beta3=None if t_alpha_beta3 is None else float(t_alpha_beta3),
...
def _group_config(group: Mapping[str, Any], config: AdEMAMixTrainingExecutorConfig) -> dict[str, Any]:
    betas = group.get("betas", (config.beta1, config.beta2, config.beta3))
    return {
        "lr": float(group.get("lr", config.lr) or config.lr),
        "beta1": float(betas[0]),
        "beta2": float(betas[1]),
        "beta3": float(betas[2] if len(betas) >= 3 else config.beta3),
        "alpha": float(group.get("alpha", config.alpha) or config.alpha),
        "t_alpha_beta3": group.get("t_alpha_beta3", config.t_alpha_beta3),
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    blockers: list[str] = []
    if group_cfg.get("t_alpha_beta3") is not None:
        blockers.append("ademamix_t_alpha_beta3_schedule_not_supported_for_canary")
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("ademamix_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("weight_decouple", False)):
        blockers.append("ademamix_weight_decouple_not_supported_for_canary")
    if bool(group_cfg.get("fixed_decay", False)):
        blockers.append("ademamix_fixed_decay_not_supported_for_canary")
    if bool(group_cfg.get("cautious", False)):
        blockers.append("ademamix_cautious_not_supported_for_canary")
    if bool(group_cfg.get("stable_adamw", False)):
        blockers.append("ademamix_stable_adamw_not_supported_for_canary")
    return blockers
...
__all__ = ["AdEMAMixTrainingExecutor", "AdEMAMixTrainingExecutorConfig", "build_plugin_ademamix_training_executor"]
