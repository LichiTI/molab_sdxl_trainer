"""Default-off AdaBelief Triton TrainingLoop executor for selected plugin canaries."""
...
class AdaBeliefTrainingExecutorConfig:
    optimizer_kind: str = "adabelief"
    lr: float = 1.0e-3
    beta1: float = 0.9
    beta2: float = 0.999
    eps: float = 1.0e-16
...
if triton is not None and tl is not None:
    ...
    def _adabelief_dense_kernel(
        ...
        residual = g - m_new
        v_new = v * beta2 + residual * residual * (1.0 - beta2) + eps
...
class AdaBeliefTrainingExecutor:
    """Launch a real AdaBelief fp32 Triton kernel against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: AdaBeliefTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("AdaBeliefTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("adabelief_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("adabelief_training_executor_requires_cuda_params")
        if len(self.params) != 1 or len(self.optimizer.param_groups) != 1:
            return _blocked("adabelief_training_executor_requires_single_param_group_canary")
        if not triton_adamw_flat_available() or triton is None:
            return _blocked(f"adabelief_triton_unavailable:{triton_adamw_flat_unavailable_reason()}")
        if bool(getattr(self.optimizer, "maximize", False)):
            return _blocked("adabelief_maximize_not_supported")
        ...
        if not cases:
            blockers.append("adabelief_training_executor_no_grad_params")
        ok = bool(cases) and all(case.get("ok") is True for case in cases)
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_adabelief_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "adabelief_native_step_failed"),
    ...
    def _step_param(
        ...
        if param.dtype != torch.float32 or (param.grad is not None and param.grad.dtype != torch.float32):
            return _case_failed("adabelief_training_executor_requires_float32")
        if not param.is_contiguous() or param.grad is None or not param.grad.is_contiguous():
            return _case_failed("adabelief_training_executor_requires_contiguous_tensors")
        ...
        step_size = float(group_cfg["lr"]) / (1.0 - math.pow(float(group_cfg["beta1"]), step))
        ...
        return {
            ...
            "blocked_reasons": [] if mutated else ["adabelief_training_executor_parameters_not_mutated"],
        }


def build_plugin_adabelief_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: AdaBeliefTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> AdaBeliefTrainingExecutor:
    return AdaBeliefTrainingExecutor(optimizer=optimizer, params=params, config=config, workspace_root=workspace_root)


def _normalize_config(
    value: AdaBeliefTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> AdaBeliefTrainingExecutorConfig:
    if isinstance(value, AdaBeliefTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = payload.get("betas", group.get("betas", (0.9, 0.999)))
    return AdaBeliefTrainingExecutorConfig(
        optimizer_kind=str(payload.get("optimizer_kind") or "adabelief"),
        lr=float(payload.get("lr", group.get("lr", 1.0e-3)) or 1.0e-3),
        beta1=float(betas[0]),
        beta2=float(betas[1]),
        eps=float(payload.get("eps", group.get("eps", 1.0e-16)) or 1.0e-16),
...
def _group_config(group: Mapping[str, Any], config: AdaBeliefTrainingExecutorConfig) -> dict[str, Any]:
...
def _unsupported_group_blockers(group_cfg: Mapping[str, Any]) -> list[str]:
    ...
    if abs(float(group_cfg.get("weight_decay", 0.0) or 0.0)) > 0.0:
        blockers.append("adabelief_weight_decay_not_supported_for_canary")
    if bool(group_cfg.get("rectify", False)):
        blockers.append("adabelief_rectify_not_supported_for_canary")
    if bool(group_cfg.get("ams_bound", False)):
        blockers.append("adabelief_ams_bound_not_supported_for_canary")
    if bool(group_cfg.get("adam_debias", False)):
        blockers.append("adabelief_adam_debias_not_supported_for_canary")
    if bool(group_cfg.get("adanorm", False)):
        blockers.append("adabelief_adanorm_not_supported_for_canary")
    return blockers
...
__all__ = ["AdaBeliefTrainingExecutor", "AdaBeliefTrainingExecutorConfig", "build_plugin_adabelief_training_executor"]
