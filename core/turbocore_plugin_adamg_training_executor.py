"""Default-off training executor for selected plugin AdamG native dispatch."""
...
class PluginAdamGTrainingExecutorConfig:
    lr: float = 1.0
...
class PluginAdamGTrainingExecutor:
    """Launch selected AdamG native steps against live optimizer state."""
    ...
    def __init__(
        ...
        *,
        ...
        config: PluginAdamGTrainingExecutorConfig | Mapping[str, Any] | None = None,
        workspace_root: str | Path | None = None,
    ) -> None:
        self.optimizer = optimizer
        self.params = [param for param in params if isinstance(param, torch.nn.Parameter) and param.requires_grad]
        if not self.params:
            raise ValueError("PluginAdamGTrainingExecutor requires trainable parameters")
    ...
    def __call__(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not bool(payload.get("training_dispatch", False)) or not bool(payload.get("training_path_enabled", False)):
            return _blocked("plugin_adamg_training_executor_requires_training_dispatch")
        if self.config.require_native_cuda and any(param.device.type != "cuda" for param in self.params):
            return _blocked("plugin_adamg_training_executor_requires_cuda_params")
        native = self._load_native()
        if native is None:
            return _blocked("plugin_adamg_training_dispatch_entrypoints_missing")
        ...
        if not cases:
            blockers.append("plugin_adamg_training_executor_no_grad_params")
        return {
            "schema_version": 1,
            "executor": "turbocore_plugin_adamg_training_executor_v0",
            "ok": ok,
            "reason": "called" if ok else (blockers[0] if blockers else "plugin_adamg_native_step_failed"),
    ...
    def _step_param(
        ...
        if type(self.optimizer).__name__.lower() != "adamg":
            return _case_failed("plugin_adamg_training_executor_optimizer_not_adamg", "plugin_adamg_optimizer_class_unsupported")
        if bool(group_cfg.get("maximize", False)):
            return _case_failed("plugin_adamg_native_step_requires_plain_gradient", "plugin_adamg_gradient_mode_unsupported")
        if param.dtype != torch.float32 or param.grad is None or param.grad.dtype != torch.float32:
            return _case_failed("plugin_adamg_native_step_requires_float32", "plugin_adamg_native_step_dtype_unsupported")
        if not param.is_contiguous() or not param.grad.is_contiguous():
            return _case_failed("plugin_adamg_native_step_requires_contiguous_param_grad", "plugin_adamg_native_step_layout_unsupported")
        state = self.optimizer.state[param]
        if "m" not in state or "v" not in state or "r" not in state:
            return _case_failed("plugin_adamg_live_state_missing", "plugin_adamg_live_state_missing")
        m = state["m"]
        v = state["v"]
        r = state["r"]
        if not (torch.is_tensor(m) and torch.is_tensor(v) and torch.is_tensor(r)):
            return _case_failed("plugin_adamg_live_state_tensors_missing", "plugin_adamg_live_state_missing")
        if m.dtype != torch.float32 or v.dtype != torch.float32 or r.dtype != torch.float32:
            return _case_failed("plugin_adamg_native_state_requires_float32", "plugin_adamg_native_state_dtype_unsupported")
        if not m.is_contiguous() or not v.is_contiguous() or not r.is_contiguous():
            return _case_failed("plugin_adamg_native_state_requires_contiguous", "plugin_adamg_native_state_layout_unsupported")
        registry = TurboCoreTensorHandleRegistry(namespace="plugin_adamg_training_executor")
        ...
        try:
            ...
            r_record = registry.register(r_flat, role="r", expected_numel=int(param_flat.numel()))
            ...
            if not bool(session.get("ok", False)):
                return _case_failed(
                    "plugin_adamg_tensor_binding_session_create_failed",
                    "plugin_adamg_tensor_binding_session_create_failed",
                    session=session,
                )
            ...
            try:
                launch = dict(
                    native.tensor_binding_session_cuda_adamg_tensor_probe(
                        session_id,
                        json.dumps(
                            {
                                **dict(group_cfg),
            ...
            return _case_failed(f"{type(exc).__name__}: {exc}", "plugin_adamg_native_step_call_failed")
        if not bool(launch.get("ok", False)):
            return _case_failed(
                str(launch.get("reason") or "plugin_adamg_native_step_failed"),
                "plugin_adamg_native_step_failed",
                launch=launch,
            )
        return {
            "schema_version": 1,
...
def build_plugin_adamg_training_executor(
    *,
    optimizer: torch.optim.Optimizer,
    params: Iterable[torch.nn.Parameter],
    config: PluginAdamGTrainingExecutorConfig | Mapping[str, Any] | None = None,
    workspace_root: str | Path | None = None,
) -> PluginAdamGTrainingExecutor:
    return PluginAdamGTrainingExecutor(
        optimizer=optimizer,
        params=params,
        config=config,
...
def _normalize_config(
    value: PluginAdamGTrainingExecutorConfig | Mapping[str, Any] | None,
    optimizer: torch.optim.Optimizer,
) -> PluginAdamGTrainingExecutorConfig:
    if isinstance(value, PluginAdamGTrainingExecutorConfig):
        return value
    payload = dict(value or {})
    group = optimizer.param_groups[0] if optimizer.param_groups else {}
    betas = _adamg_beta_triplet(payload.get("betas", group.get("betas", (0.95, 0.999, 0.95))), group)
    return PluginAdamGTrainingExecutorConfig(
        lr=float(payload.get("lr", group.get("lr", 1.0))),
        betas=betas,
        p=float(payload.get("p", group.get("p", getattr(optimizer, "p", 0.2)))),
        q=float(payload.get("q", group.get("q", getattr(optimizer, "q", 0.24)))),
...
def _group_config(group: Mapping[str, Any], config: PluginAdamGTrainingExecutorConfig) -> dict[str, Any]:
    betas = _adamg_beta_triplet(group.get("betas", config.betas), group)
    return {
        "lr": float(group.get("lr", config.lr)),
        "betas": [float(betas[0]), float(betas[1]), float(betas[2])],
        "p": float(group.get("p", config.p)),
        "q": float(group.get("q", config.q)),
...
def _adamg_beta_triplet(value: Any, group: Mapping[str, Any] | None = None) -> tuple[float, float, float]:
    values = list(value) if isinstance(value, (list, tuple)) else []
    while len(values) < 2:
        values.append((0.95, 0.999)[len(values)])
    if len(values) < 3:
        group_beta3 = dict(group or {}).get("beta3", 0.95)
...
__all__ = [
    "PluginAdamGTrainingExecutor",
    "PluginAdamGTrainingExecutorConfig",
    "build_plugin_adamg_training_executor",
]
